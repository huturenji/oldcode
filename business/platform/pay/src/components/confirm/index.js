...
export default class Confirm {
  ...
  create(){
    const pcWidth = store.state.config.pcWidth;
    ...
    if(!mask || mask.length==0){
      ...
      mask.style.cssText = `position: fixed;z-index: ${store.getters.zIndex(2)};top: 0;right: 0;left: 0;bottom: 0;background: rgba(0, 0, 0, 0.6);`
    ...
    dialog.style.cssText = `position: fixed;display: table;z-index: ${store.getters.zIndex(3)};width: 80%;max-width: 300px;left: 50%;top: 50%;transform: translate(-50%, -50%);margin: auto;background-color: #FFFFFF;`
