...
export function reFreshPage(funName) {
    ...
    extendUtils.callHandler('RegisterMenuFunction', extendUtils.isPC() ? null : array);
    extendUtils.registerHandler('clickMenuCallBack', function (data) {
      var dataJson = JSON.parse(data);
      // 找出menuList中的与注册事件对应的函数并且执行
      if (array) {
        ...
        for (var i = 0; i < array.length; i++) {
          if (dataJson.menuId == array[i].menuId && array[i].func) {
            array[i].func();
            break;
          }
        }
...
export function toggleReturnBtn(show) {
    var arr = null;  
      //商旅通超级首页入口进入的不隐藏返回按钮
    if(!!extendUtils.isPC('indexPageType') && extendUtils.isPC('indexPageType') == 'ind'){
        return;
    }else{
 ...
 */
export function goBackPageCore(param) {
    if (window.AlipayJSBridge) {
        let backSteps = param.backSteps;
        if (!!backSteps) {
            _mPaasPopTo(backSteps,param.loadData)
        }else{
            goBackFunction(param.url, param);
        }
    }else{
        extendUtils.callHandler('goBackFunction', window.JSON.stringify(param));
    }
}
/**
 * 回退页面
 * @param {Object} url  //目标页面url
 */
export function goBackFunction(url, param){
    extendUtils.PromiseJSBridgeReady(function () {
        if (!window.AlipayJSBridge) {
            extendUtils.callHandler('goBackFunction', JSON.stringify({
                loadData: '',
                url: url
            }));
            return;
 ...
 * @param {*} url 
 */
export function bridgeOpenPage(url) {
    // extendUtils.PromiseJSBridgeReady(() => {//TODO 暂时屏蔽，需要处理jsbridge的默认时间
        //url中参数sswbv_multipage=false，当前页面重定向，其它新开页面
        let sswbv_multipage = true;
        let sswbvMultiPageRegRes = url.match(/[?|&]sswbv_multipage=([^&]*)(&|$)/);
        ...
        if (sswbv_multipage == 'false') {
            ...
            window.AlipayJSBridge.call('pushWindow', {
                ...
                param: {
                    ...
                    titleBarColor: 4688622 //兆日蓝，加载页面之前就设置标题背景颜色
                }
            });
        } else {
            window.open(url);
            console.error("bridgeOpenPage：error");
        }
    // })
}
 ...
 * @param {*} loadData   数据
 */
function _mPaasPopTo(backSteps, loadData) {
    extendUtils.PromiseJSBridgeReady(() => {
