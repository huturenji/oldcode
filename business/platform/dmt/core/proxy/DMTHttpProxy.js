 ...
 */
export default class DMTHttpProxy {
  constructor(httpPostProcessors, httpPreProcessors) {
  ...
  interruptAjaxReq(arg, xhr, from) {
    ...
    if (from == 1) {
      ...
      //是否针对接口增加了 业务处理
      let findUlrProcess = this.httpPreProcessors.getProcess4Cache(
  ...
  interruptAjaxRes(xhr) {
    ...
    //是否针对接口增加了 业务处理
    let findUlrProcess = this.httpPostProcessors.getProcess4Cache(
    ...
    if (findUlrProcess) {
      ...
      if (xhr.status == 200 && xhr.responseType.indexOf("json") != -1) {
        ...
        if(postParam && (postParam.resultCode == 0 || postParam.code == 0)){
