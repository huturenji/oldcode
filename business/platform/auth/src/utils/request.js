 ...
 */
 export function request(param={},timeoutcb){
    ...
    return new Promise(function(res, rej){
        ...
        var url = param.url,method = param.method || 'get',data = param.data;
        ...
        //超时时间与返回头
        var timeout = param.timeout,responseType=param.responseType || 'json';
    ...
    　　}
        if(extendUtils.isBoolean(param.withCredentials)){
...
export function kcRequest(param, ...others){
    param.headers = Object.assign({}, param.headers, {
        'content-type': 'application/x-www-form-urlencoded'
    })
    return request(param, ...others)
}
...
export function joinObject(object,symbol){
    if(extendUtils.isEmpty(object)){
        return ''
    }
    if(!extendUtils.isObject(object)){
        return object
    }
    let objStr = '';
    for(var o in object){
