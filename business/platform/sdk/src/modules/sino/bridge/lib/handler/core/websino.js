...
/*
 * @Descripttion: websino jsbridge方法
 * @version: 1.0
 * @Author: yg
...
const BRIDGE_TYPE = constant.BRIDGE_TYPE.WEBSINO;//mpaas bridge类型为sino
...
class websinobridge {
  ...
  constructor(){
    ...
    this.bridge = _window.WeboaWebViewJavascriptBridge;
    this.init = this.init.bind(this);
  }

  /**
   * 初始化，获取T信 jsbridge的WeboaWebViewJavascriptBridge 
   */
  init(){
    const that = this;
    return new Promise((res,rej)=>{
        if(that.bridge){
            ...
            doc.addEventListener('WeboaWebViewJavascriptBridgeReady',()=>{
                that.bridge=_window.WeboaWebViewJavascriptBridge
                res(that);
            }, false);
        }
    });
...
export const bridge = new websinobridge();
