...
class ShareHandler extends base{
    ...
    openChat(shareData,toUserItem) {
        ...
        sinosdk.sino.execAction(JSON.stringify(openJson))
    }
    /**
	 * 异步获取登录用户名字
	 */	
     ...
     * 获取微信ticket
     */
    proxyWxTicket(param,httpConfig) {
        return this.request('/content/v1/proxyWxTicket', param, Object.assign({
            noSSOFlag: true,
            method: 'get',
            noZipFlag: true
        },httpConfig));
     ...
     * @param json 游客header
     */ 
    setThirdShareInfo(shareInfo,requestHeader){
        //伴正事分享
        try {
        ...
        //微信分享暂时不上2021年3月29日
        //处理微信分享
        this.setWxShare(shareInfo,requestHeader);
    }
    /**
     * 注册微信分享信息
     * @param json 分享信息
     * @param json 游客header
     */ 
    setWxShare(shareInfo,requestHeader){
        let that = this;
        wx.ready(function(){
        ...
        if(this.checkMicromessenger()){
            //注册微信验证通过后的回调事件
            this.proxyWxTicket({},requestHeader).then((res)=>{
    ...
    */
    ...
    async registerBizmateShare(shareDate) {  
        ...
        let shareParam = {
            ...
            appId: appInfo.appId+'',//小应用Id
            appName: appInfo.appName||'资讯',//小应用名字,无合法appId时使用appName
