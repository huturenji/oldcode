...
function getVerificationCode(realmName) {
    ...
    params.append('smsTemplateCode', 3)//传1 注册，2 登录 ，3 重置
...
function cancelpopwindow() {
    ...
    // console.log('isResetPwdOk', isResetPwdOk)
    if(isResetPwdOk){
        document.getElementById('banckbtn').click()
    }
}
...
function resetpwd(realmName) {
    ...
    let userServerDomain = sessionStorage.getItem('userServerDomain')
    let configId = sessionStorage.getItem('configId')
    let theUrl = userServerDomain + '/auth/realms/' + realmName + '/bplus_user_defined/restPassword'
    let params = new URLSearchParams();
    params.append('configId', configId)
...
function regInputValue(type) {
    ...
    if (type == 'phone') {
        ...
        if (!!password && password.length > 5 && password.length < 21) {
            ...
            errorBox.style.display = "block"
            errorBox.innerHTML = (type == 'ID_password' ? "请输入密码（6-20位字符）" : "请再次输入新密码（6-20位字符）")
            return false
        }
