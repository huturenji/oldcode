import base from 'common/lib/requestHandler/base';
...
function getIdTokenByTGC() {
    return new Promise(resolve => {
        ...
        terType = !terType || isNaN(terType) ? 0 : parseInt(terType);
        let param = {
...
export async function guestAuthorize(){
    let guestToken = await guestLogin();
    let primaryKey = requestHanlder.getGuestPrimaryKey();
    extendUtils.setStorage(primaryKey,JSON.stringify(guestToken));
    extendUtils.setSession('identity','guest');
}
    ...
    return new Promise(async resolve => {
        extendUtils.loadScript({
            ...
            onload: async function () {
                ...
                let _config = {
                    channelId: channelId,
                    primaryKey: requestHanlder.getGuestPrimaryKey()
                }
