 ...
 * addNumShow([dom]) +1动画效果
 * twinkleAnimation([dom]) 闪烁动画效果
 * shakeAnimation([dom]) 抖动动画效果
 */
export default class AddCart extends BaseAnimation{
    constructor(endDom){
        super();
     ...
     * +1浮动效果
     * @param dom 在dom上浮动+1
     * @param text 需要显示的内容，默认是“+1”
     */
    addNumShow(dom, text='+1'){
        const styleId = 'addCartNum';
        let rect = dom.getBoundingClientRect();
        let addNum = document.createElement('div');//单例
        //初始样式,并插入body
        addNum.innerText = text;
        addNum.style.fontSize = '0.32rem';
        addNum.style.color = '#FF4E3A';
        addNum.style.fontWeight = 'bold';
        addNum.style.padding = '0.02rem 0.03rem';
        addNum.style.position = 'fixed';
        addNum.style.left = rect.left + dom.offsetWidth / 2 + 'px';//水平居中
        addNum.style.top = rect.top - 10 + 'px';//默认向上偏移10px
        addNum.style.zIndex = '999';
        addNum.style.visibility = 'hidden';//开始不可见。不能用display:none，否则拿不到宽高，下面无法计算
        dom.parentElement.appendChild(addNum);

        this.addStyle(styleId, `@-webkit-keyframes ${styleId} {
            0% {
                top: ${rect.top-20}px;
                opacity: 1
            }
        ...
        //重新覆盖样式，以生成动画
        addNum.style.left = parseFloat(addNum.style.left) - addNum.offsetWidth / 2 + 'px';
        addNum.style.visibility = 'visible';
        addNum.style.opacity = '0';
        addNum.style.animation = `${styleId} 2s`;
        addNum.style.display = "block";//再添加上去，动画就出来了
        //用完就删，保证动画能再次执行（PC端兼容问题）
        setTimeout(()=>{
            addNum.remove();
        }, 2000)
    }
     ...
     * @param {*} dom 
     */
    twinkleAnimation(dom){
        const time = 0.1;
        return new Promise((resolve, reject)=>{
            try{
                const styleId = 'twinkleAnimation';
                //动画样式
                this.addStyle(styleId, `@-webkit-keyframes ${styleId} {
                    0% {
                        transform: scale(1);
                ...
                //添加动画
                dom.style.animation = `${styleId} ${time}s`;
            }catch(e){
                console.error(e);
                reject();
            }
    ...
    shakeAnimation(dom){
        ...
        return new Promise((resolve, reject)=>{
            try{
                //动画样式
                let keyFrames = `@-webkit-keyframes shakeAnimation {
                    0% {
                        transform: rotate(0)
                    }
                    25% {
                ...
                //添加动画
                dom.style.animation = `shakeAnimation ${time}s`;
            }catch(e){
                console.error(e);
                reject();
            }
            setTimeout(()=>{
                dom.style.animation = '';//用完就移除，防止下次不生效
                resolve();
            },time * 1000)
        })
    }
