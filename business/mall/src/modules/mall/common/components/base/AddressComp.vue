<template>
    <div class="addresscomp-container" ref="addressContainer">
        <AddressTitle :showTitle='showTitle' :title="titleName" @closePopup='closePopup' :showBottomBorder='true'></AddressTitle>
            ...
            <!-- 顶部选择后的地址显示 -->
                ...
                <div class='area-list' ref='areaList'>
                    ...
                    <div v-for="(temp, i) in dataList" :key="i" v-show="temp.show" >
                        ...
                        >
                            <Icon v-if="item.selected" type='icon_mall_select' size='.36'></Icon>
                            <span>{{item.name}}</span>
                        </div>
                    </div>
                </div>
...
export default {
    ...
    methods:{
        ...
        async setCheck(arr){
            let _this = this;
            this.isShow = false;
            this.$loading.show({dom: this.$refs.addressContainer});
            
            for(let i = 0;i<arr.length;i++){
                let temp = this.dataList[i];
                let data;
                if(!!extendUtils.getStorage(requestHandler.primaryKey+ '_provinceList') && i == 0){
                    data = JSON.parse(extendUtils.getStorage(requestHandler.primaryKey+ '_provinceList'));
                }else{
        ...
        async initProvinceList(){
            ...
            extendUtils.setStorage(requestHandler.primaryKey+ '_provinceList', JSON.stringify(this.dataList[0].list));
        },
         ...
         */        
        getAddressData(apiName, id, transportId){
            
            let param = {supplierId: requestHandler.supplierId};
            //增加transportCode用于支持苏宁查询省市区id不用再传
            if(!!transportId){
                param = Object.assign({},param,{id:transportId})
            }
            // this.$loading.show({dom: this.$refs.areaList});
            return new Promise((resolve,reject)=>{
                requestHandler[apiName](param).then(res=>{
                    // this.$loading.hide();
        ...
        initData(){
            this.dataList = [
                {
                    ...
                    transportId: '',
                    name:'',
                    list: !!extendUtils.getStorage(requestHandler.primaryKey+ '_provinceList') ? JSON.parse(extendUtils.getStorage(requestHandler.primaryKey+ '_provinceList')) : [],
                    show: true,
                },
                {
                    type:'city',
             ...
             * 获取省份数据列表
             */
            if(!extendUtils.getStorage(requestHandler.primaryKey+ '_provinceList')){
