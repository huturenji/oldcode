...
import { Form, Spin, Input, InputNumber, Radio, DatePicker, Table,Empty,Select } from 'antd';
...
export default class AddCoupon extends Component {
      ...
      this.props.form.validateFieldsAndScroll((err, values) => {
          if (!err) {
              ...
              if(publishType == 1){ // 免费领取
                  ...
                  values.limitReceive = 0; //活动赠送 和 密码领取的优惠券不限制会员领取的数量
              ...
              if (query.id != undefined && query.id > 0 && query.type == 'edit') {
                  ...
                  dis_type = 'coupon/edit_coupon';
              } else {
                  //新增优惠券
                  dis_type = 'coupon/add_coupon';
              }
              if(!couponFlag){
  ...
  render() {
      const {
          ...
          cat_data, coupon_detail,enableFlag, isFirstLoading, publishType, query,link_data,link_type,channelType,
          channelList,channelProps,fullPath,categoryVOList,linkName,couponFlag
      } = this.state;
      let {
              ...
              <Spin spinning={loading}>
                ...
                <Fragment>
                    ...
                    <div className={`${promotion.full_acm_activity} ${global.flex_column_start_start}`}>
                            ...
                            <div className={`${promotion.right}`}>
                                ...
                                >
                                    {getFieldDecorator('couponName', {
                                        ...
                                        <Input maxLength={20} style={{ width: 400 }} placeholder={`${sldComLanguage('请输入优惠券名称')}`} />,
                                    )}
                                </FormItem>
                            </div>
                        </div>
                        <div className={`${promotion.item} ${global.flex_row_start_start}`}>
                            ...
                            <div className={`${promotion.right}`}>
                                <FormItem
                                    style={{ width: 300 }}
                                    extra={`${sldComLanguage('最多10000张，请谨慎设置')}`}
                                >
                        ...
                        <div className={`${promotion.item} ${global.flex_row_start_start}`}>
                            <div className={`${promotion.left}`}>
                                <span style={{ color: 'red' }}>*</span>{sldComLanguage('与店铺优惠券叠加')}
                            </div>
                            <div className={`${promotion.right}`}>
                                <FormItem
                                    style={{ width: 300 }}
                    ...
                    <div className={`${promotion.item} ${global.flex_row_start_start}`}>
                        ...
                        <div className={`${promotion.right}`}>
                            <FormItem
                                extra={`${sldComLanguage('输入90代表9折，85代表85折')}`}
                                style={{ width: 190 }}
                            >
                                <div className={global.flex_row_start_center}>
                                    <span
                            ...
                            >
                                <div className={global.flex_row_start_center}>
                                    ...
                                    {getFieldDecorator('discountLimitAmount', {
                                        initialValue: coupon_detail.discountLimitAmount,rules:[{
                                            required: true,
                                            message: `${sldComLanguage('请输入优惠金额')}`
                                        }]
                                    })(
                                        <InputNumber
                                            max={99999999}
