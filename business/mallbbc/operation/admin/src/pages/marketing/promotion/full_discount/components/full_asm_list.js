...
import AddFullAsm from './add_full_asm';
...
export default class FullAsmList extends Component {
    constructor(props) {
        ...
        this.state = {
            ...
            columns: [
                ...
                {
                    title: `${sldComLanguage(`${sldComLanguage('操作')}`)}`,
                    align: 'center',
                    width: 100,
                    render: (text, record) => (
                        <Fragment>
                            ...
                            {/* 只有未开始、进行中的才可以失效 */}
                            <AuthBtn eventKey={['invalid_full_asm']} btnAuth={btnAuth}>
                                {
                            ...
                            {/* 只有待发布、已失效、已结束的才可以删除 */}
                            <AuthBtn eventKey={['delete_full_asm']} btnAuth={btnAuth}>
                                {
  ...
  operate = (id, type) => {
      ...
      let dis_type = '';
      if (type == 'invalid') {
          dis_type = 'full_discount/invalid_full_asm';
          param_data.fullId = id;
      } else if (type == 'del') {
          dis_type = 'full_discount/del_full_asm';
          param_data.fullId = id;
      } else if (type == 'publish') {
          dis_type = 'full_discount/publish_full_asm';
          param_data.fullId = id;
      }
  ...
  get_list = (params) => {
      ...
      const { dispatch } = this.props;
      dispatch({
          type: 'full_discount/get_full_asm_list',
          payload: { ...params },
          callback: (res) => {
