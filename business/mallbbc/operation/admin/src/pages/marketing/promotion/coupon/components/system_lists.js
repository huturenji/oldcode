...
@Form.create()
export default class SystemLists extends Component {
    goods_spec_columns = [
        {
            title: ' ',
            dataIndex: 'id',
            ...
            columns: [
                ...
                },
                {
                    ...
                    render: (text, record) => <Link to={{
                        pathname: '/marketing_promotion/coupon_to_receive_list',
                        query: {
                            id: record.couponId
                        }
                    }}
                    ...
                    render: (text, record) => (
                        <Fragment>
                            <AuthBtn eventKey={["audit_coupon"]} btnAuth={btnAuth}>
                                ...
                                <Fragment>
                                    <Link to={{
                                        pathname: '/marketing_promotion/coupon_to_view',
                                        query: {
                                            id: record.couponId,
                                            type: 'system'
                                        }
                            ...
                            <AuthBtn eventKey={["view_coupon"]} btnAuth={btnAuth}>
                                <Link to={{
                                    pathname: '/marketing_promotion/coupon_to_view',
                                    query: {
                                        id: record.couponId,
                            ...
                            <AuthBtn eventKey={["edit_coupon"]} btnAuth={btnAuth}>
                                {/* 审核拒绝才可以编辑 */}
                                {record.state == 7 &&
                                <Fragment>
                                    <span className={global.splitLine}></span>
                                    <Link to={{
                                        pathname: '/marketing_promotion/coupon_to_add',
                                        query: {
                                            id: record.couponId,
                                            type: 'edit'
                                        }
                            ...
                            <AuthBtn eventKey={["copy_coupon"]} btnAuth={btnAuth}>
                                <span className={global.splitLine}></span>
                                <Link to={{
                                    pathname: '/marketing_promotion/coupon_to_add',
                                    query: {
                            ...
                            <AuthBtn eventKey={["invalid_coupon"]} btnAuth={btnAuth}>
                                {/* 只有未开始、进行中的才可以失效 */}
                  ...
                  <Fragment>
                      ...
                      {sldPopConfirmDiy('leftBottom', `${sldComLanguage('失效后不可恢复，是否确定失效？')}`, () => this.operate(record.couponId, 'invalid'), `${sldComLanguage('确定')}`, `${sldComLanguage('取消')}`,
                          sldtbaleOpeBtnText(`${sldComLanguage('失效')}`, () => null))}
                            ...
                            <AuthBtn eventKey={["delete_coupon"]} btnAuth={btnAuth}>
                  ...
                  <Fragment>
                      ...
                      {sldPopConfirmDiy('leftBottom', `${sldComLanguage('删除后不可恢复，是否确定删除？')}`, () => this.operate(record.couponId, 'del'), `${sldComLanguage('确定')}`, `${sldComLanguage('取消')}`,
                          sldtbaleOpeBtnText(`${sldComLanguage('删除')}`, () => null))}
                            ...
                            <AuthBtn eventKey={["end_coupon"]} btnAuth={btnAuth}>
                                {(record.state == 4) &&
                                <Fragment>
                                    <span className={global.splitLine} />
  ...
  operate = (id, type) => {
      ...
      let dis_type = '';
      if (type == 'invalid') {
          dis_type = 'coupon/invalid_coupon';
          param_data.couponId = id;
      } else if (type == 'copy') {
          dis_type = 'coupon/copy_coupon';
          param_data.couponId = id;
      } else if (type == 'del') {
          dis_type = 'coupon/del_coupon';
          param_data.couponId = id;
      } else if (type == 'recommend') {
          dis_type = 'coupon/recommend_coupon';
          param_data = id;
      } else if (type == 'stopGet') {
          dis_type = 'coupon/stop_coupon';
          param_data.couponId = id;
      }
  ...
  search = (data) => {
      ...
      if (values.search_user_time) {
          ...
          values.effectiveEnd = values.search_user_time[1] ? `${values.search_user_time[1].format(dateFormat) } 23:59:59` : '';
  ...
  render() {
      ...
      return (
          <AuthBtn eventKey={['view_coupon']} btnAuth={btnAuth} showPage>
              <div className={global.common_page} style={{ flex: 1, paddingTop: 0 }}>
                  ...
                  {/*公共功能条-start*/}
                  <AuthBtn eventKey={["add_coupon"]} btnAuth={btnAuth}>
                      <div className={global.operate_bg}>
                          ...
                          >
                              {enableFlag == 0
                                  ? sldIconBtnNo(() => null, `${sldComLanguage('新建优惠券')}`, 7, 0, 14, 14, 3, 'fabu1', '#FA6F1E', `${sldComLanguage('优惠券活动未开启')}`)
                                  : sldIconBtn(() => null, `${sldComLanguage('新建优惠券')}`, 7, 0, 14, 14, 3, 'fabu1', '#FA6F1E')
                              }
