...
export default class LabelLists extends Component {
        ...
        this.state = {
            ...
            columns: [
                ...
                },
                {
                    ...
                    render: (text, record) => (
                        <Switch
                            onChange={(checked) => this.operateLabel({labelId:record.labelId,state:checked?1:0},'switch')}
                ...
                },
                {
                    ...
                    render: (text, record) => (
                        <Fragment>
                            <AuthBtn eventKey={["edit_seckill_label"]} btnAuth={btnAuth}>
                                {sldtbaleOpeBtnText('编辑', () => this.editLabel(record))}{/*编辑*/}
                            </AuthBtn>
                           
                            <span className={global.splitLine} />
                            <AuthBtn eventKey={["delete_seckill_label"]} btnAuth={btnAuth}>
                                {/*删除后不可恢复，是否确定删除？*/}
                                {sldPopConfirmDiy('leftBottom', `${sldComLanguage('删除后不可恢复，是否确定删除')}`, () => this.operateLabel(record.labelId, 'del'), `${sldComLanguage('确定')}`, `${sldComLanguage('取消')}`,
	...
	operateLabel = (id, type) => {
	    ...
	    let param_data = {};
	    if (type == 'del') {
	        dis_type = 'seckill/del_label';
	        param_data = { labelId: id };
	    } else if (type == 'edit') {
	        dis_type = 'seckill/edit_label';
	        param_data = id;
	    } else if (type == 'switch') {
	        dis_type = 'seckill/switch_label';
	        param_data = id;
	    }
	    dispatch({
	        type: dis_type,
	...
	sldSearch = (val) => {
	    ...
	    this.get_list({ pageSize: pageSize, labelName: val , pageIndex :1 });
	};
	    ...
	    return (
	        <div className={global.common_page} style={{ flex: 1 }}>
	            <div className={global.operate_bg}>
	                <AuthBtn eventKey={['add_seckill_label']} btnAuth={btnAuth}>
