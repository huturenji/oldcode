...
import AddFullAld from './add_full_ald';
...
export default class FullAldList extends Component {
                ...
                },
                {
                    ...
                    render: (text, record) => (
                        <Fragment>
                            ...
                            {/* 只有未开始、进行中的才可以失效 */}
                            <AuthBtn eventKey={['invalid_full_ald']} btnAuth={btnAuth}>
                                {
                            ...
                            {/* 只有待发布、已失效、已结束的才可以删除 */}
                            <AuthBtn eventKey={['delete_full_ald']} btnAuth={btnAuth}>
                                {
  ...
  //满N元折扣操作  type: invalid 失效  del 删除
  operate = (id, type) => {
      ...
      let dis_type = '';
      if (type == 'invalid') {
          dis_type = 'full_discount/invalid_full_ald';
          param_data.fullId = id;
      } else if (type == 'del') {
          dis_type = 'full_discount/del_full_ald';
          param_data.fullId = id;
      } else if (type == 'publish') {
          dis_type = 'full_discount/publish_full_ald';
          param_data.fullId = id;
      }
  ...
  get_list = (params) => {
      ...
      const { dispatch } = this.props;
      dispatch({
          type: 'full_discount/get_full_ald_list',
          payload: { ...params },
          callback: (res) => {
