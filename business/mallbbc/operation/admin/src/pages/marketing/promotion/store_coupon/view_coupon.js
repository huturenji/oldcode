...
export default class ViewCoupon extends Component {
  ...
  get_detail = async (id) => {
      ...
      dispatch({
          type: 'store_coupon/get_coupon_detail',
          payload: { couponId: id },
          callback: async (res) => {
              if (res.state == 200) {
  ...
  get_goods_list = (id) => {
      ...
      const { dispatch } = this.props;
      dispatch({
          type: 'store_coupon/get_coupon_goods_list',
                          ...
                          {coupon_detail.publishStartTime} ~ {coupon_detail.publishEndTime}
                                  ...
                                  <div className={`${promotion.item} ${global.flex_row_start_center}`}>
                                      ...
                                      <div className={`${promotion.right}`}>
                                          ...
                                          {coupon_detail.publishType == 6 && coupon_detail.state!=6 && coupon_detail.state!=7 && <Button type="primary" size='small' style={{marginLeft:'10px'}} onClick={()=>{this.exportWithPass()}}>导出卡密</Button>}
                                      </div>
                                  </div>
                                  <div className={`${promotion.item} ${global.flex_row_start_center}`}>
                                      <div className={`${promotion.left}`}>
                  ...
                  <div className={`${promotion.item} ${global.flex_row_start_start}`}>
                      ...
                      <div className={`${promotion.right}`}>
                          ...
                          >
                              ...
                              {/* 领取和使用规则-start */}
                              {coupon_detail.publishType != undefined && (coupon_detail.publishType ==1||coupon_detail.publishType ==5||coupon_detail.publishType ==6) &&
                  <Fragment>
                      {getSldEmptyH(10)}
                      ...
                      <div className={`${promotion.full_acm_activity} ${global.flex_column_start_start}`}>
                          <div className={`${promotion.item} ${global.flex_row_start_center}`}>
                              ...
                              {getSldEmptyH(30)}
                              {/*审核 对话框-start*/}
                              {/* <AuthBtn eventKey={["audit"]} btnAuth={btnAuth}>
                                  {
