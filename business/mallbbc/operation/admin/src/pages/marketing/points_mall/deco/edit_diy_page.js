...
import mdiy from './css/mdecorate.less';
import global from '@/global.less';
import MDiyItem from '@/components/point/MDiyItem';
import MDiyItemEdit from '@/components/point/MDiyItemEdit';
import ALibbSvg from '@/components/ALibbSvg';
import styles from '@/components/point/MDiyItem/index.less';
...
@connect(({ pc_home, project,global,point_mall }) => ({
    pc_home,
    project,
    global,
    point_mall
...
export default class Edit_diy_page extends Component {
    ...
    constructor(props) {
        ...
        this.state = {
            ...
            show_top_cat_nav: true,//是否显示顶部分类导航，默认显示
  ...
  get_diy_menu = () => {
      ...
      dispatch({
          type: 'point_mall/get_m_diy_menu',
  ...
  //获取一级积分标签，用于顶部分类导航展示数据
  get_point_label = () => {
      const { dispatch } = this.props;
      let { cat_nav } = this.state;
      dispatch({
          type: 'point_mall/get_label_tree_list',
          payload: { parentLabelId: 0 ,grade: 1 },
          callback: (res) => {
              if (res.state == 200) {
                  cat_nav = res.data;
                  cat_nav = [{ labelName: `${sldComLanguage('首页')}`, labelId: -1 }, ...cat_nav];
  ...
  get_diy_page_detial = (id) => {
      ...
      dispatch({
          type: 'point_mall/get_diy_page_detail',
          payload: { decoId: id },
          callback: (res) => {
              if (res.state == 200) {
                  //空处理
  ...
  saveMDiyData = () => {
      ...
      dispatch({
          type: 'point_mall/edit_m_diy_page',
          payload: params,
          callback: (res) => {
              if (res.state == 200) {
                  sucTip(res.msg);
                                              ...
                                              <div className={`${mdiy.center_con_border}`}>
                                                  ...
                                                  >
                                                      <img
                                                          ...
                                                          src={require('@/assets/img/decorate/tabbar/m_top_status_bar.png')}
                                                      />
                                ...
                                >
                                    ...
                                    >
                                        <img
                                            ...
                                            src={require('@/assets/img/marketing/points_mall/cat_nav_right_fixed.png')}
                                        />
