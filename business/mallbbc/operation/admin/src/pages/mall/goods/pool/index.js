...
export default class GoodsLabel extends Component {
        ...
        this.state = {
            ...
            columns: [
                ...
                },
                {
                    ...
                    render: (text, record) => (
                        <Fragment>
                            {sldtbaleOpeBtnText(`${sldComLanguage('详情')}`, () => this.edit(record.id,'view'))}
                            <span className={global.splitLine} />
                            <AuthBtn eventKey={["edit_goods_pool"]} btnAuth={btnAuth}> 
                                {sldtbaleOpeBtnText(`${sldComLanguage('编辑')}`, () => this.edit(record.id,'edit'))}
                                <span className={global.splitLine} />
                            </AuthBtn>
                            <AuthBtn eventKey={["delete_goods_pool"]} btnAuth={btnAuth}> 
                                {sldPopConfirmDiy('leftBottom', `${sldComLanguage('删除后不可恢复，是否确定删除')}`, () => this.delete(record.id), `${sldComLanguage('确定')}`, `${sldComLanguage('取消')}`,
	...
	edit = (productPoolId,type) => {
	    router.push(`/manage_product/goods_pool_add?productPoolId=${productPoolId}&type=${type}`);
	};
	...
	toAdd = ()=>{
	    router.push('/manage_product/goods_pool_add?type=add');
	}
	...
	render() {
	    ...
	    return (
	        <div className={global.common_page} style={{ flex: 1 }}>
	            ...
	            <AuthBtn eventKey={["view_goods_pool"]} btnAuth={btnAuth} showPage>
	            ...
	            <div className={global.operate_bg}>
	                    <AuthBtn eventKey={["add_goods_pool"]} btnAuth={btnAuth}> 
	                {
	                    showAdd? sldIconBtn(() => this.toAdd(), `${sldComLanguage('新增商品池')}`, 7, 7)
	                        :
	                        sldIconBtnNo(() => null, `${sldComLanguage('新增商品池')}`, 7, 0, 14, 14, 3, 'xinzeng', '#FA6F1E', `${sldComLanguage('最多新增10个商品池')}`)
	                }
	                    </AuthBtn>
	            </div>
