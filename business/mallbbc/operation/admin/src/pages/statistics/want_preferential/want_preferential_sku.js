...
export default class WantPreferentialSku extends Component {
    state = {
        search_height: 0,
        search_data: [
            {
            ...
            },
            {
                ...
                name: 'storeId',
                placeholder: `${sldComLanguage('请选择店铺名称')}`
            }
        ],// 筛选条件数组
        data: {},// 要优惠统计列表数据
        columns: [
    ...
    get_store_list = (params) => {
        ...
        const search_data = deepCopy(this.state.search_data);
        ...
        dispatch({
            ...
            callback: (res) => {
                ...
                if (res.state == 200) {
                    ...
                    storeData.unshift({ storeId: null, storeName: '全部', supplierTypes: [] })
                    let storeArr = []
        ...
        return (
            <div className={global.common_page}>
                <div className={global.flex_com_space_between} style={{ marginBottom: 10,justifyContent:'right' }}>
                    <AuthBtn eventKey={['export_want_pref_by_sku']} btnAuth={btnAuth}>
                ...
                <Spin spinning={loading}>
                    <StandardTable
                        totalHeight={document.body.clientHeight - 180 - search_height - 50}
                        columns={columns}
                        data={data}
