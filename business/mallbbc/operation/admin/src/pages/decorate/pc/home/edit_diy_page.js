...
//设置顶部logo
// eslint-disable-next-line no-unused-vars
let main_site_logo = require('@/assets/img/decorate/pc/pc_edit_diy_page_logo.png');
...
export default class Edit_diy_page extends Component {
  ...
  sldHandleConfirm = (val = []) => {
      ...
      //轮播图处理
          ...
          for (let index1 = 0; index1 < val.length; index1++) {
              //楼层模板处理
              let tmp_tpl = { key: val[index1].dataId, html: val[index1].html, id: data.length + 1 };
              if (this.id != '') {
                                              ...
                                              >
                                                  <div style={{ position: 'relative' }}>
                                                      ...
                                                      <div className={diy.sld_mask}>
                                                          ...
                                                          >{sldSvgIcon('#FA6F1E', 16, 16, 'jia')}</a>
                                                          <span
                                                              onClick={() => this.del_instance(item.id)}
                                                          >{sldComLanguage('删除')}</span>{/*删除*/}
                                                      </div>
                                                  </div>
                                              </div>
                                          )}
