...
import order from './css/channel.less';
...
export default class Order_lists extends Component {
  ...
  get_list = (params) => {
      ...
      dispatch({
          type: 'systemset_channel/operation_list',
          payload: { ...params },
          callback: (res) => {
  ...
  toAdd(item){
      let urlObj = {
          pathname:'/sysset_channel/list_to_add'
      }  
      let query = {};
      if(!!item){
  ...
  render() {
      const { search_data, data, loading, params,search_height,stateMap,viewAuth } = this.state;
      return (
          <div
          ...
          >
              <div className={global.flex_com_space_between} style={{ marginBottom: 10 }}>
                  {sldLlineRtextAddGoods('#FA6F1E', `${('运营管理 / 渠道管理')}`)}
                  <AuthBtn eventKey={["add_channel"]} btnAuth={btnAuth}>
                      ...
                      <Spin spinning={loading}>
                          ...
                          <div className={order.order_list}>
                              <ul className={`${order.header} ${order.nopadding}`}>
                                  ...
                                  <li className={`${order.width_30} ${order.center}`}>启用/停用时间</li>
                                  ...
                                  >
                                      {data.channelInfos != undefined && data.channelInfos.length > 0 && data.channelInfos.map((item, index) => <div className={order.item} key={index}>
                                          <div className={`${order.order_goods_part} ${global.flex_row_start_center}`}>
                                              <div className={`${order.order_state} ${order.width_30} ${order.center}`}>{item.channelId}</div>
                                              <div className={`${order.order_state} ${order.width_30} ${order.center}`}>{item.channelName}</div>
                                              <div className={`${order.order_state} ${order.width_30} ${order.center}`}>{stateMap[item.channelState]}</div>
                                              <div className={`${order.order_state} ${order.width_30} ${order.center}`}>{sthis.dateTimeFormat(item.activationTime*1000)}</div>
