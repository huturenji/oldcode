...
import {
    Empty,
    Form, Modal,Upload,Button,message
} from 'antd';
...
export default class SldSelMoreLeftRightGoods extends Component {
      ...
      if( data.list && Array.isArray(data.list) ){
          data.list.forEach((item)=>{
              if (selectedRowKeys.indexOf(item.productId) == -1) {
                  selectedRowKeys.push(item.productId);
                  selectedRows.push(item);
              }
          })
          this.setState({
          ...
          if(!!jsonData.length && jsonData.length > 0){
              ...
              for(let index = 0; index < jsonData.length; index++) {
                  ...
                  if(Array.isArray(ele)&&ele.length>0){
                      ...
                      }else if( ele[0] && (!isNaN(parseInt(ele[0]))) ){
              ...
              //   message.error(`最多支持导入100条数据`);
              //   return false
              // }
              if(skuList.length>0){
                  this.get_list({ pageSize:10000, current: 1,productIds:skuList.join(',') },2);
              }
  ...
  render() {
      ...
      const { selectedRows, search_data, data, selectedRowKeys,uploading } = this.state;
      return (
          <Modal
              destroyOnClose
                  ...
                  >
                      ...
                      <div className={`${styles.item_right} ${global.flex_column_start_start}`}>
                        ...
                        <div className={`${styles.sele_svideo_flag}`}>
                            <ALibbSvg fill="#FC701E" width={19} height={19} type="yixuan" onClick={() => this.handleLeftAll()} />
                        </div>
                          }
                      </div>
                  </a>)
