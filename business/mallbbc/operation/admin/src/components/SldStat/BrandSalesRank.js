...
export default class BrandSalesRank extends Component {
    constructor(props) {
        super(props);
        this.state = {
            ...
            categoryColumns: [
                {
                    'title': '序号', 'dataIndex': 'brandId', 'align': 'center', 'width': 35,
            ...
            categoryData: { list: [], pagination: {} },//品牌销售排行-TOP10表格数据
            categoryParams: {
      ...
      return (
          <div style={{ marginTop: '10px' }} className={`${stat.common_table_item}`}>
              <div className={`${stat.label_panel} ${global.flex_row_between_center}`}>
                  {sldLlineRtextAddGoodsAddMargin('#FA6F1E', `${sldComLanguage('品牌销售排行 - TOP10')}`, 10, 0, 0)}
                  <SldStatDate
