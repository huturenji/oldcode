...
export default class Search extends Component {
	...
	componentDidMount() {
	    // 存在默认值是，给form设置上默认值
	    const { form, search_data } = this.props
	    search_data.forEach(item => {
	        if(item.defaultValue) {
	            form.setFieldsValue({ [item.name]: item.defaultValue })
	        }
	    })
	...
	selectChange = (e, val)=>{
	    const {selectChange,setItemInitValue} = val
	    if (selectChange) {
	        selectChange(e);
	    }
	    if(setItemInitValue){
	        setItemInitValue && setItemInitValue.forEach(v => {
	    ...
	    if (val.type == 'input') {
	        return (
	            <FormItem
	                ...
	                label={val.label}
	            >
	                {getFieldDecorator(val.name)(<Input maxLength={250} style={{ width: val.widthMode == 'fat' ? 180 :150 }} placeholder={val.placeholder} />)}
	            </FormItem>
	        ...
	        return (
	            <FormItem
	                ...
	                label={val.label}
	            >
	                {getFieldDecorator(val.name)(<InputNumber style={{ width: 150 }} placeholder={val.placeholder} />)}
	            </FormItem>
	        );
	            ...
	            >
	                ...
	                rules: val.rules
	                    ...
	                    <Select
	                        ...
	                        showSearch
	                        onSelect={(e) => this.selectChange(e, val)}
	                    >
	            ...
	            >
	                {getFieldDecorator(val.name, { initialValue: val.initialValue, rules: val.rules })(
	                    <div onClick={() => val.callback(val.operate_obj)}>
	                        <Input style={{ width: 150 }} disabled addonAfter={sldInputAfterAddons()} placeholder={val.placeholder} />
	                    </div>
	                )}
	            </FormItem>
	        );
	            ...
	            <FormItem label={val.label}>
	                {val.initialValue && getFieldDecorator(val.name, {
	                    ...
	                    <TreeSelect
	                        ...
	                        getPopupContainer={triggerNode => triggerNode.parentNode}
	                    >
	                        {val.data.map((valitem) => <TreeNode value={valitem.sellerName} title={valitem.sellerName} key={valitem.id} />)}
	                    </TreeSelect>,
	                )}
	...
	//搜索条件 - showLessCount 默认展示数量 默认为 4
	renderSearchFiist = (search_data) => search_data.map((item, index) => index < (this.props.showLessCount ? this.props.showLessCount : 4) ? this.commonCon(item, index) : null);

	//搜索条件-默认前两条之后数据
	renderSearchSecond = (search_data) => search_data.map((item, index) => index > (this.props.showLessCount ? this.props.showLessCount - 1 : 3) ? this.commonCon(item, index) : null);
	    ...
	    return (
	        search_data.length > 0 && <Form onSubmit={this.handleSearch} ref="sld_common_search_part" layout="inline">
	            ...
	            <span style={{ position: 'absolute', right: search_data.length > 4?20:0, top: top != undefined ? top : 0 }}>
	                ...
	                {search_data.length > 4 && <a style={{ marginLeft: 3, fontSize: 12,color:'#FF6A12' }} onClick={this.toggleForm}>
