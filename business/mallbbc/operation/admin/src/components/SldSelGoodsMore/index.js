	    ...
	    this.state = {
	        ...
	        sel_columns: [
	            ...
	            },
	            {
	                ...
	                width: 60,
	                render: (text, record) => 
						 sldPopConfirm('leftBottom', `${sldComLanguage('删除后不可恢复，是否确定删除')}`, () => this.delSelGoods(record.id), `${sldComLanguage('确定')}`, `${sldComLanguage('取消')}`, sldtbaleOpeBtn(sldComLanguage('删除'), 'shanchu2', null), 0, 0, '#1890ff')//删除 删除后不可恢复，是否确定删除？
	        ...
	        columns: [
	            ...
	            {
	                title: `${sldComLanguage('价格(元)')}`,//价格
	                dataIndex: 'goodsPrice',
	                align: 'center',
	                width: 100
	            },
	            {
	                title: `${sldComLanguage('商品状态')}`,//商品状态
