...
export default class SelectMemberPop extends Component {
  ...
  //获取数据列表 有options参数 代表是导入的  导入用下面的，这个不用
  get_member_list = (params, options={type: 1, step: 0}) => new Promise((resolve)=>{
      ...
      if(channelProps.channelId){
          new_params.channelId = channelProps.channelId
      }
      dispatch({
                  ...
                  if(Array.isArray(ele)&&ele.length>0){
                      if(index==0){
                          if(ele[0]=='*会员ID'){
                              ...
                              message.error('第一列应为*会员ID,请按格式重新上传!');
                              return false
                          }
                          if(ele[4]=='*公司ID'){
                              // todo
                          }else{
                              message.error('第五列应为*公司ID,请按格式重新上传!');
                              return false
                          }
                      }else{
                          let params = {}
                          ...
                          if( ele[0] ){
                              ...
                              message.error(`解析失败,请检查第${index+1}行数据`,5);
                              return false;
                          }
                          // 公司ID
                          if( ele[4] ){
  ...
  render() {
      const { modalVisible, width, title, height,channelProps } = this.props;
      ...
      return (
          ...
          >
              <div className={`${styles.component_sele_more} ${global.flex_column_start_start}`}>
                  <div>
                      {channelProps?.channelName?`指定渠道:${channelProps.channelName}`:''}
                  </div>
