...
import { sldTsvg, getSldEmptyH, formItemLayoutModal ,sldComLanguage,getStorage} from '@/utils/utils';
...
export default class SldExpressCom extends Component {
	com_area_data = getStorage('common_area_list') != undefined && getStorage('common_area_list') ? JSON.parse(getStorage('common_area_list')) : [];//通用的地址信息
	...
	constructor(props) {
	    ...
	    this.state = {
	        ...
	        base_info: [{
	            ...
	            rules: [{
	                required: true,
	                message: `${sldComLanguage('首重/件(kg/件)')}`
	            }]
	        }, {
	            type: 'inputnum',
	            label: `${sldComLanguage('首费(元)')}`,
	            name: `trans_com_weight${ type}`,
	            placeholder: `${sldComLanguage('默认运费-首费(元)')}`,
	            initialValue: '',
	            rules: [{
	                required: true,
	                message: '默认运费-首费(元)'`${sldComLanguage('默认运费-首费(元)')}`
	            }]
	        }, {
	            type: 'inputnum',
	            ...
	            rules: [{
	                required: true,
	                message: `${sldComLanguage('续重/件(kg/件)')}`
	            }]
	        }, {
	            type: 'inputnum',
	            label: `${sldComLanguage('续费(元)')}`,
	            name: `trans_com_add_fee${ type}`,
	            placeholder: `${sldComLanguage('默认运费-续费(元)')}`,
	            initialValue: '',
	            rules: [{
	                required: true,
	                message: `${sldComLanguage('默认运费-续费(元)')}`
	            }]
	        }],
	        column: [
	            {
	            ...
	            },
	            {
	                ...
	                render: (text, record) => (
	                    ...
	                    >
	                        ...
	                        >{sldTsvg('shanchu', '#1890FF', 20, 20)}</a>
	                    </Popconfirm>
	                )
	            },
	            {
	...
	get_com_area = () => {
	    ...
	    dispatch({
	        type: 'project/get_common_area',
	        callback: (res) => {
	            if (res.state == 200) {
	                this.com_area_data = res.data;
	                this.setState({area_data:res.data})
	            }
	        }
	    });
	};
