...
export default function SnCouponPicker(props) {
    ...
    const addCoupon = (couponShowType) => {
        ...
        if (couponShowType === 'redPacket') {
            // 添加红包
            const selectedRowsArr = value.info.filter(item => item.coupon_show_type === 404).map(item => item.info)
            const couponIdArr = selectedRowsArr.map(item => item.couponId)
            setselCouponType(404)
            // 修改父组件linkType的值，控制专题/商品选择弹窗弹出,并且绑定优惠券弹窗中的勾选
            setLinkType('decoRedPacket')
            setSelectedRowKeys(couponIdArr)
            setSelectedRows(selectedRowsArr)
        }
    ...
    const seleSku = (val) => {
        ...
        if (val.length > 0) {
            ...
            tar_data.couponIdArr = [];
            tar_data.info = tar_data.info.filter(item => item.coupon_show_type !== selCouponType)
        }
    ...
    const delCoupon = (coupon_index, delType) => {
        ...
        if (delType === 'coupon') {
            ...
            if (couponShowType === 'vopCoupon'||couponShowType === 'freightCoupon'||couponShowType === 'consumerCoupon'||couponShowType === 'decoRedPacket') {
                // 从优惠券编号数组集合中删除指定编号
