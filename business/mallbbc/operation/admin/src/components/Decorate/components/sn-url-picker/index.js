...
export default class SnUrlPicker extends Component {
    ...
    handleTimeRules = (e, d)=>{
        if(!!d && !!d[0] && d.length > 1){// 说明此时有选中的时间规则
            // rules={interval:[123,456]}
            let startTimeStamp = new Date(d[0]).getTime()
            let endTimeStamp = new Date(d[1]).getTime()
            let rules = {interval:[startTimeStamp, endTimeStamp]}
            let rulesQuery =encodeURIComponent(JSON.stringify(rules));
            this.onChange(rulesQuery, 'rules')
        } else {
            this.onChange(null, 'rules')
        }
    }

    /**
     * 设置url跳转规则参数
     * 目前只有一个场景，即时间段限制，只有在当前时间段，才能跳转地址
     */
    setRulesElement = (rules, rulesTips)=>{
        let defaultValue = []
        let rulesValue = rulesTips ? rulesTips : ''
        if(!!rules){
            let values = JSON.parse(decodeURIComponent(rules)).interval
            if(!!values && !!values[0] && values.length > 1){
                defaultValue = [moment(moment(values[0]).format(dateFormat), dateFormat), moment(moment(values[1]).format(dateFormat), dateFormat)]
            }
        }
        let domElement = 
        <div>
            <div style={{width:"100%"}}>
                <div style={{fontWeight: "bold"}}>跳转规则</div>
                <div className={styles.url_rules}>
                    <span>时间间隔：</span>
                    <RangePicker
                        style={{width:298}}
                        format={dateFormat}
                        ...
                        onChange={(e, d) => this.handleTimeRules(e, d)}
                    />
                </div>
                <div className={styles.url_rules}>
                    <span>跳转提示：</span>
                    <TextArea
                        style={{ width: 298,height:100 }}
                        value={rulesValue}
                        placeholder="请输入当跳转规则不符合时的提示"
                        onChange={(e) => this.onChange(e.target.value, 'rulesTips')}
                    />
                </div>
            </div>
        </div>;
    ...
    showCompent = (type, data)=>{
        ...
        if (type == '') {
            ...
            <div>
                ...
                {this.setRulesElement(data.rules, data.rulesTips)}
            </div>;
        } 
            ...
            domElement =
            <Select
                value={(data.storeId?data.storeId:'')}
                style={{ width: 232 }}
                placeholder="请选择数据来源"
    ...
    get_store_list = () => {
        ...
        dispatch({
            type: 'mdecorate/get_own_store_list',
            payload: {},
            callback: (res) => {
                if (res.state == 200) {
