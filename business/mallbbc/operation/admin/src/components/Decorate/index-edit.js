...
import activityCombination from './activity-combination/activity-combination-edit.js';
...
const comLists = {
    ...
    'tab': tab,
    'claim-all':claimAll,
    'activity-combination': activityCombination
}
...
export default class MDiyItem extends Component {
    ...
    componentDidMount() {
        ...
        this.custEvents.addListener('goods', params => {
            let { select_data, handleCurSelData } = this.props;
            if (params.type === 'upload') {
                select_data.data = [{ url: '', ids: [], info: [] }]
            } else if(params.type === 'goodsPool'){
                select_data.data = [{ goodsPool:{},goodsGroup:[] }]
            }
            handleCurSelData(select_data)
        })
    ...
    get_store_list = (params) => {
        ...
        dispatch({
            type: 'decocoms/get_own_store_list',
            payload: params,
            callback: (res) => {
                if (res.state == 200) {
    ...
    renderItem = (data, list) => {
        let con = list.map(item => {
            ...
            if (item.name === 'sn-text') {
                ...
                conItem = <SnTitle label={item.label} key={item.guid} style={item.style} />
