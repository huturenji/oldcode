module.exports = {
  ...
  rules: {
    ...
    "react/jsx-indent": 0,
    "no-unused-expressions": [2, { 'allowShortCircuit': true, 'allowTernary': true }],   //允许三目，允许短路
    //禁用的规则————————————————————————————————————————————————————————————————————————————————————
    "import/extensions":[0, { extensions: ['.js'] }],   //文件后缀名
    "no-lonely-if":0,
    "import/no-extraneous-dependencies": ["error", {"devDependencies": true, "optionalDependencies": false, "peerDependencies": false}], //dev依赖
    "react/jsx-closing-tag-location":0, //jsx 关闭标签闭合
    "react/destructuring-assignment":0,//  数组的解构赋值 
    "global-require":0,  //图片引入
    "jsx-a11y/alt-text":0, // 图片设置alt
    "react/no-unused-state":0,  // 禁止使用 未使用的state
    "prefer-destructuring":0,   //数组解构
    "jsx-a11y/label-has-for":0, //input label-for
    "jsx-a11y/label-has-associated-control":0,  //label标记具有文本标签和关联控件
    "class-methods-use-this": 0, //如果一个类方法不使用 this，那么要安全的做为静态函数出现。
    "react/no-string-refs":0,   //废弃的属性
    "no-restricted-syntax":0,  //不严格的语法
    "no-await-in-loop":0,  //循环中使用await
    "no-mixed-spaces-and-tabs":0, // tab和空格混合
    "react/no-array-index-key":0,  //index为key
    "no-alert": 0,//禁止使用alert confirm prompt
    "no-array-constructor": 0,//禁止使用数组构造器
    "no-async-promise-executor":0,//promise方法内部禁止使用async
    ...
    'prefer-promise-reject-errors':0,//禁止reject抛出str错误信息
    ...
    "yoda": [0, "never"]//禁止尤达条件if('red'==color)
  },
  settings: {
