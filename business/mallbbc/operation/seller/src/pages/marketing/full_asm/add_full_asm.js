...
get_goods_list_all =async (skuList)=>{
  ...
  handleSaveAllData = () => {
      const { dispatch } = this.props;
      const { query, ladder_promotion,cacheVoucher,selectedRows } = this.state;
      let param = {};
...
refuseEvent = (auditReason)=>{
    // 审核拒绝或通过,0-拒绝,1-通过
    this.fullAsmAudit({state:0,auditReason})
}

successEvent = ()=>{
    this.fullAsmAudit({state:1})
}
...
render() {
    ...
    return (
        ...
        >
            <AuthBtn btnAuth={btnAuth} eventKey={["full_asm_view"]} showPage>
                    ...
                    <Form layout="inline">
                        ...
                        <div className={`${global.flex_row_start_center} ${promotion.add_new}`}>
                            ...
                            >
                                ...
                                {
                                    tar == 'audit' && hasAuth('full_asm_audit') &&
                                    <ReviewButton 
                                        refuseText='审核拒绝'
                                        refuseTitle='确定拒绝该条阶梯满减活动么?'
                                        refuseEvent={this.refuseEvent}
                                        successText='审核通过'
                                        successTitle='确定通过该条阶梯满减活动么?'
