...
export default class ViewCoupon extends Component {
  ...
  exportWithPass = () => {
      ...
      this.setState({loading:true})
      dispatch({
          type: 'promotion/exportWithPass',
          payload: {couponId:coupon_detail.couponId,fileName:'店铺优惠券卡密'},
          callback: (res) => {
  ...
  render() {
      const {
          loading, curCouponType, useTimeType, useType, columns_spu, columns_member, selectedRows, query,selectedMemberRows, coupon_detail,pagination,linkInfo
      } = this.state;
      return (
          <div className={`${global.common_page} ${global.com_flex_column}`} style={{ position: 'relative' }}>
              <AuthBtn btnAuth={btnAuth} eventKey={["coupon_list_view"]} showPage>
                              ...
                              {coupon_detail.publishStartTime} ~ {coupon_detail.publishEndTime}
                                      ...
                                      <div className={`${promotion.item} ${global.flex_row_start_center}`}>
                                          ...
                                          <div className={`${promotion.right}`}>
                                              ...
                                              {coupon_detail.publishType == 6 && coupon_detail.state !=6 && coupon_detail.state !=7 && hasAuth("coupon_list_export") && <Button type="primary" size='small' style={{marginLeft:'10px'}} onClick={()=>{this.exportWithPass()}}>导出卡密</Button>}
                                          </div>
                                      </div>
                      ...
                      <div className={`${promotion.item} ${global.flex_row_start_start}`}>
                          ...
                          <div className={`${promotion.right}`}>
                              ...
                              >
                                  ...
                                  {/* 领取和使用规则-end */}
                                  {getSldEmptyH(30)}
                                  <ReviewLog type="promotion/listRecords" params={{couponId:coupon_detail.couponId}} />
                                  {getSldEmptyH(30)}
                                  {/*审核 对话框-start*/}
                                  {
                                      query.type == 'system' && coupon_detail.state == 6 && hasAuth("coupon_list_audit") &&
