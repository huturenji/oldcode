                    ...
                    render: (text, record) => (
                        <Fragment>
                            ...
                            <AuthBtn eventKey={['seckill_attend_edit']} btnAuth={btnAuth}>
                                {
                                    record.state!=3 &&  
                                sldPopConfirmDiy('leftBottom', `${sldComLanguage('退出活动会退出所有场次的商品，是否确定退出')}`, () => this.quitActivity(record), `${sldComLanguage('确定')}`, `${sldComLanguage('取消')}`,
                                    sldtbaleOpeBtnText(`${sldComLanguage('退出活动')}`, () => null))
                                }
                            </AuthBtn>
  ...
  render() {
      ...
      return (
          <div className={global.common_page} style={{ flex: 1, paddingTop: 0 }}>
              <AuthBtn btnAuth={btnAuth} eventKey={["seckill_attend_view"]} showPage>
