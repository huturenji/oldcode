...
import VerifyedList from './components/review_list';
...
export default class GoodsList extends Component {
    ...
    render() {
        ...
        return (
            <AuthBtn eventKey={['together_buy_unattend_view','together_buy_attend_view','together_buy_audit_view']} btnAuth={btnAuth} showPage>
                <div className={global.common_page} style={{ flex: 1 }}>
                    ...
                    <div className={`${_styles['attention']}`}>
                        ...
                        <div className={`${_styles['attention_item']}`}>4、用户购买一起买商品的待付款订单，订单生成后30分钟内未完成支付的由系统自动取消该订单；</div>
                    </div>
                    {
                        enableFlag == 1&&
                    <Tabs type="card" activeKey={tabActiveKey} animated={false} onTabClick={this.onHandleTabClick}>
                        <TabPane tab={`${sldComLanguage('一起买活动')}`} key="1">
                            <ALLList />
                        </TabPane>
                        <TabPane tab={`${sldComLanguage('已参加活动')}`} key="2">
                            <JoinedList />
                        </TabPane>
                    ...
                    <Fragment>
                        ...
                        <Empty
                            ...
                            description={
                                <span>{sldComLanguage('一起买活动模块暂未开启')}</span>
                            }
                        />
                    </Fragment>
                    }
