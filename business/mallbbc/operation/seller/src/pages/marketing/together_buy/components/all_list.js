...
export default class AllList extends Component {
        ...
        this.state = {
            ...
            columns: [
                ...
                },
                {
                    ...
                    align: 'center',
                    width: 100,
                    render: (_, record)=> <span>{this.getDate(record.startTime)}～{this.getDate(record.endTime)}</span>
                ...
                },
                {
                    ...
                    render: (text, record) => (
                        <Fragment>
                            {hasAuth('together_buy_unattend_add')?
                                <Link to={{
                                    pathname: '/marketing/together_buy_add',
                                    query: {
  ...
  search = (data) => {
      ...
      if (values.activity_date) {
          values.startTime = values.activity_date[0] ? `${values.activity_date[0].format(dateFormat) } 00:00:00` : '';
          values.endTime = values.activity_date[1] ? `${values.activity_date[1].format(dateFormat) } 23:59:59` : '';
  ...
  render() {
      ...
      return (
          <div className={global.common_page} style={{ flex: 1, paddingTop: 0 }}>
              <AuthBtn eventKey={['together_buy_unattend_view']} btnAuth={btnAuth} showPage>
