...
export default class FullAldList extends Component {
                ...
                },
                {
                    ...
                    render: (text, record) => (
                        <Fragment>
                            ...
                            {
                                record.state == 7 && hasAuth('full_ald_audit') && 
                                <Link to={{
                                    pathname: '/marketing/full_ald_to_add',
                                    query: {
                                        id: record.fullId,
                            ...
                            {/* 只有审核拒绝的才可以编辑 */}
                            {
                                record.state == 8 && hasAuth('full_ald_edit') &&
                                <Fragment>
                                    <span className={global.splitLine} />
                                    <Link to={{
                                        pathname: '/marketing/full_ald_to_add',
                            ...
                            {/* 只有待发布的才可以发布 */}
                            {
                                record.state == 1&& hasAuth('full_ald_edit') &&
                                <Fragment>
                                    <span className={global.splitLine} />
                            ...
                            {/* 只有未开始、进行中的才可以失效 */}
                            {
                                (record.state == 9||record.state == 3)&&hasAuth('full_ald_edit') &&
                                <Fragment>
                                    <span className={global.splitLine} />
                            ...
                            {/* 只有待发布、已失效、已结束的才可以删除 */}
                            {
                                ([1,4,5,7,8].includes(record.state))&&hasAuth('full_ald_edit') &&
                                <Fragment>
                                    <span className={global.splitLine} />
  ...
  render() {
      ...
      return (
          <AuthBtn eventKey={['full_ald_view']} btnAuth={btnAuth} showPage>
              <div className={global.common_page} style={{ flex: 1, paddingTop: 0 }}>
                  {getSldEmptyH(10)}
                  {sldLlineRtextAddMargin('#FA6F1E', `${sldComLanguage('满N元折扣活动')}`, 0, 0, 10)}
                  ...
                  {/*公共功能条-start*/}
                  {hasAuth('full_ald_add')&&<div className={global.operate_bg}>
                      <Link to={{
                          pathname: '/marketing/full_ald_to_add'
                      }}
                      >
