...
refuseEvent = (auditReason)=>{
    // 审核拒绝或通过,0-拒绝,1-通过
    this.fullAldAudit({state:0,auditReason})
}

successEvent = ()=>{
    this.fullAldAudit({state:1})
}
...
render() {
    ...
    return (
        ...
        >
            <AuthBtn btnAuth={btnAuth} eventKey={["full_ald_view"]} showPage>
                    ...
                    <Form layout="inline">
                        ...
                        <div className={`${global.flex_row_start_center} ${promotion.add_new}`}>
                            ...
                            >
                                ...
                                {
                                    tar == 'audit' && hasAuth("full_ald_audit")&&
                                    <ReviewButton 
                                        refuseText='审核拒绝'
                                        refuseTitle='确定拒绝该条满N元折扣活动么?'
                                        refuseEvent={this.refuseEvent}
                                        successText='审核通过'
                                        successTitle='确定通过该条满N元折扣活动么?'
