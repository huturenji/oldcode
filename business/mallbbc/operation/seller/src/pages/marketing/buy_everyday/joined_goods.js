        ...
        this.state = {
            ...
            columns: [
                ...
                },                   
                {
                    ...
                    width: 100,
                    render: (text, record) => {
                        const { detail: { state, bisType } } = this.state
                        return (
                            <Fragment>
                                {
                                    ( (bisType!='audit') && (state == 1 || state == 2)) && hasAuth("buy_everyday_attend_edit")?
        ...
        return (
            <div className={global.common_page}>
                <div className={global.flex_com_space_between}>
                    {sldLlineRtextAddMargin('#FA6F1E', detail.bisType == 'audit' ? `${sldComLanguage('活动商品审核')}` : `${sldComLanguage('天天专场 / 专场商品列表')}`, 0, 0, 10)}
                    {sldIconBtnBg(() => this.goBackList(), 'fanhui', `${sldComLanguage('返回上级页面')}`, '#fff', 7, 0, 15, 15, 5)}
                </div>
                <div className={`${_styles['content_des']}`}>
                    <div className={`${_styles['des_item']} ${_styles['des_item_name']}`}><span className={`${_styles['des_item_title']}`}>专场名称：</span> <span className={`${_styles['des_item_content']}`}>{detail.promotionName}</span></div>
                    <div className={`${_styles['des_item']} ${_styles['des_item_date']}`}><span className={`${_styles['des_item_title']}`}>专场时间：</span> <span className={`${_styles['des_item_content']}`}>{detail.promotionTime}</span></div>
                    <div className={`${_styles['des_item']} ${_styles['des_item_state']}`}><span className={`${_styles['des_item_title']}`}>专场状态：</span> <span className={`${_styles['des_item_content']}`}>{stateTxtValue[detail.state]}</span></div>
