                ...
                },
                {
                    ...
                    render: (text, record) => (
                        <Fragment>
                            ...
                            >
                                {hasAuth('presale_full_view') && sldtbaleOpeBtnText(`${sldComLanguage('查看商品')}`, () => null)}
                            </Link>
                            <Link to={{
                            ...
                            >
                                {hasAuth('presale_full_view') && sldtbaleOpeBtnText(`${sldComLanguage('查看详情')}`, () => null)}
                            </Link>
                            ...
                            >
                                {hasAuth('presale_full_edit') && sldtbaleOpeBtnText(`${sldComLanguage('复制')}`, () => null)}
                            </Link>
                            {/* 只有待发布的才可以编辑 */}
                            {record.state == 1 &&
                <Fragment>
                    ...
                    >
                        {hasAuth('presale_full_edit') && sldtbaleOpeBtnText(`${sldComLanguage('编辑')}`, () => null)}
                    </Link>
                </Fragment>
                            }
                            {/* 只有待发布的才可以发布 */}
                            {record.state == 1 && hasAuth('presale_full_edit') && 
                <Fragment>
                    {sldPopConfirmDiy('leftBottom', `${sldComLanguage('发布后不可撤销，是否确定发布？')}`, () => this.operate(record.presellId,'publish'), `${sldComLanguage('确定')}`, `${sldComLanguage('取消')}`,
                        sldtbaleOpeBtnText(`${sldComLanguage('发布')}`, () => null))}
                            ...
                            {/* 只有未开始、进行中的才可以失效 */}
                            {(record.state == 2 || record.state == 3) && hasAuth('presale_full_edit') && 
                <Fragment>
                    {sldPopConfirmDiy('leftBottom', `${sldComLanguage('确定失效该活动吗？')}`, () => this.operate(record.presellId,'invalid'), `${sldComLanguage('确定')}`, `${sldComLanguage('取消')}`,
                        sldtbaleOpeBtnText(`${sldComLanguage('失效')}`, () => null))}
                            ...
                            {/* 只有待发布、已失效、已结束的才可以删除 */}
                            {(record.state == 1 || record.state == 4 || record.state == 5) && hasAuth('presale_full_edit') && 
                <Fragment>
  ...
  render() {
      ...
      return (
          <div className={global.common_page} style={{ flex: 1, paddingTop: 0 }}>
              <AuthBtn eventKey={['presale_full_view']} btnAuth={btnAuth} showPage>
                  ...
                  <div className={global.operate_bg}>
                      ...
                      >
                          {hasAuth("presale_full_add")&&sldIconBtn(() => null, `${sldComLanguage('新建全款预售')}`, 7, 0, 12, 12, 3, 'fabu1', '#08A9B7')}
                      </Link>
                  </div>
