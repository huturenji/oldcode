...
componentDidMount() {
  ...
  render() {
      const {
          ...
          modalVisible, operateData, deliverModal, expressList
      } = this.state;
          ...
          >
              ...
              >
...

                  {
                      order_detail.orderState == 0 && <div className={`${order.state_part} ${global.flex_column_start_center}`}>
                          ...
                          <span className={order.tip}>{sldComLanguage('取消原因:')}{order_detail.refuseReason+(order_detail.refuseRemark?(`,${order_detail.refuseRemark}`):'')}</span>
                      </div>
                  }
            ...
            <div className={`${order.state_part} ${global.flex_column_start_center}`}>
                ...
                <div className={order.btnsty}>
                    <div onClick={() => this.agreeReturn('deliver')} className={order.deliver_btn} style={{background:'rgba(255, 113, 30, .9)'}}>{sldComLanguage('发货')}</div>
                </div>
            </div>
                  }
              ...
              >
                  <Form layout="horizontal">
                      ...
                      <FormItem
                          ...
                          {...formItemLayoutModal}
                      >
                          {getFieldDecorator('deliverType', { initialValue: 0,rules: [{ required: true, message: `${sldComLanguage('请选择发货方式')}` }] })(
