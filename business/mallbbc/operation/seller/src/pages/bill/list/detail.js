...
import bill from './css/bill.less';
      ...
      dispatch({
          ...
          callback: async (res) => {
              if (res.state == 200) {
                  ...
                  //结算账户信息
                  for(let i=0;i<bill_base_data.length;i++){
                      if (bill_base_data[i].name == 'startTime') {
                          bill_base_data[i].text = `${bill_detail_data.startTime } ~ ${ bill_detail_data.endTime}`;
                      } else {
                          bill_base_data[i].text = bill_detail_data[bill_base_data[i].name];
                      }
                  }
                  ...
                  for(let i=0;i<bill_progress_data.length;i++){
                      if (i < bill_detail_data.logList.length) {
                          bill_progress_data[i].time = bill_detail_data.logList[i].createTime;
                      }
                      if (bill_detail_data.state < bill_progress_data[i].state_code) {
                          bill_progress_data[i].cur_state = 'no';
                      } else if (bill_detail_data.state == bill_progress_data[i].state_code) {
                          bill_progress_data[i].cur_state = 'cur';
                      } else {
                          bill_progress_data[i].cur_state = 'pass';
                      }
                  }
  ...
  render() {
      ...
      return (
          ...
          <div className={`${bill.state_part} ${global.flex_column_start_center}`}>
              ...
              <AuthBtn btnAuth={btnAuth} eventKey={["lists_edit"]}>
