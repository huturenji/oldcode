                          ...
                          {data.data.map((item, index) => <div key={index} className={`${styles.item} ${styles.left}`}>
                                  ...
                                  <div className={`${styles.r_title}`}>
                                      <ul>
                                          ...
                                          >
                                              <div
                                                  className={styles.sld_mask}
                                                  onClick={() => this.editTpl('r_title', index, `${sldComLanguage('标题tab设置')}`, [`${sldComLanguage('为达到页面效果，建议上传4张 172*106大小的图片')}`, `${sldComLanguage('标题名称不能为空，最多输入4个字')}`], key)}
                                              >
                                                  <span>{sldComLanguage('编辑')}</span>
                                              </div>
                            ...
                            vals.data.map((val, key) => <div key={key} className={`${styles.item}`}>
                                <div className={`${styles.title_wrap}`}>
                                    <Tooltip placement="topLeft" title={val.main_title ? val.main_title : `${sldComLanguage('图片标题')}`}>
                                        <a
                                    ...
                                    <Tooltip placement="topLeft" title={val.sub_title ? val.sub_title : `${sldComLanguage('图片子标题')}`}>
                                        <a
                                            style={{ overflow: 'hidden', whiteSpace: 'nowrap', textOverflow:'ellipsis', cursor:'pointer'}}
