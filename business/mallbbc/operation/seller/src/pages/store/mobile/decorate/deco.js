...
export default class Edit_diy_page extends Component {
    ...
    onDragEnd = result => {
        ...
        if (source.droppableId === destination.droppableId) {
            ...
            selected = items
            this.setState({selected});
        } else if(destination.droppableId === 'droppable2'&&source.droppableId == 'droppable'){
    ...
    get_diy_page_detial = (id) => {
        ...
        dispatch({
            ...
            payload: { decoId: id, storeId:query.storeId },
            callback: (res) => {
                if (res.state == 200) {
                    //空处理
                    if (res.data.data) {
    ...
    handleCurSelData = (changedData, allData = this.state.selected, selData = this.state.select_data,type) => {
        ...
        }else if(changedData?.props?.isChildren){
        ...
        dispatch({
            ...
            callback: (res) => {
                if (res.state == 200) {
                    ...
                    //2s之后返回上一页
                    setTimeout(() => {
                        router.push('/store/decorate_mhome');
                    }, 2000);
                } else {
                    failTip(res.msg);
                }
                ...
                <div className={`${mdiy.decoheight}`} style={{ overflowX:'scroll',minWidth:1100,overflowY:'hidden' }}>
                    <div className={global.flex_row_between_start} style={{ height: '100%' ,width:1730}}>
                        <DragDropContext onDragEnd={this.onDragEnd}>
                                        ...
                                        <div className={`${mdiy.left_wrap} ${global.flex_com_row_start_start}`} style={{ flexWrap: 'wrap' }} ref={provided.innerRef}>
                                            {menu_data.map((item, index) => (
                                                <Draggable key={item.id} draggableId={item.id} index={index}>
                                                    ...
                                                    {(provided, snapshot) => <div className={`${mdiy.item}  ${global.flex_com_column_center_center}`} ref={provided.innerRef} {...provided.draggableProps} {...provided.dragHandleProps}>
                            ...
                            <Scrollbars style={{ width:450 ,overflowX:'heidden' }}>
                            ...
                            { showIframe &&<PreviewIframe decoParams={decoParams} decoType={decoType} changeIframe={this.changeIframe} />}
                        </DragDropContext>
