...
import { loginOut,getLocalStorageStingVal, setSession } from './utils';
...
const cachedSave = (response, hashcode) => {
    ...
    if (contentType && contentType.match(/application\/json/i)) {
        ...
        response
            ...
            .text()
            .then(content => {
                setSession(hashcode, content);
                setSession(`${hashcode}:timestamp`, Date.now());
            });
    }
    return response;
};
...
export default function request(url, options, type = '') {
    ...
    if(url.indexOf('seller/oauth/token')>-1||url.indexOf('sellerLogin/oauth/register')>-1||url.indexOf('system/common/smsCode')>-1||url.indexOf('seller/seller/vendor/updatePwd')>-1){
        newOptions.headers.Authorization = 'Basic c2VsbGVyOnNlbGxlcg==';
    }else{
        newOptions.headers.Authorization = `Bearer ${getLocalStorageStingVal('token')}`;
    }
