...
import { sldInputAfterAddons, sldBeforeUpload, getSldComImg, getLocalStorageStingVal,sldBeforeUploadVideo,getStorage } from '@/utils/utils';
	    ...
	    if (val.type == 'input') {
		  ...
		  //树选择器——多选
	        ...
	        >
	            ...
	            >
	                {val.fileList.length >= (val.num!=undefined?val.num:9) ? null : uploadButton}
	            </Upload>
	        </FormItem>;
	    } if (val.type == 'upload_video') {
	        return <FormItem
	            key={index}
	            style={{ width: '100%' }}
	            extra={val.extra}
