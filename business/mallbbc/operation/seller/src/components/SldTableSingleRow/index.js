...
import { addGoodsSpecLimit,addGoodsSpecValLimit } from '@/utils/sldconfig';
...
export default class SldTableSingleRow extends PureComponent {
  ...
  commonCon = (val, index) => {
      let {
          ...
          form: { getFieldDecorator }, item_width,disable_spec
      } = this.props;
      //普通输入框
      ...
      //radio
          ...
          let con = <div className={styles.spec_wrap}>
              {
                  val.show_data.length > 0 && val.show_data.map((vall, keyl) => {
                      ...
                      if (vall.specValueInfoList != null && vall.specValueInfoList.length > 0) {
                          vall.specValueInfoList.forEach(item => {
                              if (item.sele_flag == undefined || (item.sele_flag != undefined && !item.sele_flag)) {
                                  specValData.push(item.specValueName);
                              }
                          });
                      }
                      ...
                      >
                          <div className={`${styles.spec_bg} ${global.flex_row_start_center}`}>
                              ...
                              <div
                                  className={styles.del_spec}
                                  onClick={() => val.handleDelAddSpec({ type: 'spec', specId: vall.specId })}
                                  style={{display:`${disable_spec?'none':'block'}`}}
                              >
                                  <ALibbSvg
                                      fill="#c8c8c8"
                                      width={18}
                              ...
                              <Select
                                  ...
                                  style={{ width: 200 }}
                                  getPopupContainer={triggerNode => triggerNode.parentNode}
                                  disabled={disable_spec?true:false}
                              >
                                  {specData.map(item => (
                              ...
                              <div className={styles.spec_default_img_spec} style={{display:'none'}}>
                                  <Checkbox
                                      checked={vall.is_img_spec != undefined && vall.is_img_spec ? true : false}
                                      onChange={(e) => val.setImgSpec(e, vall.specId)}
                                  >{sldComLanguage('设为图片规格')}</Checkbox>
                              </div>
                          </div>
                          ...
                          <div className={styles.spec_item_wrap}>
                              ...
                              <div className={global.flex_com_row_start_center} style={{ flexWrap: 'wrap' }}>
                                  {vall.showValList.map((valr) => valr.sele_flag != undefined && valr.sele_flag && <div className={styles.spec_val_item_wrap}>
                                      <div
                                          className={styles.spec_val_del}
                                          onClick={() => val.handleDelAddSpec({
                                              type: 'spec_val',
                                              specId: vall.specId,
        ...
        <Fragment>
            ...
            <span className={styles.spec_name_tile}>{sldComLanguage('规格名：')}</span>
                ...
                <div
                    className={styles.del_spec}
                    style={{ display: val.is_add_spec ? 'inline-block' : 'none' }}
            ...
            <div>
                ...
                <Button className={disable_spec?'':styles.add_spec} disabled={disable_spec?true:false} onClick={() => val.addSpec(!val.is_add_spec)}>添加规格项</Button>
            </div>
                }
            </div>
        </Fragment>
