                ...
                render : ( text , record ) => {
                    ...
                    if(record.type == 'input'){
                        ...
                        content = <div className={styles.modal_img}>
                            ...
                            <span className={styles.modal_tip_color}>{sldComLanguage('此处对应上传上方选中标签项内容，要求宽度为')}{this.state.img_width}{sldComLanguage('像素、高度')}{this.state.img_height==0?`${sldComLanguage('不限制')}`:`${this.state.img_height}${sldComLanguage('像素的图片；支持格式gif，jpg，png。')}`}</span>
                            <Upload
				                        withCredentials
  ...
  sldConfirm = (e) => {
      ...
      this.props.form.validateFieldsAndScroll((err, values) => {
          if (!err) {
              ...
              this.props.sldHandleConfirm({ ...tmp_info, ...values });
              this.sldCancle();
          }
      });
  };
