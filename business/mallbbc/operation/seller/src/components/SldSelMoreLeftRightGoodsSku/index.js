...
import {
    Form, Modal,Empty,Upload,Button,message,Tooltip,Spin
} from 'antd';
...
export default class SldSelMoreLeftRightGoods extends Component {
  ...
  get_list_upLoad = (params,sliceNum,sliceTotal,resSliceTotal) => {
      ...
      let new_params = { pageSize:1000, pageIndex: 1,skus: ids,storeId };
          ...
          callback: (res) => {
              if (res.state == 200) {
                  ...
                  if( sliceTotal && (sliceTotal > 0) ){
                      ...
                      // resSliceTotal = this.mergeTree(resSliceTotal,'goodsId','productList');
                      // console.log(resSliceTotal)
                      // 这里对返回数据进行处理 用于sku处理
                      // let skuList = []
                      // resSliceTotal.map((item)=>{
                      //   item.productList.map((el)=>{
                      //     el.goodsPrice = el.salePrice
                      //     el.mainImage = el.mainImage
                      //   })
          ...
          if(!!jsonData.length && jsonData.length > 0){
              ...
              for(let index = 0; index < jsonData.length; index++) {
                  ...
                  if(Array.isArray(ele)&&ele.length>0){
                      ...
                      }else if( ele[0] && (/^[A-Za-z0-9]+$/.test( ele[0] )) ){
