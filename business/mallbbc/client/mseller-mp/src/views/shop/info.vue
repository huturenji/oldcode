    ...
    <view class="container">
        <template v-if="!loading">
            <view v-if="shopList && shopList.length > 0" class="wrap">
                <view v-for="(item, index1) in shopList" :key="item.shopId" class="wrap_item">
                    <view class="left">
                    ...
                    <view class="right">
                        ...
                        <view v-if="item.servicePhone && item.servicePhone.length > 0" class="phone">
                            ...
                            <view class="text">
                                <text @click="callTel(temp)" v-for="(temp, index) in shopList[index1].servicePhone" :key="index">{{index==(item.servicePhone.length-1) ? temp : (temp+',')}}</text>
                            </view>
                        </view>
                    </view>
                </view>
...
import { isNotEmpty, checkPaginationHasMore, callPhone } from '@/utils/common';
export default {
    mixins: [],
    data() {
        return {
