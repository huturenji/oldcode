        ...
        <view v-if="detail.state == 1" class="promotion_will_start flex_row_start_center">
            ...
            <view class="center flex_column_between_start">
                <view class="tabs flex_row_center_center">
                    <image :src="imgUrl + 'goods/icon_jsh_yiqimai.png'" mode=""></image>
            ...
            <view v-if="!disabledModule" class="right flex_column_between_end">
                <view class="preview flex_row_center_center" v-if="!detail.remind" @click="subscribe">
                ...
                <view class="start_time">
                    {{detail.startTime.substr(0,16) + ' '+$L("开售")}}
                </view>
            </view>
        </view>
        <!-- 一起买活动未开始 end -->
        ...
        <view v-if="detail.state == 2" class="promotion_start flex_row_between_center">
            <view class="left">               
               <view class="left_top flex_row_start_center">
                    ...
                    <view v-if="detail.buyQuantity && detail.buyQuantity >= 5" class="tab1 flex_column_center_center">已拼{{detail.buyQuantity}}件</view>
                    <view class="tab flex_column_center_center">{{detail.wishStock}}件成团</view>
               </view>
               <view class="left_bottom flex_row_start_end">
                    <view class="price1">
                        <text class="num-font">¥</text>
                        <text class="num-font intPrice">{{filters.toSplit(filters.toFix(detail.promotionPrice))[0]}}.</text>
                        <text class="num-font">{{filters.toSplit(filters.toFix(detail.promotionPrice))[1]}}</text>
                    </view>
                    <view v-if="priceInfo.originalSalePrice" class="price2">
                        <text class="num-font">¥</text>
                        <text class="num-font intPrice">{{filters.toSplit(filters.toFix(priceInfo.originalSalePrice))[0]}}.</text>
                        <text class="num-font">{{filters.toSplit(filters.toFix(priceInfo.originalSalePrice))[1]}}</text>
                    </view>
                    <view v-else class="no_price" >
                        <text>￥</text>
            ...
            if (!this.hasLogin) {
                ...
                promotionHandler.setBuyTogetherRemind(param).then(res => {
                    if (res.state == 200) {
                        ...
                        window.stageSkuId = this.detail.productId;
...
.promotion_will_start{
    padding: 0 28rpx;
    height: 100%;
    background: url('@/static/shared/goods/bg_jsh_yiqimmai.png') center no-repeat;
    ...
    .center{
        ...
        .tabs{
            ...
            .tab{
                color: var(--elementColor1);
                min-width: 110rpx;
                height: 36rpx;
                font-size: 20rpx;
                border: 1px solid var(--elementColor1);
                margin-left: 10rpx;
                text-align: center;
                border-radius: 4rpx;
                padding: 8rpx 6rpx;
...
.promotion_start{
    padding: 0 28rpx;
    height: 100%;
    background: url('@/static/shared/goods/bg_jsh_yiqimmai_nor.png') center no-repeat;
    background-size: 100% 100%;
    .price{
        margin-bottom: 10rpx;
        color: #fff;
...
.promotion_icon{
    line-height: 40rpx;
    height: 40rpx;
    background: url('@/static/shared/goods/icon_common_naozhong_white.svg') left center no-repeat;
...
.sec_kill_countdown .time { 
    ...
    height: 36rpx;
    line-height: 36rpx;
    background: url('@/static/shared/goods/bg_jsh_time1.png') center no-repeat;
    background-size: 100% 100%;
    font-weight: bold;
    text-align: center;
    color: #ffffff;
...
.sec_kill_countdown .time_tips {
    width: 6rpx;
    height: 24rpx;
    background: url('@/static/shared/goods/bg_jsh_time5.png') center no-repeat;
    background-size: 100% 100%;
    margin: 0 12rpx;
}
