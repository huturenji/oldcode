    ...
    <view class="container flex_column_start_start">
        <template v-if="step==1">
            <view class="bind_mobile flex_row_start_center">
                {{$L('使用已绑定手机号')}}{{userCenterData.memberMobile?this.$replaceConByPosition(this.userCenterData.memberMobile,4,6,'****'):''}}{{$L('进行身份验证')}}
            </view>
            <view class="main_content">
                <view class="input-item pwd_wrap flex_row_between_center">
                    <input type="number" class="sms_code" :value="smsCodeOri" maxlength="6" :placeholder="$L('请输入短信验证码')" placeholder-class='smsCodePlaceholder'
        ...
        <template v-if="step==2">
            <view class="second_main_content">
                <view class="flex_row_between_center input-item pwd_wrap ">
                    <input type="number" class="sms_code" :value="mobile" maxlength="11" :placeholder="$L('请输入新手机号')" placeholder-class='smsCodePlaceholder'
                     data-key="mobile" @input="inputChange"  />
                    <view class="pwd-right flex_row_end_center">
                        <text class="clear-pwd iconfont icon_close" v-show="mobile" @click="clearContent('mobile')"></text>
                    </view>
                
                </view>
                <view class="flex_row_between_center input-item pwd_wrap " style="margin-top: 20rpx">
                    <input type="number" class="sms_code" :value="smsCode" maxlength="6" :placeholder="$L('请输入短信验证码')" placeholder-class='smsCodePlaceholder'
...
export default {
    ...
    methods: {
        ...mapMutations(['setUserCenterData', 'hasLogin']),
        //获取个人中心数据
        initData() {
            if (this.hasLogin){
        ...
        getSmsCode() {
            ...
            param.data.verifyAddr = this.step==1?this.userCenterData.memberMobile:this.mobile;
            param.data.type = '';
        ...
        confirm(){
            ...
            if (this.step == 1){
                param.method = 'GET';
                param.url = 'v3/member/front/member/verifyOldMobile';
