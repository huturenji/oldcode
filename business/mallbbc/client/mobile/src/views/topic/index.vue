<template>
    <view class="container" :key="topic_id">
        <!-- #ifdef H5-->
        <home-deco :deco_info="deco_data" ref="homeDecoComp" :defaultScrollView='defaultScrollView' :is_show_top="false" :topic_name="topic_name" :is_from_found="false"></home-deco>
        <!-- #endif -->
        <!-- #ifdef MP-WEIXIN||APP-PLUS -->
        <home-deco :deco_info="deco_data" :defaultScrollView='defaultScrollView' :is_show_top="false" :topic_name="topic_name" :is_from_found="true"></home-deco>
        <!-- #endif -->
    </view>
</template>
    ...
    methods: {
            ...
            decorateHandler.getTopicDeco(param,config).then(async res => {
                if (res.state == 200) {
                    ...
                    if (res.data.data != ''){
                        ...
                        // console.log('this.deco_data', this.deco_data)
                    } else {
                        this.deco_data = []
                    }
        ...
        async setThirdShare(shareLink){
            ...
            if (false===shareLink){ //装修页配置不分享链接
                ...
                if (setShareUrl(locationHref,this.shareChannelId)) {
                    callBackUrl = setShareUrl(locationHref,this.shareChannelId)
                }
                let shareInfo = {
