    ...
    <view class="select_service">
        <view class="order_goods">
            <view class="goods_list">
                <view class="goods_pre" @click="goProductDetail(orderProduct.sku,orderProduct.spu)">
        ...
        <view class="uni-list service_list">
            <radio-group>
                <label class="service_pre" v-for="(item,index) in servicelList" :key="index" @click="goApplyRefund(item.value)">
                    ...
                    <image :src="imgUrl+'common/icon/icon_common_rightarrow.svg'" mode="" class="service_right"></image>
                </label>
            </radio-group>
        </view>
        <view v-if='showTips && servicelList.length==0' class="empty_part flex_column_start_center">
            <view class="img"></view>
            <text>{{$L('该商品无法售后~')}}！</text>
        </view>
        <!-- 服务方式 end -->
...
export default {
    ...
    methods: {
        ...
        goApplyRefund(value){
            ...
            this.$Router.push({
                path:'/views/order/aftersale/apply',
                query:{
