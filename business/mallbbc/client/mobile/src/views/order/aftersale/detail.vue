...
<template>
    <scroll-view class="container" scroll-y v-if="allData && isShow">
        ...
        <view class="main_content">
            ...
            <view class="order_state">
                ...
                <block v-if="allData.state == 101">
                    <view class="state_title">
                        <image :src="imgUrl+'order/refund_progress.png'" mode=""></image>
                ...
                <block v-if="allData.state == 201">
                    <view class="state_title">
                        <image :src="imgUrl+'order/refund_progress.png'" mode=""></image>
                        <text>{{allData.stateValue}}{{allData.pickWareMethod==4?$L('请等待上门取件'):$L('请退货并填写物流信息')}}</text>
                    </view>
            ...
            <!-- 退货地址 及 换货地址 -->
            <view class="refund_address" v-if="allData.afsAddress && allData.afsAddress != 'null' && (allData.afsType == 1 || allData.afsType == 2)">
            ...
            <view class="refund_amount" v-if="allData.afsType == 4">
                <view class="refund_amount_left">{{$L('维修数量')}}</view>
                <view class="refund_amout_right">
                    <view class="refund_amout_price">
                        <text>{{allData.afsNum}}</text>
                    </view>
            ...
            <view class="logistics_bill" v-if="allData.state == 280">
                <text @click="userConfirm">{{$L('用户确认')}}</text>
            </view>
            <!-- 填写物流单号 end -->
            ...
            <!-- 等待商家收货并退款 -->
            <view class="refund_logisticsBill" @click="lookLogistics(1)" v-if="allData.buyerExpressName && (allData.state == 102 || allData.state == 203 || allData.state == 300)">
                <text>{{$L('退货物流')}}</text>
                <image :src="imgUrl+'common/icon/icon_common_rightarrow.svg'" mode=""></image>
            </view>

            <!-- 维修返件物流 -->
            <view class="refund_logisticsBill" @click="lookLogistics(2)" v-if="allData.storeExpressName && (allData.state == 280 || allData.state == 300)">
                <text>{{$L('商家返件')}}</text>
                <image :src="imgUrl+'common/icon/icon_common_rightarrow.svg'" mode=""></image>
            </view>
            ...
            <view class="order_goods">
                <view class="order_goods_title">{{$L(`${afsText}信息`)}}</view>
                <view class="goods_list">
                    <view class="goods_pre" @click="goProductDetail(allData.sku)">
                        ...
                        <view class="goods_pre_right">
                            ...
                            <view class="goods_prices fontScaleIgnore">
                                <view class="goods_price">
                                    ...
                                    <text class="price_decimal">{{$getPartNumber(allData.productPrice,'decimal')}}</text>
                                </view>
            ...
            <view class="order_des">
                ...
                <view class="order_des_pre">
                    <text>{{$L(`${afsText}原因`)}}：</text>
                    <text>{{allData.applyReasonContent}}</text>
                </view>
                <view class="order_des_pre">
                    <text style="white-space: nowrap;">{{$L(`${afsText}说明`)}}：</text>
                    ...
                    <view class="refund_img_wrap">
                        <view class="refund_vouhcer_list">
                            <view class="refund_voucher_pre" v-for="(item, index) in applyImageList" :key="index">
                                <image :src="item" mode="widthFix" @click="preRefundVoucher(index)"></image>
                            </view>
                        </view>
                    </view>
                </view>
                ...
                <view class="order_des_pre" v-if="allData.afsType==1 && allData.returnMoneyAmount>0">
                    <text>{{$L('支付方式')}}：</text>
                    <text>{{allData.paymentName}}</text>
                </view>
                <view class="order_des_pre" v-if="allData.afsType==1 && allData.returnMoneyAmount>0">
                    <text>{{$L('退款路径')}}：</text>
                    <text>原路退回</text>
                ...
                <view class="order_des_pre">
                    <text>{{$L(`${afsText}编号`)}}：</text>
                    <text>{{allData.afsSn}}</text>
                    <text class="copytextbut" @click="copyStr(allData.afsSn)" v-if='allData.afsSn'>复制</text>
                </view>
                ...
                <view class="order_des_pre" v-if='allData.pickWareMethod==4 && allData.pickWareAddress'>
                    <text>{{$L('取件地址')}}：</text>
                    <text>{{`${allData.pickWareAddress.addressAll}${allData.pickWareAddress.detailAddress} ${allData.pickWareAddress.memberName} ${allData.pickWareAddress.telMobile}`}}</text>
                    <text  class="copytextbut" @click="copyStr(`${allData.pickWareAddress.addressAll}${allData.pickWareAddress.detailAddress} ${allData.pickWareAddress.memberName} ${allData.pickWareAddress.telMobile}`)" v-if="allData.pickWareAddress&&allData.pickWareAddress.addressAll">复制</text>
                </view>
                <view class="order_des_pre" v-if='allData.pickWareMethod==40'>
                    <text>{{$L('退回地址')}}：</text>
                    <text>{{`${allData.storeAddress?allData.storeAddress:''} ${allData.storeContactName?allData.storeContactName:''} ${allData.storeTelphone?allData.storeTelphone:''}`}}</text>
                </view>
                <view class="order_des_pre" v-if="(allData.afsType==2||allData.afsType==4) && allData.receiveWareAddress">
                    <text>{{$L('收货地址')}}：</text>
                    <text>{{`${allData.receiveWareAddress.addressAll}${allData.receiveWareAddress.detailAddress} ${allData.receiveWareAddress.memberName} ${allData.receiveWareAddress.telMobile}`}}</text>
                </view>
            </view>
            <!-- 退款信息  end-->
            ...
            <view v-if="allData.showCancelBtn ">
                <view class="user_cancel">
                    <text @click="userCancel">{{$L('取消申请')}}</text>
                </view>
            </view>

            <uni-popup ref="popup" type="dialog">
                <uni-popup-dialog type="input" title="提示" :content="`确定取消${afsType==1?'退货':afsType==2?'换货':'维修'}申请`" :duration="2000" @confirm="subCancel">
                </uni-popup-dialog>
            </uni-popup>
...
export default {
    ...
    methods: {
        ...
        getOrderDetail(){
            let that = this;
            let param = {};
            param.url = that.sourceType == 'exchange' ? 'v1/front/member/afterSale/replacementDetail' : 'v3/postsale/front/after/sale/detail'; //获取退货详情接口
            param.method = 'GET';
            param.data = {};
        ...
        //去填写物流单号页面
        goLogisticsBill(){
            this.$Router.push({path:'/views/order/logistics/fill',query:{afsSn:this.afsSn}})
        },
        //去查看物流页面
        lookLogistics(traceType){
            this.$Router.push({path:'/views/order/logistics/detail',query:{afsSn:this.allData.afsSn,type:'afs',traceType}})
        },
        //去进度详情页面
        progressDetails(){
            this.$Router.push({path:'/views/order/aftersale/progress',query:{afsSn:this.afsSn}})
        },
        //退款失败，重新申请退款
        goApplyRefund(){
            let that = this;
            //都进入到选择服务页面
            this.$Router.push({
                path:'/views/order/aftersale/index',
                query:{
        ...
        userConfirm(){
            let param = {};
            param.url = 'v3/postsale/front/after/sale/apply/confirm';
            param.method = 'POST';
            param.data = {};
            param.data.afsSn = this.allData.afsSn;
        ...
        subCancel(){
            ...
            let param = {};
            param.url = 'v3/postsale/front/after/sale/apply/cancel';
            param.method = 'POST';
            param.data = {};
            param.data.afsSn = this.allData.afsSn;
            this.$request(param).then(res=>{
