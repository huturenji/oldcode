<template>
    <view id="container" class='container'>
        ...
        <view @click="operateAddress" class="address-section">
            <view class="order-content" v-if="addressDone && orderAddress">
            ...
            <view class="order-content default_order_content" v-else>
                <view class="default_top animated-background"></view>
                <view class="default_address first-animated-background"></view>
                <view class="default_address first-animated-background"></view>
                <view class="default_member_info animated-background"></view>
            </view>

            <!-- <image class="a-bg" :src="imgUrl+'order/address_split_border.png'"></image> -->
        </view>
        ...
        <view class="goods-section flex_column_start_start" v-if="goodsData.length>0">
            ...
            <view class="store_list" v-for="(item,key,index) in goodsData" :key='index'>
                ...
                <view class="product_con" v-for="(product,key,index) in item.products" :key='index'>
                    <view class="g-item flex_row_start_start">
                        ...
                        <view class="right flex_column_between_start">
                            ...
                            <view class="priceAndnumber flex_row_between_end">
                                ...
                                <uni-number-box
                                    :min="product.lowestBuy ? product.lowestBuy : 1"
                                    :value="product.number"
                                    :inputClass="'goodsInput'+product.sku"
                                    @change="changeGoodsNumber($event,product)">
                                </uni-number-box>
                            </view>
                        </view>
                    </view>
                ...
                <view class="yt-list store_info" v-if="loadNumber>0">
                    ...
                    <view class="yt-list-cell" v-if="!!orderAddress.addressId" @click="showFreightDetail(item.expressFeeInfo,item.storeId)">
                        <view class="cell-tit cell_image">
                            <text>{{$L('运费')}}</text>
                            <image :src="imgUrl+'common/icon/btn_common_gantanhao2@2x.png'" mode="" @click.stop="showFreightRule(item.expressFeeInfo,item.storeId)" v-if="item.expressFeeInfo.state==1"></image>
                        </view>
                        <view class="cell-tip voice num-font" :class="{arrow_right:loadedData && !!orderAddress.addressId && item.expressFeeInfo.state==1 && !(item.expressFeeInfo.lowerAmount==-1 || item.expressFeeInfo.originFreight==0)}"
                        v-if="loadedData && !!orderAddress.addressId">{{(item.expressFee || (item.expressFee==0 && item.expressFeeInfo.state==1 && item.expressFeeInfo.lowerAmount!=-1 && item.expressFeeInfo.originFreight>0))? ('+￥' +
                            $getPartNumber(item.expressFee,'int')+$getPartNumber(item.expressFee,'decimal')):'免运费'}}</view>
                        <view v-if="loadedData && !orderAddress.addressId" :style="{color:'#999'}">{{$L('暂无运费信息，填写收货地址后将自动获取')}}</view>
                    </view>
        ...
        <template>
            <view class="yt-list" v-if="loadNumber>0">
                ...
                <!-- 运费总价 -->
                <view class="yt-list-cell" v-if="!!orderAddress.addressId">
                    <view class="cell-tit">{{$L('运费总价')}}</view>
                    <view class="cell-tip voice num-font">{{allData.freightAmount ? ('+￥' + $getPartNumber(allData.freightAmount,'int') + $getPartNumber(allData.freightAmount,'decimal')) : '免运费'}}</view>
                </view>
                <!-- 满减金额 -->
            ...
            <view id="store_no_good" v-if="store_show_no_good" @touchmove.stop.prevent="moveHandle">
                <view class="content">
                    ...
                    <view v-if='no_good_info.state==2' class="part_no_goods">
                        ...
                        <view class="remove" @click='removeGoods'>
                            {{$L('移除无货商品')}}
                        </view>
                    </view>
        ...
        <bottomPopup ref="intInstallDate" type="bottom" height="832rpx" :showTitle="false" :showCloseBtn="false">
               <view class="promise-date-container">
                    <view class="popup_title">{{$L("配送时间")}}</view>
                    <view class="close_icon" @click="close_intInstallDate"></view>
                    <block>
        ...
        <bottomPopup ref="SendDatePop" type="bottom" height="900rpx" :showTitle="false" :showCloseBtn="false">
               <view class="promise-date-container">
                    <view class="popup_title">{{$L("配送时间")}}</view>
                    <view class="close_icon" @click="close_sendDatePop"></view>
                    <block>
...
export default {
    data() {
        return {
            ...
            fromeCart:null,//页面是从购物车下单
            cartIdList:[],
            goodsMaxNum:999,//单个商品最大购买数
            loadedData:false,
            ...
            tempReMarkMap:{},
            currRemarkstoreId:'',
            lastComfirmHaveAddress:null//确认订单时是否有地址信息
        }
    },
    async mounted(){
        ...
        this.fromeCart = !!this.$Route.query.ifcart && this.$Route.query.ifcart=='1';
    ...
    methods: {
        ...
        getDiscount(item){
            ...
            const content = item.usePromotionDes;
            ...
            const lastContent = content.substring(lastRightBrackets+1, content.length);
            let relContent 
            if (promotionType === 201) {
                relContent = `满${fullPrice}减${discount}${lastContent}`
            }
            if (promotionType === 202) {
                relContent = `每满${fullPrice}减${discount}${lastContent}`
            }
            if (promotionType === 203) {
                ...
                relContent = `满${fullPrice}打${_discount}${lastContent}`
            }
            if (promotionType === 204) {
                ...
                relContent = `${fullPrice}件${_discount}${lastContent}`
            }
            if(promotionType === null){
                relContent = item.promotionDescription
            }
            relContent = relContent.replace('元','')
            return relContent
        },
        //选择优惠券
        ...
        async getAddressList(isShow) {
            ...
            if(this.fromeCart && this.cartIdList.length==0){
                this.cartIdList = JSON.parse(this.$getStorageSync('confirmParams')).cartIdList;
            }
         ...
         * @param Object 商品对象
         * @param String 事件类型blur change
         * 
        */
        changeGoodsNumber(numObj,product){
            let numObjNew=JSON.parse(JSON.stringify(numObj))
            let num = parseInt(numObjNew.value);
            let preNum = parseInt(numObjNew.preValue);
            if(num>this.goodsMaxNum){
                num = this.goodsMaxNum
                uni.showToast({
                    title: '超过最大购买量',
                    icon: 'none'
                })
            }
            this.changeGoodsNumberBis(num,product);

            if(preNum==num){ return }
            this.changeGoodsNumcheck(preNum,product);
        }, 
        /**
         * 修改商品数量后check是否有库存和限购
         * @param String 切换的商品数量对象
         * @param Object 商品对象
         * @param String 事件类型blur change
         * 
        */      
        changeGoodsNumcheck(preNum,product) {
            this.loadedData = false
            const { preParam } = this;
            let param= {};
            param.isAloneBuy = this.isAloneBuy //拼团商品是否单独购买
            param.source = 2;
            param.products = JSON.parse(preParam.products).map(item => {
                return {
                    sku:item.sku,
            ...
            param.products.map( goodsItem =>{
                !!!goodsItem.notAttendDiscount && (goodsItem.notAttendDiscount = false);
                return product
            })
            ...
            orderHandler.checkOnce(param).then(res => {
                if (res.state == 200) {
                    this.confirmOrder(1,()=>{
                        this.changeGoodsNumberBis(preNum,product);
                    });
                }else if (res.state == 267) {
                    this.no_good_info_data = res.data;
                    //2023-3-22原check逻辑移动到confirm进行，todu check接口需要废弃。
                    this.loadedData = true
                    //修改数量后异常情况回滚操作
                    if (res.data.productList.findIndex(item1 => item1.sku == product.sku) > -1) {
                        this.changeGoodsNumberBis(preNum,product);
                        //修改数量引起的库存不足或限购使用showtoast提示
                        // if(res.data && res.data.state==6){
                        //     this.$api.msg('超过商品限购数量');
                        // }else if(res.data && (res.data.state==4 ||res.data.state==3)){
                        //     this.$api.msg('库存不足');
                        // }else{
        ...
        /**
         * 修改商品数量
         * @param String 切换的商品数量
         * @param Object 商品对象
         * 
        */
        changeGoodsNumberBis(num,product){
            let that = this;
            product.number = num;
            document.querySelector('.goodsInput'+product.sku+' input').value = num;
            let products = JSON.parse(that.preParam.products);
            let index = products.findIndex((skuItem)=>{ return skuItem.sku == product.sku })
            products[index].number = num;
            that.preParam.products = JSON.stringify(products);
        },
        /**
         * 购物车跳转下单后修改数量，删除购物车商品
         * 
        */
        deleteCartGoods(){
            let carts = this.cartIdList.map(item=>{
                return item.cartId;
            })
            let param = {cartIds:carts.join(',')}
        ...
        //用于切换地址，使用优惠券，获取信息，运费等  type 服务端参数解释 1立即购买 去结算;2 提交订单页 修改优惠券 修改云豆 修改地址 修改数量 更新页面数据 ;3 提交订单 
        confirmOrder(type,unchanngeGoodsNum) {
            //修改数量后需要清除选中的优惠券红包积分等信息
            let isChanngeGoodsNum = !!unchanngeGoodsNum;
            let param = {};
            let handlerType = 'confirm';
            let isAddAddressTimes = false;
            let comfirmHaveAddress = null;
            param.data = {};
            

            // 订单接口 地址id替换为地址相关对象
            if (this.orderAddress.addressId) {
                comfirmHaveAddress = true;
                ...
                comfirmHaveAddress = false;
            }
            //从无地址变成有地址，清空优惠券数据，默认选中运费券
            if(this.lastComfirmHaveAddress==false && comfirmHaveAddress){
                isAddAddressTimes = true;
            }

            //修改商品数量清空优惠券数据；从无地址变成有地址，清空优惠券数据，默认选中运费券
            if(!isChanngeGoodsNum && !isAddAddressTimes){
                param.data.platformCouponCode = this.currentPlatformCouponCode
            }
            this.lastComfirmHaveAddress = comfirmHaveAddress;
            param.data.orderSn = '';
            ...
            //修改商品数量清空积分数据
            if (this.integral > 0 && !isChanngeGoodsNum) {
            ...
            this.goodsData.forEach((store) => {
                ...
                store.availableCouponList && store.availableCouponList.forEach(item => {
                    //修改商品数量清空优惠券数据
                    if (item.checked == true && !isChanngeGoodsNum) {
            ...
            orderHandler[handlerType](param.data).then(res => {
                ...
                if (res.state == 200) {
                    ...
                    if (this.getGiftList.skuGiftAndAttachmentInfoList) {
                    ...
                    //修改数量后异常情况回滚操作
                    unchanngeGoodsNum && unchanngeGoodsNum();
                    uni.hideLoading();
                    this.$api.msg(res.msg);
                }
                this.loadedData = true
            ...
            try {
                ...
                if ( productList && productList.length > 0){
                    for (let i = 0; i < productList.length; i++) {
                        ...
                        let list = JSON.parse(this.preParam.products)
                        list.splice(list.findIndex(item => item.sku == element.sku),1)
                        this.preParam.products = JSON.stringify(list)

                        if (this.cartIdList.findIndex(item => item.sku == element.sku)>-1) {
                            this.cartIdList.splice(this.cartIdList.findIndex(item => item.sku == element.sku),1)
                        }
        ...
        _submit(gift) {
            ...
            this.goodsData.forEach(item => {
                ...
                //submit提交之前，对商品的sku做一个保存，然后传递到支付页
                this.queryskus = []
                item.products.forEach(product=>{
                    this.queryskus.push(product.sku)
                })
            ...
            orderHandler[handlerType](param.data).then(res => {
                if (res.state == 200) {
                    ...
                    //页面初始是从购物车下单,是否修改了商品数量导致修改了下单模式为普通商品下单，此时需要手动删除购物车对应的商品
                    if(this.fromeCart){
        ...
        gotoNextPage(res){
            ...
            //orderPayState ==1 来判断 已支付 0元单跳转zeroPayResult
        ...
        showFreightDetail(info,storeId) {
            if (this.loadedData && !!this.orderAddress.addressId && info.state==1 && !(info.lowerAmount==-1 || info.originFreight==0)) {
