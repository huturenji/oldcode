...
<template>
    <scroll-view class="container" scroll-y @scrolltolower='getData' v-if="allData && isShow">
        ...
        <view class="main_content" >
            ...
            <view class="order_state">
                ...
                <block v-if="allData.orderState == 10">
                    <view class="state_title">
                        <image :src="imgUrl+'order/to_pay.png'" mode=""></image>
                        <text>{{allData.orderStateValue}}</text>
                    </view>
                    
                    <!-- 定金预售 已付定金 还未到付尾款阶段 start -->
                    <view class="state_time" v-if="allData.orderType == 103&&allData.orderSubState == 102&&!(allData.presellInfo&&allData.presellInfo.isStartRemainPay)">
                        {{allData.presellInfo.remainStartTime}} {{$L('开始付尾款')}}
                    </view>
                    
                    <view class="state_time" v-else-if="allData.orderType == 104">
                        {{$L('剩余')}}{{stateTime}}{{$L('自动关闭')}}
                    </view>
                    
                    <!-- 定金预售 已付定金 还未到付尾款阶段 end -->
                    <view class="state_time" v-else-if="stateTime">
                        {{$L('剩余')}}{{stateTime}}{{allData.orderType==105&&allData.orderSubState==102&&allData.ladderGroupDetailInfo.depositRemainTime>0?$L('生成'):$L('自动关闭')}}
                    </view>
                ...
                <block v-if="allData.orderState == 20">
                    <view class="state_title">
                        <image :src="imgUrl+'order/delivered.png'" mode=""></image>
                        <text>{{allData.orderStateValue}}</text>
                    </view>
                    <!-- 预售展示发货时间 start -->
                    <view v-if="allData.orderType==103&&allData.presellInfo.deliverTime" class="state_time">
                        {{allData.presellInfo.deliverTime}} {{$L('开始发货')}}</view>
                    <!-- 预售展示发货时间 end -->
                </block>
                <!-- 待发货 end-->
                <!-- 待收货 -->
                <block v-if="allData.orderState == 30">
                    <view class="state_title">
                        <image :src="imgUrl+'order/wait_receive.png'" mode=""
                            style="width: 71rpx;height: 45rpx;"></image>
                        <text>{{allData.orderStateValue}}</text>
                    </view>
                ...
                <block v-if="allData.orderState == 40">
                    <view class="state_title await">
                        <image :src="imgUrl+'order/success.png'" mode="" style="width: 46rpx;height: 51rpx;">
                        </image>
                        <text>{{allData.orderStateValue}}</text>
                    </view>
                </block>
                ...
                <block v-if="allData.orderState==50">
                    <view class="state_title await">
                        <image :src="imgUrl+'order/to_pay.png'" mode="">
                        </image>
                        <text>{{allData.orderStateValue}}</text>
                    </view>
                </block>
            ...
            <view class="logistics_information"
                ...
                <view class="logistics_des_right">
                    ...
                    <image :src="imgUrl+'common/icon/icon_common_rightarrow.svg'" mode="" class="right_down"
            ...
            <view class="order_goods">
                <view class="goods_list">
                    <view v-for="(store,index) in orderProductList" :key="index" class="store_item">
                        <view class="store_name" @click="goStore(store)">
                        ...
                        <view class="goods_pre" v-for="(item,indexChild) in store.orderProductListVOList" :key="indexChild">
                            ...
                            <view class="goods_pre_right">
                                ...
                                <view :class="{'goods_prices':true,'justify':item.productType==0}">
                                    ...
                                    <block v-if="false && (allData.orderState == 40 && item.serviceList)">
                            ...
                            <view class="store_remark" v-if="indexChild==(store.orderProductListVOList.length-1) && store.orderRemark">
                                <text>买家留言:</text>
                                <text>{{store.orderRemark}}</text>
                            </view>
                        </view>
                        <!-- 订单内商品信息 end -->
                        <view class="store_price_info no_top" v-if="allData.orderState != 10&&allData.orderState != 0&&allData.orderType!=105">
                            <view class="freight">
                        ...
                        <view class="share_btn">
                            <view class="share_btn_pre" @click="contactCustomerRefund">
                                <image :src="imgUrl+'common/icon/lianxishangjia.png'" mode=""></image>
                                <text>{{$L('联系商家')}}</text>
                            </view>
            ...
            <!-- 定金预售订单信息 start -->
            <view class="ladder_group flex_column_start_start"
                v-if="allData.orderType == 103&&allData.presellInfo.isAllPay==0">
                <template v-for="(presale_item, presale_index) in presaleInfo">
                    <view :class="{item:true, split:presale_index<(presaleInfo.length-1)?true:false}" :key="presale_index">
                        ...
                        <view class="goods_amount flex_row_between_center">
                            ...
                            <text
                                class="right">{{presale_item.goods_right=='--'?'':$L('￥')}}{{presale_item.goods_right}}</text>
                        </view>
                        <view v-if="presale_item.goods_expand_right" class="goods_amount flex_row_between_center">
                            ...
                            <text
                                class="right">{{presale_item.goods_expand_right=='--'?'':$L('￥')}}{{presale_item.goods_expand_right}}</text>
                        </view>
                        <view class="need_pay_amount flex_row_between_center">
                            <text class="left">{{presale_item.need_pay_left}}</text>
                            <text
                                :class="{right:true, cur:presale_item.is_cur==1?true:false}">{{presale_item.need_pay_right=='--'?'':$L('￥')}}{{presale_item.need_pay_right}}</text>
                        </view>
                    </view>
                </template>
            </view>
            ...
            <view class="ladder_group flex_column_start_start" v-if="allData.orderType == 105">
                <template v-for="(ladder_pro_item, ladder_pro_index) in ladderInfo">
                    <view :class="{item:true, split:ladder_pro_index<(ladderInfo.length-1)?true:false}" :key="ladder_pro_index">
                        <view class="title flex_row_start_center">
                            <view class="right_split"></view>
                        ...
                        <view class="goods_amount flex_row_between_center">
                            ...
                            <text
                                class="right">{{ladder_pro_item.goods_right=='--'?'':$L('￥')}}{{ladder_pro_item.goods_right}}</text>
                        </view>
                        <view class="need_pay_amount flex_row_between_center">
                            <text class="left">{{ladder_pro_item.need_pay_left}}</text>
                            <text
                                :class="{right:true, cur:ladder_pro_item.is_cur==1?true:false}">{{ladder_pro_item.need_pay_right=='--'?'':$L('￥')}}{{ladder_pro_item.need_pay_right}}</text>
                        </view>
                    </view>
                </template>
            </view>
            ...
            <!-- //订单价格信息 -->
            <view class="store_price_info" v-if="(allData.orderState == 10 || allData.orderState == 0)&&allData.orderType!=105">
                <view class="actual_payment">
                    <view class="actual_payment_title">
                        <text>{{$L('商品总额')}}</text>
                    </view>
            ...
            <view class="order_des">
                ...
                <view class="order_des_pre">
                    ...
                    <text v-if="allData && allData.invoice && allData.invoice.receiverMobile">{{allData.invoice.receiverMobile|formateTel}}</text>
                    <text v-else>--</text>
                </view>
                <!-- 收票人邮箱 -->
                <view class="order_des_pre">
                    <text class="invoice">收票人邮箱：</text>
                    <text v-if="allData && allData.invoice && allData.invoice.receiverEmail">{{allData.invoice.receiverEmail}}</text>
                    <text v-else>--</text>
                </view>
                ...
                <view class="order_des_pre">
                    <text>{{$L('订单编号')}}：</text>
                    <div class="orderNo"><div>{{allData.orderSn}}</div><div class="copytextbut" @click="copyStr(allData.orderSn)">复制</div></div>
                ...
                <block v-if="orderLogs && orderLogs.length > 0">
                    <view class="order_des_pre" v-for="(item,index) in orderLogs" :key="index">
                        <text>{{item.orderStateLog == 10 ? '创建时间：' : item.orderStateLog == 20 ? '付款时间：' :
                            item.orderStateLog == 30 ? '发货时间：' : item.orderStateLog == 40 ? '完成时间：' :item.orderStateLog == 102?'定金支付时间：':'取消时间：'}}</text>
                ...
                <block v-if="productPromiseCalendars&&productPromiseCalendars.length>1&&!isGift">
                    <view class="order_des_pre" >
                        ...
                        <block>
                            <text>
                                <view style="display:block" v-for="(item, index) in productPromiseCalendars" :key="index" >
        ...
        <view class="order_det_bottom"
            ...
            <!-- 待付款 -->
            <block v-if="allData.orderState == 10">
                <view class="edit_address_btn" @click="editAddress" v-if="false">{{$L('修改地址')}}</view>
                <view class="cancel_order fontScaleIgnore" v-if='!$isTransferPay(allData.paymentCode)' @click="cancelPopup()">{{$L('取消订单')}}</view>
                <template
                    ...
                    <view v-show="ladderGroupDisplayStatus" class="go_pay fontScaleIgnore" @click="goPay">{{$L('立即支付')}}</view>
                </template>
            </block>
            <!-- 待发货 -->
            <block v-if="allData.orderState == 20">
                <view v-if="false" class="edit_address_btn" @click="editAddress">{{$L('修改地址')}}</view>
                <!-- 一起买 拼团成功 未发货 订单可以联系退款 --> 
                <view  v-if="!(allData.orderType==106)" class="contact_customer_btn" @click="contactCustomerRefund">{{$L('联系客服退款')}}</view>
                <!-- 一起买 拼团中 未发货 订单可以取消订单 -->
                <view class="cancel_order fontScaleIgnore" v-if='allData.orderType==106 && allData.promotionStatus==0' @click="cancelPopup()">{{$L('取消订单')}}</view>
            </block>
            <!-- 待收货 -->
            <block v-if="allData.orderState == 30">
                <!-- 订单详情没有发票信息，此时说明下单的时候没有选择发票，此时展示补开发票的按钮 -->
                <!-- 经陈sir确认，下单金额为0时，不显示开发票和补开发票 2022/07/29 -->
                <view v-if="false && !!!allData.invoice && allData.actualPayment != 0" class="cancel_order fontScaleIgnore" @click="addInvoice()">补开发票</view>
                <view class="cancel_order fontScaleIgnore" @click="lookLogistics()">{{$L('查看物流')}}</view>
                <view v-if="false && !isGift" class="go_pay fontScaleIgnore" @click="orderAgain()">{{$L('再次购买')}}</view> 
                <view class="confirm_receipt fontScaleIgnore" @click="confirmReceipt()">{{$L('确认收货')}}</view>
            </block>
            <!-- 待评价 -->
            <block v-if="allData.orderState == 40">
                <!-- 订单详情没有发票信息，此时说明下单的时候没有选择发票，此时展示补开发票的按钮 -->
                <view v-if="false && !!!allData.invoice && allData.actualPayment != 0" class="cancel_order fontScaleIgnore" @click="addInvoice()">补开发票</view>
                <!-- 订单详情，有发票的相关信息，并且已经开出了发票，并且没有换开过一次。此时展示换开发票的按钮 -->
                <view v-if="false && showReInvoiceBtn" class="cancel_order fontScaleIgnore" @click="resInvoice()">换开发票</view>
                <view class="cancel_order fontScaleIgnore" v-if="showViewInvoiceBtn" @click="viewInvoice()">查看发票</view>
                <view class="cancel_order fontScaleIgnore" @click="lookLogistics()">{{$L('查看物流')}}</view>
                <view v-if="false && !isGift" class="go_pay fontScaleIgnore" @click="orderAgain()">{{$L('再次购买')}}</view> 
                <!-- <view class="go_pay fontScaleIgnore" @click="goEvaluate()" v-if="allData.evaluateState!=3">{{$L('评价')}}</view> --><!--2022/8/22屏蔽评价功能-->
            </block>
            <!-- 订单取消 -->
            <block v-if="allData.orderState == 0||allData.orderState == 50">
                <view class="edit_address_btn fontScaleIgnore" @click="delOrder()">{{$L('删除订单')}}</view>
                <view v-if="false && !isGift" class="go_pay fontScaleIgnore" @click="orderAgain()">{{$L('再次购买')}}</view> 
            </block>
        </view>
        <!-- 详情底部操作按钮 end-->
    ...
    methods: {
        ...
        addInvoice(){
            this.$Router.push({
                ...
                query: {
                    applyType: 'add',
                    orderSn: this.allData.orderSn, //订单编号
        ...
        resInvoice(){
            ...
            if (this.allData.invoice && this.allData.invoice.invoiceContent){
                invoiceContent = this.allData.invoice.invoiceContent
            }
            // 如果需要换开发票，此时跳转到我的发票页面此时需要显示和当前订单一致的发票信息，
            this.$setStorageSync('choosedInvoice', {titleId: this.allData.invoice.invoiceId, name: this.allData.invoice.invoiceTitle}); //将选择的发票存入缓存 为了订单详情页面和我的发票页面显示的发票一致, 同时要更新字段 因为订单这里的字段和发票的字段不一致
        ...
        contactCustomerRefund(){
            this.gotoCustomerService(this.allData, 'detail');
        },
        ...
        goPay() {
            //阶梯团 付定金  付尾款、
            let storeInfo = this.allData.childOrdersVOS[0];
            let goodsInfo = storeInfo.orderProductListVOList[0];
            // 整合商品相关的参数
            let products = [
                {
            ...
            if (this.allData.orderType == 105 && this.allData.orderSubState == 102 && this.allData.ladderGroupDetailInfo.depositRemainTime == 0) {
                ...
                this.$Router.push({path:'/pages/order/pay',query:{amount: this.allData.orderAmount, orderSn: this.orderSn,paySn:this.allData.paySn,payMethodType:'orderDetail'}})
            }
        },
        //修改地址
        editAddress() {
...
<style lang='scss'>
    ...
    .container {
        ...
        .order_det_bottom {
            ...
            height: calc(var(--safe-area-inset-bottom) + 90rpx);
            background: #FFFFFF;
