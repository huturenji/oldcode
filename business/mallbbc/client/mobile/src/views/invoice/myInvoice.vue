    ...
    <view class="container">
        ...
        <view v-if="is_need == true">
            ...
            <view class="invoice_content_tip">
                {{isGoodsDetail?$L('发票内容将显示商品详情名称与价格信息，发票金额为实际支付金额，不含优惠等扣减金额。'):$L('发票内容将显示本单商品所属类别及价格信息，发票金额为实际支付金额，不含优惠等扣减金额。')}}
            </view>
            ...
            <view class="invoice_history">
                ...
                <view @click="choosedInvoiceFun" class="choosed_invoice_item">
                    ...
                    <img :src="imgUrl + 'common/icon/icon_common_rightarrow.svg' " alt="">
                </view>
            </view>
...
export default {
    ...
    mounted(){
        ...
        this.needInvoice = eval(this.$Route.query.needInvoice) || false;//url传递boolean值会被转字符串
            ...
            if (this.is_need) { //需要发票
                ...
                if (!!flag){ //说明收票人信息变更成功
                    if (this.isConfirmOrderInvoice){ //下单选择发票
                        this.$setStorageSync('is_need_invoice', this.is_need); //是否需要开发票存入缓存
                        this.$setStorageSync('invoice_info', this.choosedInvoice); //抬头信息存入缓存
                        this.$setStorageSync('invoice_content', this.invoiceContent); //发票内容存入缓存
                        this.$Router.back(1);
                ...
                this.$setStorageSync('is_need_invoice', this.is_need);
                this.$Router.back(1)
            }
        },    
