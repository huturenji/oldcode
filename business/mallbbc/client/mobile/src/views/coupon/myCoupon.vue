...
<template>
    <view class="my_coupon"  ref="contentDom">
        <view class="my_coupon_nav" ref="navDom">
            ...
            >
                <template v-slot:content="{navItem}">
                    <view class="item_container flex_row_center_center" :class="{activeTab:tabCurrentIndex==navItem.index}">
                        <view class="name">{{ navItem.item.text }}</view>
                        <view class="number_box">[<text class="num-font number">{{navItem.item.num}}</text>]</view>
                    </view>
                </template>
        ...
        <view class="helpTips flex_row_center_center" @click="showHelpTips" v-if="showHelpTipsButt && tabCurrentIndex==0" :class="{backgroundfff: !filterFlag}">{{$L('使用帮助')}}</view>
            ...
            <swiper duration="300" @change="changeTab" :current="tabCurrentIndex" class="swiper-box">
                <swiper-item  class="tab-content" v-for="(tabItem,tabIndex) in navList" :key="tabIndex">
                    <scroll-view class="list-scroll-content" scroll-y @scrolltolower="load" :lower-threshold="150">
                        <view class="my_coupon_list" v-if="tabItem.couponList.length&&tabItem.loaded" :ref="`couponListDom_${tabIndex}`">
                            <template v-if="tabIndex==0">
                                <couponItem :key="index" :couponItem="item" v-for="(item,index) in tabItem.couponList"></couponItem>
                            </template>
                        ...
                        <view class="no_data" v-if="tabItem.isShow && tabItem.loaded">
                            ...
                            <text v-if="tabIndex==0">{{$L('暂无优惠券，去领券中心看看吧')}}~</text>
                            <text v-else-if="tabIndex==1">{{$L('暂无运费券')}}~</text>
        ...
        <view class="fixed_bottom flex_row_center_center"  ref="fixedBottomDom">
...
import {fitFontSize,isNotEmpty,filterFlag} from '@/utils/common.js'
...
export default {
    ...
    watch: {
        $route(to) {
            if(to.path == "/pages/coupon/myCoupon") {
    ...
    methods: {
                ...
                if (res.state == 200) {
                    ...
                    if (!navItem.showNoMoreDataTips){ //变为true了之后就会一直显示tips，避免多次计算
                        ...
                        if (!navItem.isShow){ //有优惠券数据才计算对应高度
                            fixedBottomDomH = this.$refs.fixedBottomDom.$el.offsetHeight;
                            couponListHeight = this.$refs[`couponListDom_${index}`][0].$el.offsetHeight;
                        }
        ...
        handleNav({index:type}) {
            ...
            if (this.navList[this.tabCurrentIndex].loadingState !== 'first_loading') {
                // 金额字号自适应
                this.$nextTick(() => {
                    let fontDom = document.getElementsByClassName('fitFont');
                    fitFontSize(fontDom);
        ...
        getHelpTips(){
            let param={};
            param.data = {};
            param.data.names = 'coupon_user_help';
...
<style lang="scss">
    ...
    .fixed_bottom{
        ...
        .toCoupunCenter{
            ...
            view {
                padding: 0 24rpx;
                background:url('@/static/shared/common/icon/btn_common_rightarrow_red.svg') right center no-repeat;
    ...
    .helpTips_box{
        ...
        .helpTips_title{
            ...
            height: 64rpx;
            margin-top: -20rpx;
            background: url("@/static/shared/coupon/icon_yhq_help.png") no-repeat;
