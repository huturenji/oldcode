    ...
    <view class="my_redpacket"  ref="contentDom">
        <view class="fixedTop" ref="fixedTopDom">
            <view class="total_amout flex_column_between_center" ref="topTotalDom">
                ...
                <view class="title">{{$L('可用余额 [元]')}}</view>
        ...
        <view class="fixed_bottom" @click="goRedpacketHistory()" ref="fixedBottomDom"><view class="redpacket_history">{{$L('历史红包')}}</view></view>
    </view>
</template>
    ...
    export default {
        ...
        data() {
            return {
                ...
                showNoMoreDataTips:false //数据加载完是否显示’数据加载完毕~‘。超过一屏才显示
            };
        },
        computed: {
                ...
                goodsHandler.getMyRedpacketList(param).then(async res => {
                    if (res.state == 200) {
                        ...
                        if (!this.showNoMoreDataTips){ //变为true了之后就会一直显示tips，避免多次计算
                            ...
                            let redpacketListHeight=0,fixedBottomDomH=0, fixedTopDomH=0;
                            if (this.redpacketList && this.redpacketList.length>0){ //有优惠券数据才计算对应高度
                                ...
                                fixedTopDomH = this.$refs.fixedTopDom.$el.offsetHeight;
                            }
                            this.showNoMoreDataTips = redpacketListHeight > (contentDomH  - fixedBottomDomH - fixedTopDomH)?true:false;
                        }
...
<style lang="scss">
    ...
    .my_redpacket_content{
        ...
        overflow: auto;
        padding-bottom: calc(88rpx + var(--safe-area-inset-bottom));
        padding-top: 156rpx;
    }
    .my_redpacket {
        width: 750rpx;
        ...
        .total_amout {
            ...
            .rules {
                ...
                width: 136rpx;
                height: 52rpx;
                background:url('@/static/shared/redpacket/icon_cj_dzp_guize@2x.png') center center/100% 100% no-repeat;
            }
        }
        .my_redpacket_list {
            display: flex;
            ...
            .my_redpacket_con {
                ...
                margin-top: 32rpx;
                position: relative;
                background:url('@/static/shared/redpacket/bg_wode_hongbao.png') center center/100% 100% no-repeat;
                .left {
                    width: 232rpx;
                    padding: 0 0 0 12rpx;
                    height: 100%;
