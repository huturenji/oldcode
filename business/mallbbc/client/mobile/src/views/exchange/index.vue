...
<template>
  <view class="exchange-wrapper">
      ...
      <uni-popup ref="receive_result_popup" type="bottom">
        <view class="result-wrapper">
          <view class="popup-title">兑换结果</view>
          <view class="success" v-if="successCount>0">
            <img class="success_icon" src="@/static/shared/gift/icon_setp_sel2.svg" />
          ...
          <scroll-view class="success-content" v-if="successList.length>0" :scroll-top="scrollTop" scroll-y="true">
            ...
            <view class="redpacket" v-for="(item,index) in successList.filter(i=>i.promotionType==404)" :key="index">
              ...
              <view class="right">
                ...
                <view class="redpacket-date" v-if="item.effectiveStart && item.effectiveEnd">{{`${item.effectiveStart.substring(0,10)}～${item.effectiveEnd.substring(0,10)}`}}</view>
                <view class="exist-number">x<text class="number">{{item.num}}</text></view>
              </view>
            </view>
          </scroll-view>
          ...
          <view class="fail" v-if="failCount>0">
            <img class="fail_icon" src="@/static/shared/gift/btn_common_close _red.svg" />
...
export default {
    ...
    methods:{
        ...
        exchange(){
            ...
            let param = {};
            param.url = 'v3/promotion/front/activity/listReceiveByPwd';
            param.data = {
                promotionCouponVOList
            };
            param.method = 'POST';
            param.header = {
    ...
    .result-wrapper {
      ...
      .success-content {
        ...
        .coupon {
          width: 650rpx;
          height: 190rpx;
          background-image: url('@/static/shared/coupon/bg_exchange_youhuiquan.png');
            ...
            .use-rule {
              ...
              .rule-title {
                ...
                .downtriangle {
                  width: 24rpx;
                  height: 24rpx;
                  background-image: url('@/static/shared/common/icon/btn_common_downtriangle2@2x.png');
                  background-repeat: no-repeat;
        ...
        .redpacket {
          width: 650rpx;
          height: 190rpx;
          background-image: url('@/static/shared/redpacket/bg_wode_hongbao.png');
          background-repeat: no-repeat;
          background-size: 100% 100%;
          display: flex;
          margin: 0 auto 16rpx;
