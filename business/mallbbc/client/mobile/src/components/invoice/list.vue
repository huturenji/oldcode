...
<view class="invoice_list" :class="!loading && invoiceList.length == 0?'invoice_list invoice_listempty':'invoice_list'">
        ...
        <view v-else class="container">
            <view v-if="!loading && invoiceList.length > 0" class="invoice_wrap">
                <view v-for="(item, index) in invoiceList" :key="index" class="invoice_wrap_item" >
                    <swiper-action
                        @cellOpen="followId = item.titleId"
                        :cellShow="followId === item.titleId"
                    >
                        <view @click="chooseInvoice(item)" class="invoice_wrap_box" :class="{InvoiceShowChecked: showCheck}">
                            <view v-if="showCheck" class="invoice_choose">
                                ...
                                <img v-else :src="imgUrl + 'common/icon/btn_common_radio_nor.svg' " > 
                            </view>
...
import invoiceHandler from '@/components/invoice/handler';
    ...
    methods: {
        ...
        async initImportInvoiceUrl(){
            ...
            try {
                ...
                if (!!channelOptions.companyInvoiceTitleUrl){
                    this.importInvoiceUrl = channelOptions.companyInvoiceTitleUrl;
                } 
            } catch (error) {
                console.log(error);
            }
        ...
        checkDelete(deleteId){
            let choosedInvoice = this.$getStorageSync('choosedInvoice'); //取缓存中的存入的选择的发票
            let invoiceInfo = this.$getStorageSync('invoice_info'); //取缓存中的存入的选择的发票
            if (deleteId == choosedInvoice.titleId || deleteId == invoiceInfo.titleId ){
                this.$removeStorageSync('choosedInvoice');
                this.$removeStorageSync('invoice_info');
            }
        },
...
<style lang="scss">
.invoice_list{
    height: calc(100vh - var(--titleBarFillHeight, 0px));
    display: flex;
    ...
    .container_empty{
        ...
        .empty{
            ...
            .img{
                width: 256rpx;
                height: 256rpx;
                background: var(--invoiceEmptyIng);
