...
<script>
	...
	export default {
		...
		computed: {
			stars() {
				...
				for (let i = 0; i < this.max; i++) {
					...
					} else if (ceilValue - 1 === i&&this.allowHalf==true) {
						starList.push({
		...
		created() {
			...
			this.textSync=this.text
			this.evaItemSync=this.evaItem
			this._rateBoxLeft = 0
			this._oldValue = null
		},
		mounted() {
