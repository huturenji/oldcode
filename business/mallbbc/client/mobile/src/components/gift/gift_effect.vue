<template>
    ...
    <view class="gift_effect">
        <img id="effect" class="effect" :class="{fadeout:fadeoutOption}" @load="setFadeout" :src="imgSrc"/>
...
export default {
    data() {
        return {
            duration:2000,//动画总时长 单位ms
            fadeoutDuration:500,//淡出时长 单位ms
    ...
    methods:{
        ...
        */
        init(){
            let imgW = 750;
            let imgH = 1624;
            let sceneW = document.getElementsByClassName('gift_effect')[0].clientWidth;
            let sceneH = document.getElementsByClassName('gift_effect')[0].clientHeight;
            var giftDom = document.getElementById("effect"); 
            if (sceneW/sceneH > imgW/imgH){
                giftDom.setAttribute("style","width:100%;");
            } else {
                giftDom.setAttribute("style","height:100%;");
            }
        },
        /**
         * 设置淡出动画
