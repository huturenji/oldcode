...
<template>
    <view class="swiper-cell">
        ...
        <view class="swiper-cell-right" ref="swiperCellRight" :style="cellRStyle">
            <slot name="right" />
        </view>
    </view>
</template>
...
export default {
    ...
    mounted() {
        ...
        // 监听全局的mouseup事件
        /* #ifdef H5 */
        window.addEventListener('mouseup', this.goodsMouseUp)
        window.addEventListener('touchend', this.goodsMouseUp)
        /* #endif */
    },
    onUnload() {
        /* #ifdef H5 */
        window.removeEventListener('mouseup', this.goodsMouseUp)
        window.removeEventListener('touchend', this.goodsMouseUp)
        /* #endif */
    },
    methods: {
        lock(){
