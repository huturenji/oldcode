...
<template name="goodsItemH">
    <view class="my_coupon_pre">
        <view class="coupon_pre_top">
            ...
            <view class="coupon_pre_right">
                <view v-if="couponItem.useState == '1'" class="coupon_right haveNotUse">
                    <view>{{$L('已领取')}}</view>
                    <view @click="goFeightCoupon(couponItem)">{{$L('去使用')}}</view>
                </view>
        ...
        <view class="coupon_type fontScaleIgnore" :class="{grey_type:couponItem.useState != 1}">{{$L('运费券')}}</view>
    </view>
</template>
...
.my_coupon_pre {
    ...
    .coupon_pre_top {
        ...
        .kacao{
            ...
            &.kacao1{
                background-image: url('@/static/shared/coupon/bg_yhq_xuxian4.png');
            }
            &.kacao3{
