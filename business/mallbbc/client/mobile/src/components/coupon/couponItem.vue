    ...
    <view class="my_coupon_pre">
        <view class="coupon_pre_top">
            ...
            <view class="coupon_pre_cen" :class="{grey:couponItem.useState != '1'}">
                ...
                <view class="coupon_pre_rules" @click="descriptionOpen(couponItem)">
                    <text>使用规则</text>
                    <image :src="couponItem.isOpen ? imgUrl + 'common/icon/uptriangle2@2x.png' : imgUrl + 'common/icon/btn_common_downtriangle2@2x.png'" :style="{opacity:couponItem.useState != '1'?'0.4':'1'}"
            ...
            <view class="coupon_pre_right">
                <view v-if="couponItem.useState == '1'" class="coupon_right haveNotUse">
                    <view>{{$L('已领取')}}</view>
                    <view @click="goGoodsList(couponItem)">{{$L('去使用')}}</view>
                </view>
        ...
        <view class="coupon_rules" v-if="couponItem.isOpen == true" :class="{grey:couponItem.useState != '1'}">
            <view>{{$L('优惠券类型：')}}<text class="coupon_type_text">{{$L('【')}}{{couponItem.storeId==0?'平台优惠券':'店铺优惠券'}}{{$L('】')}}</text></view>
...
.my_coupon_pre {
    ...
    .coupon_pre_top {
        ...
        .coupon_pre_right {
            ...
            .haveUse{
                view{
                    background: url('@/static/shared/coupon/icon_yhq_yishiyong.svg') center/100% 100% no-repeat;
                }
            }
            .haveExpired{
                view{
                    background: url('@/static/shared/coupon/icon_yhq_yiguoqi.svg') center/100% 100% no-repeat;
                }
            }
            .invalid{
                view{
                    background: url('@/static/shared/coupon/icon_common_yishixiao.svg') center/100% 100% no-repeat;
                }
            }
        }
        .kacao{
            ...
            &.kacao3{
                background-image: url('@/static/shared/coupon/bg_yhq_xuxian2.2.png');
            }
            &.kacao3_haveExpired{
                background-image: url('@/static/shared/coupon/bg_yhq_xuxian2.png');
            }
        }
    }
