...
export default {
    ...
    methods: {
        ...
        _handleHtml(append) {
            if (typeof this.html != 'string') { this.html = this._Dom2Str(this.html.nodes || this.html); }
            if (!append) {
            ...
            // 处理 rpx
            if (this.html.includes('rpx'))
            { this.html = this.html.replace(/[0-9.]+\s*rpx/g, $ => parseFloat($) * rpx + 'px'); }
            return this.html;
        },
        // #endif
        setContent(append = undefined) {
            this.document = document.getElementById('rtf' + this._uid);
            // #ifdef APP-PLUS-NVUE
            if (!this.html)
            { return this.height = 1; }
            if (append)
            { this.$refs.web.evalJs("var b=document.createElement('div');b.innerHTML='" + this.html.replace(/'/g, "\\'") +
                ...
                this.html =
						...
						this.domain + '"><div id="parser"' + (this.selectable ? '>' : ' style="user-select:none">') + this._handleHtml().replace(/\n/g, '\\n') +
                ...
                this.$refs.web.evalJs("document.write('" + this.html.replace(/'/g, "\\'") + "');document.close()");
            }
            ...
            div.innerHTML = this._handleHtml(append);
            ...
            { return this.nodes = []; }
                ...
                let parser = new Parser(this.html, this);
                // 缓存读取
                if (this.useCache) {
                    var hashVal = hash(this.html);
                    if (cache[hashVal])
                    { nodes = cache[hashVal]; }
                    else {
                        nodes = parser.parse();
            ...
            } else if (Object.prototype.toString.call(this.html) == '[object Array]') {
                // 非本插件产生的 array 需要进行一些转换
                if (this.html.length && this.html[0].PoweredBy != 'Parser') {
                    let parser = new Parser(this.html, this);
                    (function f(ns) {
                ...
                nodes = this.html;
            } else if (typeof this.html == 'object' && this.html.nodes) {
                nodes = this.html.nodes;
            } else
            { return console.warn('错误的 html 类型：' + typeof this.html); }
