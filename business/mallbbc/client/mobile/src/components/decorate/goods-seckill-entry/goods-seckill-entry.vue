<template>
<view class="seckillAll" v-margin="decoItem">
    <view class="seckillEntry" @click="goDetail(decoItem.props.linkdata)" v-background.self="setBackground" :style="{borderRadius:decoItem.props.borderRadius*2+'rpx'}">
        <view class="seckillEntryTop">
            <view class="seckillEntryTopLeft">
                <view class="leftTitle">{{decoItem.props.leftText.title}}
                    <image mode="heightFix" v-if="!decoItem.props.leftText.title" :src="decoItem.props.leftText.title?'':decoItem.props.leftText.img"/>
                </view>
                <view class="distanceTime" v-if="(decoItem.props.seckillType=='vop' && (judgeState==1 || judgeState==2)) && !is_show_empty">
                    ...
                    <view class="right">
                        ...
                        <view v-else-if="judgeState==1" class="willSatrt fontScaleIgnore">{{$L('即将开始')}}</view>
                    </view>
                </view>
            </view>
            
            <view class="seckillEntryTopRight">{{decoItem.props.rightText.title}}
                <image mode="heightFix" :src="decoItem.props.rightText.title?'':decoItem.props.rightText.img" v-if="!decoItem.props.rightText.title"/>
        ...
        <view class="seckillEntryBottom" >
            <view class="seckillEntryGoods flex_row_between_center" v-if="!!goods_list && loadGoodsList">
                <thumbSeckillIndex v-for="(item,index) in goods_list" :key="index" :goods_info="item" :seckillVopType="decoItem.props.seckillType=='vop'?true:false" class="goodsItem">
                </thumbSeckillIndex>
...
<script>
import thumbSeckillIndex from "@/components/goods/thumb/thumb-seckill-index.vue";
...
export default {
    ...
    mounted(){
        this.loadingImage = window.themeObj.config.loadingImage
    },
    components: {
        thumbSeckillIndex
    },
...
<style lang="less">
        ...
        .seckillEntryTop{
            ...
            .seckillEntryTopLeft{
                ...
                .distanceTime{
                    ...
                    align-items: center;
                    justify-content: center;
                    background: url('@/static/shared/seckill/bg_xs_time.png') center no-repeat;
    ...
    .first-animated-background {
        ...
        animation-direction: alternate;
        animation-timing-function: ease;
        background: var(--skeletonBg) !important;
        position: relative;
    }
