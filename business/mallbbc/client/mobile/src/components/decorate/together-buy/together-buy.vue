<template>
    <view class='tab-row' v-if="nav_list.length > 0" v-margin="decoItem">
        <!-- 分类区域 -->
        <view class="right-cate">
            ...
            <baseTabs
                ...
                :lineColor="`url(${imgUrl + themeObj.config.categoryTabActiveLine}) 100% 100% no-repeat`"
                lineWidth="46rpx"
                lineHeight="22rpx"
            ...
            <view class="right-cate2" v-if="nav_list[currIndex1].sessionList.length > 0 && nav_list[currIndex1].sessionList[0].title">
                ...
                >
                    <template v-slot:content="{navItem}">
                        <view class="right-cate-con-box" :style="cateStyle" :class="{ active: currIndex2 == navItem.index }">
                            <view class="twoRow flex_column_center_center">
                                <view>{{ navItem.item.title }}</view>
                                <view class="distanceTime" v-if="navItem.item.frontState === 2">
                                    <text class="text1" v-if="navItem.item.distanceTime >= 0">距结束</text>
                                    <text class="num-font text2" v-if="navItem.item.distanceTime >= 0">
                                        {{ leftTime(navItem.item.distanceTime).hour }}:{{ leftTime(navItem.item.distanceTime).min }}:{{ leftTime(navItem.item.distanceTime).second }}
                                    </text>
                                    <text class="text1" v-else>已结束</text>
                                </view>
                                <view class="distanceTime" v-else>
                                    <text class="text1">{{ cateShowEnum[navItem.item.frontStateValue] }}</text>
                                </view>
                            </view>
                        </view>
                    </template>
        ...
        <view class="right-content">
            ...
            <template v-if="nav_list[currIndex1].show_style === 't_row'">
                <thumbAllBuy class="goods_item" v-for="item in goodsList" :key="item.sku" :goodsItem="item" :otherParams="nav_list[currIndex1].props" />
            </template>
            <!-- 大图 -->
            <template v-if="nav_list[currIndex1].show_style === 't_big'">
                <thumbAllBuyV2 class="goods_item" v-for="item in goodsList" :key="item.sku" :goodsItem="item" :otherParams="nav_list[currIndex1].props" />
            </template>
            <!-- 一行两个 -->
            <template v-if="nav_list[currIndex1].show_style === 't_small'">
                <view class="towRow">
                    <view class="leftRow">
                        <thumbAllBuyV class="goods_item" v-for="item in twoGoodsList(0)" :key="item.sku" :goodsItem="item" :otherParams="nav_list[currIndex1].props" />
                    </view>
                    <view class="rightRow">
                        <thumbAllBuyV class="goods_item" v-for="item in twoGoodsList(1)" :key="item.sku" :goodsItem="item" :otherParams="nav_list[currIndex1].props" />
                    </view>
                </view>
            </template>
            ...
            <!-- 商品加载骨架图 -->
            <thumbSkeleton
                v-if="!nav_list[currIndex1].sessionList[this.currIndex2].isDone"
...
<script>
import { throttle, skipTo, isNotEmpty } from '@/utils/common.js'
...
import thumbAllBuy from '@/components/goods/thumb/thumb-all-buy.vue'; // 一起买横排
import thumbAllBuyV from '@/components/goods/thumb/thumb-all-buy-v.vue'; // 一起买一行两个
import thumbAllBuyV2 from '@/components/goods/thumb/thumb-all-buy-v2.vue'; // 一起买大图
import loadingState from "@/components/loading/loading.vue";
import customTabs from "@/components/tab/custom";
import baseTabs from "@/components/tab/base";
