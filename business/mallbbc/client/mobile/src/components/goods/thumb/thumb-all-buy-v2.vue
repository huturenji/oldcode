  ...
  <view class="goods_item" @click="goGoodsDetail(goodsItem)" v-if="goodsItem">
    ...
    <view class="price_box">
        <view class="price_bg flex_column_between_start" :class="{opacity_change:goodsItem.state==3,hadStarted:goodsItem.state!=1}">
            ...
            <!-- <view class="countdownText" v-if="countTime">距结束</view> -->
        </view>
    </view>
    <view class="activity_end_icon" v-if="goodsItem.state==3"></view>
    <!-- <view class="time-left" v-if="countTime">
        <view class="time-num">{{ leftTime.hour }}</view>
        <view class="time-icon"></view>
        <view class="time-num">{{ leftTime.min }}</view>
        <view class="time-icon"></view>
        <view class="time-num">{{ leftTime.second }}</view>
    </view> -->
  </view>
</template>
...
export default {
    ...
    computed: {
        ...
        // 倒计时
        leftTime() {
            let hours = parseInt(this.countTime / 60 / 60 % 24)
            let minutes = parseInt(this.countTime / 60 % 60)
            let seconds = parseInt(this.countTime % 60)
    ...
    mounted(){
        let that = this;
        setTimeout(()=>{
            that.imgHeight = (parseInt(that.$refs.goodsImg.$el.clientWidth * 0.81 * 2)) || 340
        },10)
    },
    methods: {
        //进入商品详情页
...
.goods_item {
    ...
    .price_bg{
        ...
        background: var(--togetherBuyPriceBgBig);
        ...
        &.hadStarted {
            background: var(--togetherBuyPriceBgBig1);
            background-size: auto 132rpx;
            .remind {
