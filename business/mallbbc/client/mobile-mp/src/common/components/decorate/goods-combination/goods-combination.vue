...
<template name="goodsCombination">
    <view class="goodsCombination" :style="[styles]">
        <view class="goods_container flex_row_start_start" ref="container">
            <view v-for="(item,index) in decoItem.data" :key="index" class="goods_item">
                <view v-for="(item1,index1) in item.data.info" :key="index1" @click="goDetail(item1)" class="goods_item_con">
                    ...
                    <view class="old_price num-font" v-if="hadRequested && item.priceStyle.priceStyleType==1 && showPrice(item1)" :style="{left:item.priceStyle.originPositionX+'%',top:item.priceStyle.originPositionY+'%',fontSize:item.priceStyle.originPriceSize?item.priceStyle.originPriceSize*2+'rpx':'24rpx',color:item.priceStyle.originPriceColor,transform:item.priceStyle.originPositionX !== '' ? 'none' : 'translateX(-50%)'}">
                    ...
                    <view v-if="hadRequested && !showPrice(item1)" class="price"
                        :style="{
                            fontSize: '24rpx',
...
<script>
import angleLabel from '@/common/components/angle-label/angle-label.vue'
...
export default {
        ...
        price(){
            return (goodsItem) => {
                ...
                if (this.hadRequested){
                    ...
                    if (index !== -1) {
                        price = {salePrice:this.priceData[index]['salePrice'],originalSalePrice:this.priceData[index]['salePrice']}
                    }
                }
                return price
            }
    ...
    created() {},
    mounted(){
        this.skuList = this.decoItem.data.filter(item => !!item.data.info[0]).map(item=>{
            return item.data.info[0].sku
        })
        this.getGoodsDetails(this.skuList)
    },
    methods: {
        goDetail({ sku, storeId }){
            try {
                this.$Router.push({
        ...
        .goods_item{
            ...
            .label_container{
                ...
                &.label_fire{
                    ...
                    letter-spacing: 4rpx;
                    background: url('https://bbnjstaticba-sinosun.oss-cn-hangzhou.aliyuncs.com/Ba04ZWJgWx/miniProgram/goods/icon_huobiao.svg') no-repeat;
                ...
                &.label_bookmark{
                    ...
                    writing-mode:vertical-lr;
                    letter-spacing: 8rpx;
                    background: url('https://bbnjstaticba-sinosun.oss-cn-hangzhou.aliyuncs.com/Ba04ZWJgWx/miniProgram/goods/icon_shuqianbiao.svg') no-repeat;
                    background-size: 100% auto;
                    color: #fff;
                }
            }
