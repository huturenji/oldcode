...
<template name="counter-timer">
    <view class="counter-timer" :style="[styles]">
        <view v-for="(item,index) in timeList" :key="index" class="counter_item" @click="goPage(item)">
            <image :src="item.bgImg.img" mode="widthFix" />
            <view
                :style="[{
                    fontWeight: item.fontStyle.isBold ? 'bold' : 'normal',
                    top: item.positionStyle.positionY + '%',
                    left: item.positionStyle.positionX + '%',
                    fontSize: item.fontStyle.size ? (item.fontStyle.size * 2 + 'rpx') : '24rpx',
                    color: item.fontStyle.color ? item.fontStyle.color : '#222'
                }]"
                class="num-font time"
            >
                {{ item.showTime }}
...
export default {
    ...
    mounted(){
        this.timeList = this.decoItem.data
