<template>
    <view class="daydayBuy" :style="[styles]">
        <!-- title区域 含背景图 -->
        ...
        <view class="goodsContent">
            ...
            <template v-if="show_style === 'd_row'">
                <view class="goods_item" v-for="item in goodsList" :key="item.sku">
                    <thumbDayBuyRow :goodsItem="item" />
                </view>
            </template>
            <!-- 大图 -->
            <template v-if="show_style === 'd_big'">
                <view class="goods_item" v-for="item in goodsList" :key="item.sku">
                    <thumbDayBuyBig :goodsItem="item" />
                </view>
            </template>
            <!-- 一行两个 -->
                ...
                <view class="goodsList">
                    <view class="goodsLeft">
                        <view class="goods_item" v-for="item in twoGoodsList(0)" :key="item.sku">
                            <thumbDayBuySmall :goodsItem="item" />
                        </view>
                    </view>
            ...
            <template v-if="show_style === 'd_smallRow'">
                <!-- 前两个一行两个 -->
                <view class="goodsList">
                    <view class="goodsLeft goods_item">
                        <thumbDayBuySmall v-for="item in sliceGoods(0)" :key="item.sku" :goodsItem="item" />
                    </view>
                    <view class="goodsRight goods_item">
                        <thumbDayBuySmall v-for="item in sliceGoods(1)" :key="item.sku" :goodsItem="item" />
                    </view>
                </view>
                <!-- 后面的横排 -->
                <view class="goodsList">
                    <view class="goods_item" v-for="item in sliceGoods(2)" :key="item.sku">
                        <thumbDayBuyRow :goodsItem="item" />
                    </view>
                </view>
            </template>
...
import thumbDayBuyRow from '@/common/components/thumb/thumb-day-buy-row.vue'; // 一起买横排
import thumbDayBuySmall from '@/common/components/thumb/thumb-day-buy-small.vue'; // 一起买一行两个
import thumbDayBuyBig from '@/common/components/thumb/thumb-day-buy-big.vue'; // 一起买大图
...
.daydayBuy {
    ...
    .empty_page {
        ...
        align-items: center;
        padding-top: calc((100vh - 240rpx - 160px)*0.32 - 128rpx);
