    ...
    <view class="marquee_box" @touchstart='touchstartFn' @touchend='touchendFn'>
        <view class="" v-if="direction=='right'||direction=='left'">
            <view v-if="broadcastType=='text'" class="marquee_container" :style="'background:'+broadcastStylees.back_color">
                ...
                <view class="marquee_text_left" :class='[direction=="left"?"marquee_text_left":direction=="right"?"marquee_text_right":"",animation_paused?"animation_pausedcss":""]'
                 :style="[{'--marquee-width--': -(broadcastStylees.width_mal)+'px', '--speed--': broadcastStylees.time+'s','width': broadcastStylees.width_mal+'px'}]">
            ...
            <view v-if="broadcastType=='mould'" class="mould">
                <view class="" style="width: 100%;position: relative;overflow: hidden;" :style="'height:'+viewHeight+'rpx;'">
                    <view class="broadcastTopBtm dsf" :class="[direction=='left'?'broadcastDataTopBtmDatacss_let':direction=='right'?'broadcastDataTopBtmDatacss_rig':'',animation_paused?'animation_pausedcss':'']"
                     :style="'--scroll-width--:'+(-scrlloWidth)+'px;--scroll-speed2--:'+(scrlloWidth)/broadcastStyle.speed+'s;width:'+scrlloWidth*2+'px;'">
        ...
        <view v-if="direction=='top'||direction=='bottom'" class="broadcastTopBtm" :style="'font-size:'+broadcastStyle.font_size+'rpx;color:'+broadcastStyle.text_color+';background:'+broadcastStyle.back_color+';height:'+viewHeight+'rpx;--scroll-height--:'+(-broadcastTopBtmHeight/2)+'px;--scroll-speed--:'+(broadcastTopBtmHeight/2)/broadcastStyle.speed+'s;'">
...
export default {
    ...
    methods: {
        ...
        initial_let_rig() {
            ...
            for (let i=0;i<ititdata.length;i++){    
...
<style lang="scss" scoped>
    ...
    .broadcastDataTopBtmDatacss_let {
        ...
        animation-duration: var(--scroll-speed2--);
        animation-iteration-count: infinite;
        animation-timing-function: linear;
        position: absolute;
        left: 0;
