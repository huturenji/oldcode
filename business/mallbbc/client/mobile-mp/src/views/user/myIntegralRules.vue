...
<template>
    <view class="integral_rules">
        <view class="top">
            <view class="title flex_row_start_center">什么是云豆？</view>
        ...
        <view class="bottom">
            <view class="title flex_row_start_center">云豆使用常见问题</view>
...
import systemHandler from '@/views/components/system/handler';
    export default {
        ...
        methods: {
            // 获取积分抵扣比例
            getintegralConRatio(){
                systemHandler.getSettings({names: 'integral_conversion_ratio'})
                    .then(res => {
                        if (res.state == 200) {
...
.integral_rules {
    ...
    .title {
        ...
        &::before {
            ...
            height: 48rpx;
            margin-right: 16rpx;
            background: url("https://bbnjstaticba-sinosun.oss-cn-hangzhou.aliyuncs.com/Ba04ZWJgWx/miniProgram/yundou/icon_yundouguize.png") center/100% 100% no-repeat;
        }
    }
    .content {
