...
<template>
    <view class="my_coupon"  ref="contentDom">
        <view class="my_coupon_nav" ref="navDom">
            ...
            <!-- 二级分类 -->
            <view class="tab2 flex_row_between_center">
                <view class="item_container1 flex_row_center_center" @click="handleNavState(index)" v-for="(item,index) in navList[tabCurrentIndex1].stateList" :key="index" :class="{activeTab:navList[tabCurrentIndex1].currentIndex2==index}">
                    <text>{{ item.text }}</text>
                </view>
            </view>
        </view>
            ...
            <scroll-view class="list-scroll-content" scroll-y @scrolltolower="load" :lower-threshold="150" :scroll-top="scrollTop" @scroll="getScrollTop">
                    <view class="my_coupon_list" v-if="tabItem.couponList && tabItem.couponList.length && tabItem.loaded" :ref="`couponListDom_${tabCurrentIndex1}_${tabCurrentIndex2}`">
                        <view v-for="(item,index) in tabItem.couponList" :key="index">
                            <couponItem :couponItem="item" v-if="tabCurrentIndex1==0" @changeRulesState="changeRulesState($event,index)"></couponItem>
                ...
                <view class="no_data" v-if="tabItem.isShow && tabItem.loaded">
                    ...
                    <text v-if="tabCurrentIndex1==0">暂无优惠券，去领券中心看看吧~</text>
                    <text v-else-if="tabCurrentIndex1==1">暂无运费券~</text>
...
//     mapState
// } from 'vuex';
import {checkPaginationHasMore} from '@/utils/common.js'
...
export default {
    ...
    methods: {
        ...
        changeRulesState(data,index) {
            this.$set(this.navList[this.tabCurrentIndex1].stateList[this.tabCurrentIndex2].couponList[index],'isOpen',data)
        },
        comeBackMyCoupon(){
        ...
        getCouponList(index1,index2) {
            ...
            goodsHandler[this.requestApi](param).then(async res => {
                if (res.state == 200) {
                    ...
                    tabItem.loaded=true;
                    this.tabItem.hasMore = checkPaginationHasMore(result.pagination); //是否还有数据
                    if (tabItem.hasMore) {
                        tabItem.current++;
