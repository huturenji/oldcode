            ...
            <view class="right flex_column_between_start">
                ...
                <view class="goods-price flex_row_between_center">
                    <view class="left flex_row_start_end num-font">
                        ...
                        <text class="price_int">{{getPartNumber(goods_info.skuPrice,'int')}}</text>
                        <text class="price_decimal">{{getPartNumber(goods_info.skuPrice,'decimal')}}</text>
                    </view>
                </view>
            </view>
        </view>
...
export default {
    ...
    methods: {
        ...
        //进入店铺首页
        toShopDetail(goodsInfo){
            let {storeId,supplierType} = goodsInfo;
            this.$emit('gotoShop')
                
            //跳转店铺的中转页
            this.$Router.push({path:'/standard/store/transfer',query:{storeId, supplierType}})
        },
        // 点击商品
        changeSelectState() {
