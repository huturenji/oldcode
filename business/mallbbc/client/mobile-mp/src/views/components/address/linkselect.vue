<template>
<view class="wrapper1" v-if="isShowMask1">
                    ...
                    <view class="swiper-item">
                        <scroll-view class="scroll-view-item" scroll-y="true">
                            <view class="address_item flex_row_between_center" v-for="(item,index) in cityAreaArray[swiper_index]" :key="index" @click="address_item_click(swiper_index,index,item)" :style="{background:selectIndexArr[swiper_index] === index?bgGrey:bgWhite}">
                                {{item.regionName}}
                                <image v-if="selectIndexArr[swiper_index] === index" class="address_item_icon" :src="checkedImage"
                                    mode=""></image>
...
export default {
    data() {
        return {
            checkedImage: 'https://bbnjstaticba-sinosun.oss-cn-hangzhou.aliyuncs.com/Ba04ZWJgWx/miniProgram/common/icon/select_svg.svg',
            isShow: false,
            isShowMask: false,
            isShowMask1: false,
            dataList: [{ //顶部数据源
    ...
    methods: {
        ...
        getAddressListNew(){
            return new Promise((resolve) => {
                ...
                let param = {parentRegionCode:0};
                addressHandler.getAddressListNew(param).then(res => {
                    if (res.state == 200){
                        //为一级数据源赋值
        ...
        // isAutoSelect参数是用来处理 是用户点击的true，还是自动程序调用false。用来区分地址识别后自动反选
        address_item_click(swiper_index, index, item, isAutoSelect=false) {
            let param = {parentRegionCode:item.regionCode};
            return new Promise((resolve) =>{
                addressHandler.getAddressListNew(param).then(res => {
                    if (res.state == 200){
