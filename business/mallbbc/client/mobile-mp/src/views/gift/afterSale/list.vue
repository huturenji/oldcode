                        ...
                        <view class="store-item" v-for="(item, index) in tabItem.afsList" :key="index">
                            <view class="item-head">
                                <view class="store-info">
                                    <image class="img" :src="storeLogo" mode="widthFix" />
                            ...
                            <view class="item-body">
                                ...
                                <view class="detail-wrapper">
                                    ...
                                    <view>
                                        ...
                                        <view class="num">共{{ item.afsNum || 0 }}件</view>
                                    </view>
                                </view>
                            </view>
                            <view class="item-foot">
                                <view v-if="item.afsType === 2 && item.newOrderSn" class="btn exchange-btn"
                                    @click.stop="toExchangeOrderDetail(item.newOrderSn)">换货订单</view>
...
<script>
import { getAfsList } from "@/views/components/aftersale/handler";
import imgThumb from "@/common/components/thumb/imgThumb.vue";
import { afsNavList } from '@/common/lib/enum/gift.js';
...
export default {
    ...
    onShow() {
        this.orderSn = this.$Route.query.orderSn || null;
        this.reset();
        this.loadData();
    },
    methods: {
        initNavList() {
            ...
            let navList = JSON.parse(JSON.stringify(afsNavList))
            if (!sourceType) {
                uni.showToast({
                    icon: 'none',
                    title: '请标记页面来源'
            ...
            if (sourceType == 'orderDetail') {
                this.navList = navList
                // 普通订单过来的
            } else if (sourceType === 'giftDetail') {
                navList.shift();
                // 从鹅毛情订单过来的
                this.navList = navList;
            }
        ...
        loadData() {
            ...
            const params = {}
            params.afsType = this.navList[this.currentIndex].value;
            this.orderSn && (params.orderSn = this.orderSn);
            
            getAfsList(params).then(res => {
                if (res.state === 200) {
        ...
        toExchangeOrderDetail(orderSn) {
            this.$Router.push({ path: '/views/gift/afterSale/exchange', query: { orderSn } })
        },
        // 跳转到售后详情
        toAfsDetail({ afsSn, afsType }) {
            this.$Router.push({ path: '/views/gift/afterSale/detail', query: { afsSn, afsType } })
        }
