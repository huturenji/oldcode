...
<template>
    <view class="content">
        ...
        <view class="apply_refund_content">
            ...
            <!-- 单个商品 start -->
            <view class="refund_goods" v-else>
            ...
            <view class="refund_info">
                ...
                <block v-if="serviceType == '0' && cargoStatusCurrent == 1">
                    <view class="refund_amount refund_amount_edit">
                        <text class="refund_amout_title"> 退款金额  :￥</text>
                ...
                <block v-if="serviceType == '1'">
                    <view class="refund_amount">
                        ...
                        <view class="refund_amount_price">
                            <!-- 批量选择之后 -->
                            <text v-if="tosource == 1">￥{{validValue(refundInfos.maxReturnMoney)}}</text>
                            <!-- 单个商品 -->
                            <text v-else>￥{{validValue(refundDetail.maxReturnMoney)}}</text>
                        </view>
                    </view>
                    <view class="refund_amount_limit">
                         退款金额不可修改，最多  ￥{{tosource == 1 ? validValue(refundInfos.maxReturnMoney) : validValue(refundDetail.maxReturnMoney)}}
                        <text v-if="refundInfos.containsFee && tosource == 1"> (含运费  {{validValue(refundInfos.returnExpressFee)}} 元)  </text>
                        <text v-if="refundDetail.containsFee && tosource == 0"> (含运费  {{validValue(refundDetail.returnExpressFee)}} 元)  </text>
                        <text v-if="!refundDetail.containsFee"> (不包含运费)  </text>
                    </view>
                </block>
                ...
                <block v-if="(batchSelList.length == 1 && tosource == 1) || tosource == 0">
                    <view class="refund_number1" v-if="serviceType == '1' || serviceType == '2'|| serviceType == '4'">
                        <text class="refund_number1_title"> 申请件数  ：</text>
                ...
                <view class="refund_instructions">
                    ...
                    <input type="text" :value="refundInstructions" :placeholder="`请输入${serviceType == '1' ? '退款说明:':serviceType == '2'? '换货说明':serviceType == '4'? '维修说明':''}(选填)`" @input="refundDes"
                        maxlength="200" />
                </view>
            </view>
            <view class="upload_voucher">
                <view class="upload_voucher_title"> 上传凭证  </view>
                <view class="upload_voucher_con1">
                    <template  v-if="uploadFiles.length > 0">
                        <view class="upload_img" v-for="(item,index) in uploadFiles"
                            ...
                            <image src="https://bbnjstaticba-sinosun.oss-cn-hangzhou.aliyuncs.com/Ba04ZWJgWx/miniProgram/common/icon/close.svg" mode="" @click="delImg(item,index)"></image>
                        </view>
                    </template>
                    <view class="upload_voucher_con" @click="uploadVoucher" v-if="uploadFiles.length < 5">
                        <view class="upload_image">
                            <image src="https://bbnjstaticba-sinosun.oss-cn-hangzhou.aliyuncs.com/Ba04ZWJgWx/miniProgram/order/xiangji.png" mode=""></image>
                            <text> 添加图片  </text>
            ...
            <view class="refund_reason_des" v-if='orderProduct.showAddressEle'>
                ...
                <view class="address" v-if="pickWareType==enumType.pickUp" @click="operateAddress('take',pickWareAddress)">
            ...
            <view class="refund_reason_des" v-if='orderProduct.showAddressEle&&(serviceType=="2"||serviceType=="4")'>
                <view class="refund_reason">
                    ...
                    <view class="refund_reason_right" v-if='pickWareType==enumType.pickUp'>
                        ...
                        <switch  :checked="isDefaultAddress" color="#E82B29" style="transform:scale(0.7)" class="relative-left" @change="switchChange"/>
                    </view>
                </view>
                <!-- 如果是上门取件,用开关控制 如果是自行邮寄直接显示 -->
                <view class="address" v-if="isDefaultAddress==false||pickWareType==enumType.mail" @click="operateAddress('recept',receiveWareAddress)">
                    <view class="address-left">
                        <view class="address-left-top">
        ...
        <bottomPopup ref="cancelPopup" type="bottom" height="700rpx" :text="serviceType == '1' ? '退货原因' : serviceType == '2' ? '换货原因' : serviceType == '4'? '维修原因':''">
            <view class="cancel_popup">
                ...
                <view class="cancel_popup_btn">
                    <text class="confrim_btn" @click="confirmRefund()"> 确定  </text>
                </view>
            </view>
        </bottomPopup>
        <!-- 货物状态弹框 -->
        <bottomPopup ref="cargoStatusPopup" type="bottom" height="700rpx" text="货物状态">
            <view class="cancel_popup">
                ...
                <view class="cancel_popup_btn">
                    <text class="confrim_btn" @click="confirmCargoStatus()"> 确定  </text>
                </view>
            </view>
        </bottomPopup>
        <!-- 返回方式弹框 -->
        <bottomPopup ref="returnModePopup" type="bottom" height="700rpx" text="返回方式">
            <view class="cancel_popup">
                ...
                <view class="cancel_popup_btn">
                    <text class="confrim_btn" @click="closeReturnMode()"> 确定  </text>
                </view>
            </view>
        </bottomPopup>
        ...
        <uni-popup ref="afterTips" :mask-click="false">
            <view class="afterTips_box flex_column_center_center">
                ...
                <view class="close"><view @click="closeAfterTips" class="flex_row_center_center">我知道了</view></view>
            </view>
        </uni-popup>
    </view>
</template>
...
import { getOrderProductDetail, afsCheck, countReturnMoney, refundReasonList, getReturnMoney, submit, afsServiceReverseExpress } from "@/views/components/aftersale/handler";
import bottomPopup from '@/common/components/uni-popup/uni-popup-bottom.vue'
import { IMAGE_TYPES, setStorageSync, isEmpty } from '@/utils/common.js'
    ...
    methods: {
        ...
        setAfsCheckToLocalStorage(orderSn){
            afsCheck({orderSn}).then(res => {
                console.log('重新申请', orderSn, res.data);
        ...
        getOrderDetail() {
            let param = {};
            param.orderProductId = this.orderProductId; //订单货品id

            getOrderProductDetail(param).then(async res => {
                if (res.state == 200) {
                    let result = res.data;
                    // 获取可售后性
        ...
        getRefundDetail() {
            let param = {};
            param.orderSn = this.orderProduct.orderSn;
            //单个商品
            let orderProductInfos = '';
            orderProductInfos = this.orderProduct.orderProductId + '-' + this.serviceNum;
            param.orderProductInfos = orderProductInfos; //退换的订单货品列表，格式为：id1-num1,id2-num2...num为空时表示此订单货品全部退换
            countReturnMoney(param).then(res => {
                if (res.state == 200) {
                    let result = res.data;
        ...
        getCancelList() {
            ...
            refundReasonList(param).then(res => {
                if (res.state == 200) {
         ...
         * @lijm-2021-06-15
         */
        multifileUpload({ fileArray, params, success, fail }){
            if (!fileArray || !params) {
                return '';
            }
            let promiseArray = [];
            ...
            uni.chooseImage({
                ...
                success: function (res) {
                    ...
                    if (res.tempFiles.some(item => item.size > 20971520)) {
                        ...
                        that.multifileUpload({
                            fileArray: res.tempFilePaths,
                            params,
                            success: function (result) {
                                that.uploadFiles = that.uploadFiles.concat(result.map((item) => {
        ...
        getRefundMoney() {
            let param = {};
            param.orderProductId = this.orderProduct.orderProductId;
            param.applyNum = this.applyNum;
            getReturnMoney(param).then(res => {
                if (res.state == 200) {
                    let result = res.data;
        ...
        goSubmit(type) {
            ...
            case 'confirm': {
                ...
                let param = {}
                param.orderSn = that.orderProduct.orderSn;
                // serviceType:'',  //服务类型   0： 仅退款（无需退货） 1 ： 退货退款  2：换货
                param.afsType = that.sourceType == 'selecTService' && that.serviceType == '0'
                    ? 3
                    : 
                ...
                param.applyReasonContent = that.reasonContent; //申请售后服务原因，售后服务原因列表选择的内容
                param.afsDescription = that.refundInstructions; //详细问题描述
                param.applyImage = that.upLoadFile.join(','); //申请提交图片,多个图片用英文逗号隔开
                param.productList = productList; //订单货品列表
                // 货物状态目前不传了
                ...
                if (that.serviceType == '0'||that.serviceType == '1'){
                    //退款金额 , 仅退款0和1退货退款
                    param.finalReturnAmount = that.tosource == 1
                ...
                // 如果原始订单存在 需要带过去
                param.oldAfsOrderSn = this.oldAfsOrderSn?this.oldAfsOrderSn:''
                // 返回方式
                param.pickWareType = this.pickWareType?this.pickWareType:''
                // 如果是上门取件 必须要传取件地址
                if (this.pickWareType==this.enumType.pickUp){
                    param.pickWareAddress = this.pickWareAddress
                } else {
                ...
                if ( this.pickWareType==this.enumType.pickUp && ['2','4'].includes(String(that.serviceType)) ){
                    ...
                    param.receiveWareAddress = this.isDefaultAddress?this.pickWareAddress:this.receiveWareAddress
                } else if ( this.pickWareType==this.enumType.mail && ['2','4'].includes(String(that.serviceType)) ){
                    //自行邮寄 
                    param.receiveWareAddress = this.receiveWareAddress
                } else {
                    // 
                    param.receiveWareAddress = {}
                }
        ...
        getAfsServiceReverseExpress(){
            return new Promise((resolve) => {
                ...
                param.orderProductId = this.orderProductId;
                afsServiceReverseExpress(param).then(res=>{
                    if (res.state==200){
                        resolve(res.data)
                        const { data } = res
                        if (data.length>0){
        ...
        readedTips() {
            ...
            if (that.backSource == 'orderDetail') {
                try {
                    // 将售后检查结果放入缓存
                    this.setAfsCheckToLocalStorage(that.orderProduct.orderSn);
            ...
            // 从礼物详情页来 
                ...
                // 将售后检查结果放入缓存
                this.setAfsCheckToLocalStorage(that.orderProduct.orderSn);
                // 更新售后列表页,可能更新失败 ,因为从消息推送过来没有进售后列表页 ,还有pc刷新history加1的bug
                try {
        ...
        .cancel_popup {
            ...
            .cancel_popup_btn {
                ...
                .confrim_btn {
                    width: 668rpx;
                    height: 80rpx;
                    background: linear-gradient(-90deg, #FC1E1C 0%, #FF7A18 100%);
                    border-radius: 44rpx;
                    font-size: 30rpx;
                    font-family: PingFang SC;
                    font-weight: 600;
