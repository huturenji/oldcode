    ...
    <view class="look_logistics">
        ...
        <view class="logistics_des">
            ...
            <view class="logistics_desc" v-if="logisticsInfo.type!='1'">
                <view class="logistics_type">承运公司：<text>{{logisticsInfo.expressName ?
                logisticsInfo.expressName : '--'}}</text></view>
                <!-- <view class="logistics_type">物流状态：<text>{{logisticsInfo.type ? logisticsInfo.type : '--'}}</text></view> -->
                <view class="logistics_type useselect">物流单号：
                    <text>{{logisticsInfo.expressNumber ?
                    logisticsInfo.expressNumber : '--'}}
                    </text>
                </view>
                <view class="logistics_desc" v-if="logisticsInfo.type=='1'">
                    <view class="logistics_type">联系人：<text>{{logisticsInfo.expressName ?
                    logisticsInfo.expressName
                    : '--'}}</text></view>
                    <!-- <view class="logistics_type">物流状态：<text>{{logisticsInfo.type ? logisticsInfo.type : '--'}}</text></view> -->
                    <view class="logistics_type">联系电话：<text>{{logisticsInfo.expressNumber ?
                    logisticsInfo.expressNumber : '--'}}</text></view>
                </view>
            </view>
            <!-- 物流公司,及商品 end-->
            ...
            <view class="lofistics_info" v-else-if="!loading && logisticsRouteList.length > 0">
                <!-- 纵向排列 -->
                <uni-steps :options=logisticsRouteList direction="column" :active="0" activeColor="#333"
                    deactiveColor="#949494"></uni-steps>
            </view>
            <view class="no_data" v-else-if="!loading && logisticsRouteList.length ==0&&logisticsInfo.type!='1'">
                <image src="https://bbnjstaticba-sinosun.oss-cn-hangzhou.aliyuncs.com/Ba04ZWJgWx/miniProgram/empty/icon_defpage_zwnr.png" mode=""></image>
                <text>暂无物流信息，请耐心等待哦~</text>
            </view>
            <!-- 物流轨迹 end -->
            <!-- 推荐商品 start-->
...
import uniSteps from '@/views/components/uni-steps/uni-steps.vue'; //步骤条
...
export default {
    ...
    methods: {
        ...
        initDetailFromCache() {
            ...
            const that = this;
            try {
                let result = getStorageSync('packageItem')
