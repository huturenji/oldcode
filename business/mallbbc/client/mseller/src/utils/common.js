...
export function toLogin(){
    uni.redirectTo({
        url: `${config.LOGIN_PATH}?suspend=true`
    })
}
...
// #ifdef H5
export function getPrimaryKey(){
    let {thirdUserId, companyId} = getApp().globalData.userParams;
    let key = `${thirdUserId}-${companyId}-`;
    return key;    
}
// #endif
 ...
 * @param {*} dom 字体dom
 */
export function fitFontSize(dom) {
        ...
        while (!!fontOverFlow(item) && !breakLoop){
            ...
            children.forEach((citem)=>{
                if (citem.nodeType==1){
                    ...
                    if (parseInt(citem.style.fontSize) <= 12){
