 ...
 * 更新用户信息
 */
export function setUserInfoInGlobalData(userInfo = {}){
    getApp().globalData.userParams = {...getApp().globalData.userParams, ...userInfo}
}
...
export function getAppUserInfo(){
    return new Promise(resolve => {
        let userInfo = {}
        if(sinosdk.sino.isInSinoEnv()){ //说明是伴正事或者tchat平台 即兆日平台
            sinosdk.sino.getUserInfo().then(res => {
                userInfo = {
                    thirdUserId: res.UAId || '',
                    companyId: res.cpyId || ''
                }
                resolve(userInfo)
            }).catch(e => {
