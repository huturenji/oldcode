<template>
    <view :class="['container', !isInSinoEnv?'sinotop':'', isPC?'pctop':'']">
        <view class="top_wrap">
            <view class="user">
                <view @click="showActionSheetFun" class="user_name">
            ...
            <view class="shop_container">
                <template v-if="choosedShop && choosedShop.shopName">
                    ...
                    <view class="scan_box">
                        <view @click="scan" class="scan_item">
                            <template v-if="isPC">
                                <image src="https://bbnjstaticba-sinosun.oss-cn-hangzhou.aliyuncs.com/Ba04ZWJgWx/O2O/scan/btn_yhq_shangchuanwenjian.svg" mode="widthFix"></image>
        ...
        <view class="history">
            ...
            <view v-if="hasCheckedList" class="list">
                <scroll-view scroll-y="true" class="scroll-list" @scrolltolower="getMoreData">
            ...
            <view class="list empty_list" v-else-if="!hasMore">
                <empty tips="暂无内容"/>
            </view>
        </view>
...
import { getStorageSync, setStorageSync, isArray, checkPaginationHasMore, toLogin, removeShopStorage } from '@/utils/common';
...
export default {
    ...
    computed:{
        ...
        isInSinoEnv(){
            return !!(sinosdk?.sino?.isInSinoEnv())
        }
    },
    ...
    methods: {
        ...
        h5Scan(){
            ...
            sinosdk.sino.execAction(Json).then(async res => {
                console.log('res', res);
                //扫描到的内容
                if (!!(res?.[0]?.value)){
...
.container{
    ...
    padding-top: var(--titleBarHeight, 0px);
    &.pctop{
...
.history{
    ...
    .list{
        height: calc(100vh - var(--titleBarHeight, 0px) - 500rpx - 120rpx);
    }
    .empty_list{
        height: calc(100vh - var(--titleBarHeight, 0px) - 500rpx - 120rpx - 60rpx);
        border-radius: 20rpx;
        background-color: #fff;
        display: flex;
        align-items: center;
