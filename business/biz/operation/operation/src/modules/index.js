...
if (OPCL.login) {
    ...
    const clientId = 'BIZCLOUD_operation_front';
    const homePath = window.origin + "/bizcloud/static/operation/index.html";
    const jsonFilePath = NODE_ENV == 'production' ? `../keycloak.json?t=${TIMESTAMP}` : './thirdparty/keycloak.json'
    const permissionsUrl = "/bizcloud" + "/zsa-adapter/v1/getUserPermissionDetail"
