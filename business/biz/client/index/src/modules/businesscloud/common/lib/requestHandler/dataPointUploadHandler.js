...
class dataPointUploadHandler {
    ...
    */
    dataPointUploadEventListener() {
        let that = this;
        let handler;
        this.listenerEvents.forEach(function (eventType, index) {
            //先注销掉事件在注册
            document.removeEventListener(eventType, handler);
            document.addEventListener(eventType, handler = function handler(){
    ...
    dataPointUpload(sign,eventType,signData){
        ...
        if(!!globalPointData[sign] && !!globalPointData[sign].eventBisMap[eventType]){
            try {
                ...
                extendUtils.notifyActionEventFunction(bisData).then((res)=>{
                });                
            } catch (error) {
            }
        }
