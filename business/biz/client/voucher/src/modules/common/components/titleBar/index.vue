...
import HomeIndicator from 'common/lib/utils/homeIndicator'
...
function support(){
    ...
    || sinosdk.sino.getPlatform() == sinosdk.sino.constant.RUN_ENV.WEBOA)
            && sinosdk.sino.getNavigatorType() != sinosdk.sino.constant.NAVIGATOR_TYPE.PC //不可是pc端
            && extendUtils.isNotEmpty(navigatorData) //需是支持的webkit版本
...
export default {
    ...
    methods: {
        ...
        set(option={}){
            if (!this.enable || extendUtils.isEmpty(option)){
                return this;
            }
            //兼容decoData格式的数据
            ...
            if (extendUtils.isNotEmpty(statusOption)){
                let statusStyle = {};
                extendUtils.isNotEmpty(statusOption.show) && (this.showStatus = statusOption.show);
                extendUtils.isNotEmpty(statusOption.suspend) && (statusStyle.suspend = statusOption.suspend);
                extendUtils.isNotEmpty(statusOption.opacity) && (statusStyle = Object.assign({}, statusStyle, this._opacityTransfer(statusOption.opacity)))
                extendUtils.isNotEmpty(statusOption.background) && (statusStyle.background = statusOption.background)
                this._setStatusStyle(statusStyle)
            }

            let titleOption = option.title ?? option;
            if (extendUtils.isNotEmpty(titleOption)){
                let titleStyle = {};
                extendUtils.isNotEmpty(titleOption.show) && (this.showTitle = titleOption.show);
                extendUtils.isNotEmpty(titleOption.suspend) && (titleStyle.suspend = titleOption.suspend);
                extendUtils.isNotEmpty(titleOption.opacity) && (titleStyle = Object.assign({}, titleStyle, this._opacityTransfer(titleOption.opacity)))
                extendUtils.isNotEmpty(titleOption.background) && (titleStyle.background = titleOption.background)
                extendUtils.isNotEmpty(titleOption.color) && (titleStyle.color = titleOption.color)
                extendUtils.isNotEmpty(titleOption.showTitle) && (this.decoData.h5.title.showTitle = titleOption.showTitle) 
            ...
            //native设置
            let config = {};
            extendUtils.isNotEmpty(titleOption.themeMode) && (config.themeMode = titleOption.themeMode);
            extendUtils.isNotEmpty(titleOption.showBack) && (config.showBack = titleOption.showBack);
            extendUtils.isNotEmpty(titleOption.showMenu) && (config.showMenu = titleOption.showMenu);
            extendUtils.isNotEmpty(titleOption.titlePosition) && (config.titlePosition = titleOption.titlePosition);
            extendUtils.isNotEmpty(titleOption.backStyle) && (config.backStyle = titleOption.backStyle);
            extendUtils.isNotEmpty(titleOption.contentOpacity) && (config.contentOpacity = titleOption.contentOpacity);
            let statusBarMode = extendUtils.isNotEmpty(statusOption.themeMode) ? statusOption.themeMode : titleOption.themeMode;
            extendUtils.isNotEmpty(statusBarMode) && (config.statusBarMode = statusBarMode)
        ...
        updateNativeConfig(config){
            ...
            if (extendUtils.isEmpty(config) || Object.getOwnPropertyNames(config).every(key => config[key] == this.decoData.native[key])){
                return;
            }
            ...
            //设置抓握器悬浮,必须每次更新titlebar后立即更新抓握器位置，否则抓握器的位置会被下一次的titlebar设置覆盖
            HomeIndicator.suspend();
        },

        getCurrConfig(){
            return extendUtils.deepClone(this.decoData);
        },
