...
class RequestHandler {
            ...
            if(checkNetWorkEnable){
                ...
                extendUtils[request.fun](request.param).then(async (da)=>{
                    ...
                    if(status == 401 || status == 403){  
                        ...
                        httpErrorCode = status == 401 ? 'AUTHORIZE_FAILED' : 'NO_AUTHORIZATION';
                    }
    ...
    async request (url, param={}, httpConfig={}) {
        ...
        //安全链路使用相对路径
        /* eslint-disable */
        (url.startsWith('http://')||url.startsWith('https://'))?url:url = '/bizcloud' + url;
    ...
    async reRequest(url, data,paramObj){
        return new Promise((resolve, reject)=>{
            paramObj = Object.assign({}, paramObj, 
                {retryTimes: (paramObj.retryTimes || 0)+1}, //添加重试次数
                this.defaultHeaders(this.getUserToken()) );//更新token
