...
def _parse_cmd_args(args_list):
    ...
    parser.add_argument("-d", "--remote-dir", type=str, help="远程web服务器目录,如果值为false则不远程部署")
    parser.add_argument("--config", help='使用配置文件，并获取配置文件中的数值')
    parser.add_argument('-i', '--remote-ip', type=str, help="远程服务器ip地址,如果值为false则不远程部署")
...
def _do_deploy(args_list):
    ...
    print('产品信息读取结束，信息为：%s' %prodcut_info_file)
    ...
    #远程部署包到指定的服务地址
    remote_status = _remote_install(args,prodcut_info_file,devops_deploy_tar_file)
    if remote_status != 0:
      raise Exception('_remote_install %s failed' % package_name)
    else:
      if args.remote_ip and args.remote_dir and args.remote_ip != 'false' and args.remote_dir != 'false':
        print('远程部署成功')
      if args.svn_url and args.svn_name and args.svn_password and args.svn_url != 'false' and args.svn_name != 'false' and args.svn_password != 'false':
        print('开始获取rollback_tar_name')
        #获取svn上一个版本的包地址
        ...
        if render_doc != True:
            ...
            if status != 0:
              raise Exception('svn_import failed %s' %svn_import_cmd)
            else:
...
#远程部署包
def _remote_install(args,prodcut_info_file,devops_deploy_tar_file):
    #如果没有设置部署的远程ip和远程目录，则直接返回true
    if args.remote_ip is None or args.remote_dir is None or args.remote_ip == 'false' or args.remote_dir == 'false':
       return 0
