<template>
    <div class="indexWrap">
        <Announcement v-if='isCurrent' class='announcement' :config="{businessTypes: '5'}"/>
        <div class="hotelDiv">
            <div class="hotelWrap flightWrap">
                ...
                <div class="lineTextWrap starWrap">
                ...
                <div class="button search-btn normal-btn" @click="searchHotel">
                    查询
                </div>
            </div>
        </div>
        ...
        <div v-transfer-dom>
            <popup v-model="showCityPop" height="90%" width="100%" position="bottom" class="citySelect">
...
const HotelDate = ()=>import('./hotelDate');
export default {
    directives:{
        TransferDom
    },
    ...
    data() {
        ...
        return Object.assign(managerData, {
            ...
            cityPromise: null,//等待获取城市列表
            inwxmini:extendUtils.MINIPROGRAM_CONFIG.IN_MINIPROGRAM//是否是微信小程序内嵌页面
        });
    },
    async created(){
        let that = this;
        ...
        that.init();
        Bus.$on('goHotelDetail', this.goDetail)
    },
    mounted(){
        this.$emit('mounted')
    },
    watch: {
        hotelTripJson: {
            deep: true,
            handler(_newValue){
                this.$emit('changeSearchData', _newValue)
            }
        }
    },
    activated(){
        this.$emit('changeSearchData', this.hotelTripJson)
    },
    deactivated(){
        //注销定位 keep-alive时调用
    ...
    methods:{
        goDetail(history){
            ...
            if (this.inwxmini){
                ...
                extendUtils.setStorage(key,'');
                extendUtils.setStorage('hotelSearch', JSON.stringify(hotelSearch));
                requestHandler.openPage('hotel/index.html#/detail?'+param.join('&'));
                return
            }
            //先查询酒店详情是否状态正常
        ...
        async init(){
            ...
            _this.useTypeConfig = await extendUtils.useTypeConfig();
            ...
            if (!!!extendUtils.getStorage('showHotelTipsIsShow')){
        ...
        nearCity() {
            ...
            if (!_this.hoveLocation) {
                extendUtils.showToast('正在获取定位信息');
        ...
        searchHotel() {
            ...
            if (!cityName) {
                extendUtils.showToast('请选择城市');
                return;
            }
            if (0 >= that.hotelTripJson.inDays) {
                extendUtils.showToast('入住天数至少是一天');
                return;
            }
            //判断城市是否正确
            if (this.inwxmini){
                ...
                extendUtils.authHandler.moduleGate(()=>{
                    that.doHotelSearch(cityName);
                })
            }
        },
        ...
        async doHotelSearch(cityName){
            ...
            if (!cityCode){
                extendUtils.showToast('请选择正确城市');
                return;
            }
            //存储酒店历史城市
            _this.setHotelHistoryCity();
            //传递查询条件
            let url = 'hotel/index.html#/list?keywords='+window.Base64.encodeURI(_this.keywords)+
                '&useType='+_this.hotelTripType+
                '&cityName='+cityName+
                '&cityCode='+cityCode;
            let indexHotelJson = {
            ...
            extendUtils.setStorage('tripType', _this.hotelTripType);
        ...
        chooseDate(data) {
            ...
            if (!data.startTime.getTime()) {
                extendUtils.showToast('请选择正确开始时间')
                return;
            }
            
            if (!data.endTime.getTime()) {
                extendUtils.showToast('请选择正确结束时间')
                return;
            }
            if (data.startTime.getTime() == data.endTime.getTime()) {
                extendUtils.showToast('开始时间和结束时间不能是同一天')
                return;
            }
        ...
        async setHotelHistoryCity() {
            const that = this;
            await extendUtils.authInterceptor();
            let cityKey = requestHandler.primaryKey + "_HhCity";
        ...
        async getHotelHistoryCity() {
            ...
            // let a = extendUtils.getStorage("locationCity")
            ...
            if (!!that.cityList && that.cityList.length > 0 && !!extendUtils.getStorage("locationCity")){
                for (let i =0;i < that.cityList.length;i++){
                    ...
                    if (!!group.dataList && group.dataList.length > 0){
                        for (let j=0;j<group.dataList.length;j++){
                            if (group.dataList[j].name == extendUtils.getStorage("locationCity")){
        ...
        getHotelHotCity() {
            const that = this;
            requestHandler.getHotelHotCity({}).then((res) => {
                if (!!res.result) {
        ...
        getHotelCity() {
            ...
            this.cityPromise = new Promise(resolve => {
                //获取缓存中的数据
                if (extendUtils.getStorage('hotelCitys')){
                    that.cityList = JSON.parse(extendUtils.getStorage('hotelCitys'));
                }
                //获取最新酒店城市并存储缓存
                requestHandler.getHotelCitys({}).then((res) => {
                    if (!!res.result.cityList) {
                        that.cityList = res.result.cityList;
                        if (0 < res.result.cityList.length){
                            extendUtils.setStorage('hotelCitys',JSON.stringify(res.result.cityList.slice(0,1)));
                        }
                    }
                    that.getHotelHistoryCity(); 
                    resolve()
        ...
        async getLocationByGaode(){
            ...
            var onComplete = function(data){
                ...
                extendUtils.setStorage('locationCity', _this.locationCity);
            ...
            //使用高德地图定位
            extendUtils.geoLocationByGaode(onComplete,onError);
        },
        /**
         * 获取定位信息
         */
        ...
        getLastHotelFliter(){
            ...
            let cityCode = _this.isHotelCityValid(cityName);
            if (!!cityCode){
                let key = requestHandler.primaryKey + cityCode + "_LastHotelFlite";
                if (!!extendUtils.getStorage(key)){
                    let dataJson = JSON.parse(extendUtils.getStorage(key));
        ...
        async setTripInfo(){
            ...
            //酒店读取上一次查询的城市数据
            await extendUtils.authInterceptor();
            if (!!extendUtils.getStorage(requestHandler.primaryKey+'_indexHotelJson')){
                let indexHotelJson = JSON.parse(extendUtils.getStorage(requestHandler.primaryKey+'_indexHotelJson'));
            ...
            if (!!pageFrom && (pageFrom == 'trip' || pageFrom == 'order' || pageFrom == 'push')&&_this.isCurrent){ //如果是行程或者订单详情中过来的数据，需要判断是否是酒店
        ...
        hideAndSaveHotelTips(){
            ...
            extendUtils.setStorage('showHotelTipsIsShow', true);
        },
        /**
             * 首页同步列表页或详情页的日期数据
             */
        rereadHotel(){
            let _this = this;
            let hotelSearch = JSON.parse(extendUtils.getStorage('hotelSearch'));
...
<style lang="less" scoped>
    @import "~themes/default/styles/componets/hotel/index.less";
</style>
<style>
.citySelect{
