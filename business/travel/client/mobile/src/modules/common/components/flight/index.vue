<template>
    <div class="page-index-domestic">
        <Announcement v-if='isCurrent' class='announcement' :config="{businessTypes: '3'}"/>
        <div class="flightWrap">
            <div class="flightCityWrap">
                <div class="name">
                    <label
                        class="no-wrap cursorp"
                        @click="selectCity('begin')"
                        :class="{
                            turndirection: isTurnDirection,
                        }"
                        >{{
                            isStartAirport
                                ? tripJson.departAirportName
                                : tripJson.startCity
                        }}</label
                    >
                    <label
                        class="no-wrap cursorp"
                        @click="selectCity('end')"
                        :class="{
                            turndirection: isTurnDirection,
                        }"
                        >{{
                            isEndAirport
                                ? tripJson.arriveAirportName
                                : tripJson.endCity
                        }}</label
                    >
                </div>
            ...
            <div class="dateWrap">
                <div
                    class="dateItem cursorp"
                    @click="selectCalendar('start')"
                >
                    <div>
                        <span class="title">出发</span>
                        <span class="week">{{
                            new Date(
                                tripJson.selectStartDateStamp * 1000
                            ).getTime() | sDay
                        }}</span>
                    </div>
                    <div class="dateText">
                        <span class="num-font">{{
                            formatDate(tripJson.selectStartDateStamp).month
                        }}</span>
                        <span class="label">月</span>
                        <span class="num-font">{{
                            formatDate(tripJson.selectStartDateStamp).date
                        }}</span>
            ...
            <div class="cabin-wrap">
                ...
                <div
                    class="text cursorp"
                    @click="showFlightCabinPop = true"
                >
                    {{ flightCabinType.text }}
                    <Icon
                        type="icon_common_rightarrow"
                        size=".24"
                        class="icon icon-btn"
                    />
                </div>
            ...
            <div
                class="travle-type"
                v-if="!!useTypeConfig && useTypeConfig.isBoth()"
            >
                ...
                <div class="text">
                    <span
                        class="
                            ...
                            cursorp
                        "
                        @click="flightTripType = USE_TYPE_ENUM.PUBLIC.name"
                    >
                        <Icon
                            :type="
                                useTypeConfig.isPublic(flightTripType)
                                    ? 'btn_common_radio_sel'
                                    : 'btn_common_radio_nor'
                            "
                            size=".38"
                            class="icon"
                        />
                    ...
                    <span
                        class="
                            ...
                            cursorp
                        "
                        @click="flightTripType = USE_TYPE_ENUM.PRIVATE.name"
                    >
                        <Icon
                            :type="
                                useTypeConfig.isPrivate(flightTripType)
                                    ? 'btn_common_radio_sel'
                                    : 'btn_common_radio_nor'
                            "
                            size=".38"
            ...
            <div
                v-if="
                    useTypeConfig && useTypeConfig.isPublic(flightTripType)
                "
                class="travle-standards"
            >
                差旅标准:
                <span class="seatType">{{ flightCabinInfo.name }}</span>
            </div>
        </div>
        <div class="button search-btn normal-btn" @click="searchFlight">
            查询
        </div>
        ...
        <div v-transfer-dom style="display: none">
            <ScrollLock :lock="false" :bodyLock="showCalendar">
                <SnDatetimePicker
                    ref="SnDatetimePicker"
                    v-model="chooseDay"
                    @change="choseDayChange"
            ...
            >
                <div class="roomInfoWrap" @touchmove.prevent>
                    ...
                    >
                        请选择舱位
                        <Icon
                            type="btn_common_close"
                            size=".32"
                            class="but"
                            @click="showFlightCabinPop = false"
                        />
                    </div>
                    <cabinCheck
                        :cabinList="flightCabinList"
                        v-model="flightCabinType"
                    ></cabinCheck>
                </div>
            </popup>
        </div>
...
import {
    FlightFilterOptions,
    NOLIMIT
} from "custCommon/enum/flightFilterEnum.js";
import ScrollLock from "custComp/scrollLock";
const Announcement = () => import('components/announcement/index');
const City = () => import("components/city/city");
const CabinCheck = () => import("./cabinChecker");
const History = () => import("./history");
const otherSourceType = ["trip", "order", "entryList", "push", "footerBar"];
...
export default {
    ...
    computed: {
        isCurrent(){
            return this.currentView == 'Flight'
        }
    },
    data() {
        let that = this;
        let managerData = extendUtils.stateManager.setData(
            [
                ...
                //显示日历
                {
                    ...
                    hide: {
                        callback: function() {
                            that.$refs.SnDatetimePicker.onClickOverlay();
                        }
                    }
                },
                //城市选择控件
        ...
        return Object.assign(managerData, {
            ...
            tripJson: {
                ...
                arriveAirportCode: "",
                arriveAirportName: "",
                selectStartDateStamp:
                    parseInt(new Date().getTime() / 1000) + 24 * 1 * 3600
            },
    ...
    methods: {
        ...
        async setTripInfo() {
            ...
            //机票读取上一次查询的城市数据
            if (
                !!extendUtils.getStorage(
                    requestHandler.primaryKey + "_indexFlightJson"
                )
            ) {
                let indexFlightJson = JSON.parse(
                    extendUtils.getStorage(
                        requestHandler.primaryKey + "_indexFlightJson"
                    )
                );
            ...
            if (
                !!this.pageFrom &&
                otherSourceType.indexOf(this.pageFrom) > -1 &&
                that.isCurrent
            ) {
                if (
                    !!that.$route.query.startCity &&
                    "" != that.$route.query.startCity
                ) {
                    that.tripJson.startCity = decodeURIComponent(
                        that.$route.query.startCity
                    );
                }
                if (
                    !!that.$route.query.endCity &&
                    "" != that.$route.query.endCity
                ) {
                    that.tripJson.endCity = decodeURIComponent(
                        that.$route.query.endCity
                    );
                }
                let now = new Date();
                ...
                let startDate =
                    !that.$route.query.departTime ||
                    that.$route.query.departTime < now.getTime()
                        ? tommrowTime
                        : that.$route.query.departTime;
        ...
        async doFlightSearch() {
            ...
            await this.cityPromise;
            this.$vux.loading.hide()
            await extendUtils.authInterceptor();
            if (that.flighType == "single") {
                that.tripJson.startCityCode = that.isFlightCityValid(
                    that.tripJson.startCity
                );
                if (!that.tripJson.startCityCode) {
                    extendUtils.showToast("请选择正确的出发地");
                    return;
                }
                that.tripJson.endCityCode = that.isFlightCityValid(
                    that.tripJson.endCity
                );
                if (!that.tripJson.endCityCode) {
                    extendUtils.showToast("请选择正确的目的地");
                    return;
                }
                extendUtils.setStorage(
                    "startCityCode",
                    that.tripJson.startCityCode
                );
                extendUtils.setStorage(
                    "endCityCode",
                    that.tripJson.endCityCode
                );
                extendUtils.setStorage(
                    "selectDate",
                    new Date(that.tripJson.selectStartDateStamp * 1000).format(
                        "yyyy/MM/dd"
                    )
                );
                ...
                if (that.isStartAirport) {
                    extendUtils.setStorage(
                        "departAirportName",
                        that.tripJson.departAirportName
                    );
                } else {
                ...
                if (that.isEndAirport) {
                    extendUtils.setStorage(
                        "arriveAirportName",
                        that.tripJson.arriveAirportName
                    );
                } else {
                    extendUtils.setStorage("arriveAirportName", "");
                }
                if (
                    extendUtils.getStorage("startCityCode") ==
                    extendUtils.getStorage("endCityCode")
                ) {
                    extendUtils.showToast("出发地和目的地不可以相同");
                } else {
                    //存储机票的查询历史城市记录
                    that.setFlightHistoryCity();
                    let indexFlightJson = {
                    ...
                    extendUtils.setStorage(
                        requestHandler.primaryKey + "_indexFlightJson",
                        JSON.stringify(indexFlightJson)
                    );
                    let url =
                        "flight/index.html#/list?flightCabinType=" +
                        that.flightCabinType.value +
                        "&useType=" +
                        that.flightTripType;
         ...
         * 存储机票历史城市
         */
        async setFlightHistoryCity() {
            const that = this;
            await extendUtils.authInterceptor();
            let cityKey = requestHandler.primaryKey + "_FHCity";
            var cityList = !!extendUtils.getStorage(cityKey)
                ? JSON.parse(extendUtils.getStorage(cityKey))
                : [];
            if (
                !!that.cityList &&
                that.cityList.length > 0 &&
                !!that.tripJson
            ) {
        ...
        async getFligthHistoryCity() {
            ...
            await extendUtils.authInterceptor();
            let cityKey = requestHandler.primaryKey + "_FHCity";
            var cityArray = !!extendUtils.getStorage(cityKey)
                ? JSON.parse(extendUtils.getStorage(cityKey))
                : [];
            //数组去重
        ...
        getFligthCity() {
            ...
            return new Promise(() => {
                if (extendUtils.getStorage("flightCitys")) {
                    that.cityList = JSON.parse(
                        extendUtils.getStorage("flightCitys")
                    );
                    //获取最新机票城市并存储缓存
                ...
                requestHandler
                    ...
                    .then(res => {
                        if (!!res.result) {
                            ...
                            extendUtils.setStorage(
                                "flightCitys",
                                JSON.stringify(that.cityList.slice(0, 1))
                            );
                        }
                        resolve()
                    })
                    .catch(err => {
                        console.error(err);
                        extendUtils.showToast('获取城市列表失败')
                        resolve()
                    });
            })
        },
        ...
        getFlightHotCity() {
            ...
            requestHandler
                ...
                .then(res => {
                    if (!!res.result) {
                        that.hotCityList = extendUtils.sortedUniq(
                            res.result.hotCity
                        );
                    }
                })
         ...
         */
        choseDayChange(date) {
            this.tripJson.selectStartDateStamp =
...
@import "~themes/default/styles/componets/flight/index.less";
</style>
<style lang="less">
