<template>
  <div class="share-address">
    ...
    <EmptyX class="empty_wrap" type="noaddress" v-else-if='!loading && 0 == shareList.length' tipsText='您的公司暂无共享地址，赶紧为您的公司添加一个共享地址吧'/>
    ...
    <!-- 新增和编辑共享地址弹窗页面-->
    <div v-transfer-dom>
        <popup v-model="showAddShareAddress" :popup-style="{zIndex: 850}" height="100%" width="100%" position="right" class="editSharePop" :show-mask="false">
            <editShareAddress 
        ...
        let managerData = addressHandler.stateManager.setData([
            {
                ...
                parent: '$refs.addressCard.$refs.shareListComp', //todo 目前涉及到多级弹窗回退的问题，后续有时间优化处理
                show: {
                    callback: function () {
      ...
      */
       ...
       */
        deleteAddress(item) {
            ...
            let that = this;
            addressHandler.showConfirm( '确定要删除该共享地址吗？', function(){
       ...
       */
        deleteAddressFun(item){
            ...
            addressHandler.deleteCommonAddress(param).then((res) => {
                if (res.resultCode == 0) {
       ...
       */
        getShareAddressList(){
            ...
            return new Promise((resolve) => {
                addressHandler.getShareAddressList(param).then(res=>{
                    that.loading = false;
