    ...
    <div class='list'>
        <div class="type">
            <div class="card-date card-date-train" :class="{flowId: trip.flowId}">
                ...
                <div v-if="isPC" @click="deleteOrder((train||{}).orderNo,train,trip,isHis)"  class="delete_order">删除</div>
            </div>
        </div>
      <div class="full train card-shadow">
          <swipeout>
                <swipeout-item :disabled='isPC'>
                    ...
                    <div slot="content" class="trainDetail detailCard linear-gra-train cursorp" @click="openOrderDetail((train.originalOrderNo)?(train.originalOrderNo):(train.orderNo))">
                        <div class="leave">
                            <div class="code">
...
export default {
    ...
    methods:{
        ...
        openOrderDetail(orderNo){
            tripHandler.openOrderDetail('train',orderNo,this.isSelf,this.founderName);
        },
        /**
             * 计算剩余时间
        ...
        remainTime(orderInfo){
            ...
            return tripHandler.countTime(new Date().getTime(), beginTime, endTime, 'train');
        },
        /**
               * 计算相隔时间
               * @param {Object} orderInfo  订单信息
    ...
    .stage {
        ...
        .arrow {
            ...
            .trainNo {
                width: 1.8rem;
                height: 0.34rem;
                background: url(~themes/default/img/icon_train_shikebiao@2x.svg) center no-repeat;
                background-size: 100%;
                &:active{
                    opacity: .8;
                }
