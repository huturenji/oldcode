        ...
        <div class='list'>
            <div class="type">
                <div class="card-date card-date-flight" :class="{flowId: trip.flowId}">
                    ...
                    <div v-if="isPC" @click="deleteOrder((AirLine||{}).orderNo,AirLine,trip,isHis)" class="delete_order">删除</div>
                </div>
            </div>
            <div class="full flight card-shadow">
                <swipeout>
                    <swipeout-item :disabled='isPC'>
                        ...
                        <div slot="content" class="flightDetail detailCard linear-gra-flight cursorp" @click="openOrderDetail((AirLine.originalOrderNo)?(AirLine.originalOrderNo):(AirLine.orderNo))">
                            <div class="leave">
                            ...
                            <div class="content content-flight">
                                <div class="trip">
                                    <div class="station">
                                        <div class="time num-font">{{AirLine.beginTime}}</div>
                                    ...
                                    <div class="station">
                                        <div class="time num-font">{{AirLine.arriveTime}}</div>
...
export default {
    ...
    filters:{
        getWeek(value) {
            if (!!value) {
                try {
                    return tripHandler.genWeek(new Date(value).getDay(), 1);
                } catch (e){
                    return null;
                }
            }
    ...
    methods:{
        ...
        openOrderDetail(orderNo){
            tripHandler.openOrderDetail('flight',orderNo,this.isSelf,this.founderName);
        },
        /**
             * 计算剩余时间
        ...
        remainTime(AirLine){
            ...
            return tripHandler.countTime(new Date().getTime(), beginTime, endTime, 'flight');
        },
    ...
    .trip {
        ...
        .arrow {
            ...
            .icon{
                ...
                width: 1.8rem;
                height: 0.24rem;
                background: url(~themes/default/img/icon_plane_wjingting@2x.svg) center no-repeat;
                background-size: 100%;
                &:active{
                    opacity: .8;
                }
