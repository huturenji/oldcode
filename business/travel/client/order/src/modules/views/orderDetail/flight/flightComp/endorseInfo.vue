<template>
    <div class="endorse-container">
        ...
        <div class='btn-group'>
            <SnButton class="search-btn normal-btn cursorp" type="primary" @click.native="searchFlight">搜索航班</SnButton>
        </div>
        ...
        <!-- 日历控件 -->
        <popup v-model="showCalendar" position="bottom" :show-mask='false' style="background: #ffffff ">
            <div class="calendar">
        ...
        <popup v-model="showCabinPop" position="bottom" :show-mask='false' class="showCabinPop">
            ...
            <ul>
                <li v-for="(val,key) in cabinObj" class="normal-btn cursorp"
                    :class="{disable: key<cabinRank}"
                    @click.stop="chooseCabin(key, $event)" :key="key">
                    {{val}}
