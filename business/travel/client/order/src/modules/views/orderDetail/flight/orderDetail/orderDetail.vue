<template>
    <div class="page-order-detail" :class='{"filter-blur": showPriceDetail}'>
        <mescrollVue ref="mescroll" :down="mescrollDown" :up="mescrollUp" @init="mescrollInit">
            <div v-if='!loading' class="order-detail-container">
                <div>
                    <header>
                        <div class="summary">
                        ...
                        <div class="order-no" v-if="orderDetail.payName && paymentMethods.length > 0">
                            <span>支付方式</span><span>{{getPaymentName}}</span>
                        </div>
                        <div class="order-no">
                            <span>订单号</span><span>{{orderDetail.providerOrderNo || orderDetail.orderNo}}</span>
                        </div>

                        <SnButton type="primary" class="btn-again normal-btn cursorp" v-if="orderDetail.orderStatus=='ALREADY_CANCEL'"
                            @click.native="judgeOrderBook() && orderAgain()">再次预订</SnButton>
                    </header>
                    ...
                    <div class='flight-card-container'>
                        <FlightInfoCard v-for="airLine in airLineList" :key="airLine.airLineID" :selfOrder="judgeOrderBook()" ref="flightInfoCard" :orderExtraInfo="orderExtraInfo"
                            :airline="airLine" :roundTrip="airLine.specificVoyage" :orderDetail="orderDetail" @loadPay='loadPay=true'
                            ...
                            @callPhone="callPhone" @reload='payTimeout' @cancelOrder="cancelOrder" @isNeedAddMoney='isNeedAddMoney' @setLimitTime='setLimitTime'/>
                        </div>
                    </div>
                    <ul class="items">
                        <li class='cursorp' @click="showDescription=true">
                            <div class="explanation fr cursorp">
                                退改签 / 行李额详情
                            </div>
                    ...
                    <li class="insurancesOutWrap cursorp" v-if="insuredCountInfos.length > 0"  @click="gotoInsurancesList()">
                        <div class="insurancestit">
                            <div>出行保险<Icon type="icon_plane_bao" size='.28' class="icon icon-btn"/></div>
                        </div>
                    ...
                    <li v-if="invoiceSupplement" class="cursorp" @click="judgeOrderBook() && toReimburse()">
                        <div class="fl">
                            <div>
                                报销凭证
                            </div>
                        ...
                        <SnButton inline ghost :type="judgeOrderBook() ? 'primary' : 'info'" size="small" shape="round">补开报销凭证</SnButton>
                    </li>
                    <!--积分模块-->
                    <!-- <li class='no-padding'>
                ...
                <div class='content'>
                    ...
                    <div class="price-detail">
                        <ul>
                            ...
                            <li v-if="invoiceFlag=='1' || invoiceDone=='1'" class="btp">
                            ...
                            <li v-if="orderDetail && orderDetail.cashcouponAmount" class="btp">
                                <div class="fl">优惠</div>
                                <div class="fr redfont">
                                    <span class="unit rmb">-&yen;</span>
                                    <span class='num-font'>{{orderDetail.cashcouponAmount}}</span>
                                </div>
                            </li>
        ...
        <div v-transfer-dom>
            <popup v-model="showCancelTicketInfo" height="100%" width="100%" position="right" class="pop-up">
                <cancelTicketInfo ref="cancelTicketInfo" :psgList="choosedPsgArr" :airline="choosedAirLine"
                    :providerShortName="orderDetail.providerShortName" :providerType="orderDetail.providerType" :orderDetail="orderDetail"
                    @callPhone="callPhone" @onConfirm="onConfirm" @showInsuranceOrder="showInsuranceDetail" @showInsuranceOption="showInsuranceOptionPop = true"></cancelTicketInfo>
            </popup>
        </div>
        <!-- 保险说明 -->
        <div v-transfer-dom>
        ...
        <Pay ref="payComp" 
            ...
            tradeType='1'
            :limitTime="limitTime"
            :orderNoList="chaOrder.unPaidOrderNos || orderDetail.unPaidOrderNos" 
            :amount="addMoney || orderDetail.amount"
            :goodsDesc="goodsDesc"
        ...
        >
            <div slot='result' slot-scope="{closePay}">
                <payResult 
                    ...
                    :endCity = 'airline.eCityName || ""'
                    :arriveTime = 'new Date(airline.arriveDate).getTime()+(24*3600*1000)'
                    :departTime = 'new Date(airline.arriveDate).getTime()'
        ...
        <div v-transfer-dom>
            <div class="btn-group" :class="{otherBook:!judgeOrderBook()}" v-if='!loading'>
                <a class="btn-cancel normal-btn cursorp" v-if="orderDetail.orderStatus=='UNPAID'" @click="judgeOrderBook() && cancelOrder()">取消订单</a>
                <a class="btn-pay normal-btn linear-gra-waring cursorp" v-if="orderDetail.orderStatus=='UNPAID'" @click="judgeOrderBook() && orderPay()">
                    去支付
                    <i v-if='limitTime>0' class="time icon"/>
                    <span>{{limitTime | limitTimeFilterChange}}</span>
                </a>
            </div>
        </div>
        <!-- 联系客服 -->
        <div v-transfer-dom>
            <template v-if='!showPriceDetail'><!--显示价格明细高斯模糊时隐藏这个层-->
