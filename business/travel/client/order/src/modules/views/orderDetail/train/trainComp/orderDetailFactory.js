...
export const data = function (extend) {
    return function () {
        ...
        let managerData = extendUtils.stateManager.setData([
            ...
            {
                name: 'showSpreadPop', //查看卧铺票有差价明细弹窗
                hide: {
                    title: '订单详情'
                }
            }
        ...
        let datas = {
            ...
            spreadDetailDes:{},//查看卧铺票差价明细
            servicePhone: extendUtils.BIS_CUSTOMER_SERVICE_PHONE//客服电话


        };
        datas = Object.assign(managerData, datas); //注意，用assign方法，managerData必须是第一个参数，否则其中的get、set方法会失效
        if (extend) {
            datas = Object.assign(datas, extend);
        }
        return datas;
    };
}
...
export const computed = {
     ...
     */
    zcConfig(){
        let trainNo = this.orderDetail.trainNo;
        let startStation = this.orderDetail.startStation;
        let endStation = this.orderDetail.endStation;
        let orderNo = this.orderDetail.providerOrderNo || this.orderDetail.orderNo; //此处的订单号使用原始订单号
        let orderAmount = this.orderDetail.orderAmount;
        let startDate = this.orderDetail.startDate;
        let startTime = this.orderDetail.startTime;
        if (this.gaiOrderObj && this.gaiOrderObj.length > 0){ //如果存在改签票的话，优先展示改签票的相关信息（默认取第一个乘客的）
            trainNo = this.gaiOrderObj[0].trainNo;
            startStation = this.gaiOrderObj[0].startStation;
            endStation = this.gaiOrderObj[0].endStation;
            orderAmount = this.gaiOrderObj[0].seatPrice;
            startDate = this.gaiOrderObj[0].startDate;
            startTime = this.gaiOrderObj[0].startTime;
        }
        if (!!extendUtils.isPC){
            return {}
        }
        ...
        return {
            card_title: `${trainNo} ${startStation}-${endStation}`, //标题（必传）
            card_url: encodeURIComponent(callBackUrl), //商品信息的商品链接地址（必传）（建议使用encodeURIComponent转义一下，防止链接中带有特殊符号导致参数获取失败）
            card_desc: encodeURIComponent(`订单号: ${orderNo}  金额: ￥${orderAmount}  发车时间: ${startDate} ${startTime}`), //商品信息的简述内容（选传）
            card_note: `${this.custOrderStatus}${this.chargeChangeAndRefundStatus(this.orderDetail.postSaleStatus)}`, //订单状态
            card_picture: encodeURIComponent(require('orderCommon/zkimg/train.png')) //商品的缩略图（选传）（建议使用encodeURIComponent转义一下，防止链接中带有特殊符号导致参数获取失败） 此处选择用base64的方式
        } 
   ...
   */
    ...
    */
    ...
    goBackFun(){
        let that = this;
        //如果H5支付正在进行，则先关闭H5支付
        if (this.$refs.payComp && this.$refs.payComp.sinoPayInstance && this.$refs.payComp.sinoPayInstance.isOnH5Pay()){
            this.$refs.sinoPayInstance.closeH5Pay();
            return;
        }
        that.checkPopStatus = true;
        let loadData = {
    ...
    clearCountdown(key) {
        ...
        if (!!that.trainTimeInterval && !key) {
            Object.keys(that.trainTimeInterval).forEach((keys)=>{
    ...
    checkStatus(psgList) {
        ...
        psgList.forEach((item) => {
            ...
            if (!!item.refundOrderInfoList && item.refundOrderInfoList.length > 0) {
                ...
                if (that.psgStatuState(item.status) != StateStyle.SUCCESS){
                    that.$set(item, 'newGaiOrder', null); //将改签的更新为null 退票的优先级大于改签的
                    that.$set(item, 'status', tuiList[0].orderStatus);
                } else {
                    that.$set(item, 'status', item.status || tuiList[0].orderStatus);
                }
    ...
    //显示卧铺票差价明细
    // showSpread(item){
    //     this.spreadDetailDes = item;
    //     this.showSpreadPop = true;
    // },
    /**
    ...
    */
    gotoRefundAndChange() {
        let that = this;
        requestHandler.openPage('train/index.html#/regression?orderNo=' + that.orderDetail.orderNo);
    },
    /**
    * 取消改签操作
    */
    ...
    changeTicket(person, orderDetail) {
        ...
        requestHandler.verifyTrainChange(param).then((res) => {
            if (res.result && res.result.success) {
                ...
                requestHandler.openPage('train/index.html#/endorse?psgName=' + person.psgName + '&StartStation=' + orderDetail.startStation + '&EndStation=' + orderDetail.endStation + '&StartDate=' + orderDetail.startDate);
            }
        }).catch((err) => {
            that.popLoading = false;
    ...
    showChangeError(){ //不符合改签条件
        // let that = this;
        extendUtils.showConfirm('开车前48小时（不含）以上，可改签预售期内的其他列车。开车前48小时内，可改签开车前的其他列车，也可改签开车后至票面日期当日24:00之间的其他列车，不办理票面日期次日及以后的改签。', function () {
            
        }, 1, '知道了', null, '改签失败', function () {}, true);
    },
    showOverTimeShort(){
        let that = this;
        extendUtils.showConfirm('<div class="overTimePop">距离发车时间较近，已停止网络改签/退票，如有需要请前往车站窗口办理</div>', function () {
            that.showDescription = true
        }, 2, '知道了', '退改说明', null, function () {}, true);
    },
    showOverTimePop(){
        let that = this;
        extendUtils.showConfirm('<div class="overTimePop">列车已发车，已停止退票，详情请查看退改签规则！</div>', function () {
            that.showDescription = true
        }, 2, '知道了', '退改说明', null, function () {}, true);
    },
    showOverChangeTimePop(){
        let that = this;
        extendUtils.showConfirm('<div class="overTimePop">当前时间已过票面日期当日24:00，无法办理改签业务，详情请查看退改签规则！</div>', function () {
            that.showDescription = true
        }, 2, '知道了', '退改说明', null, function () {}, true);
    },
    ...
    isRefundingOrChanging(person){
        ...
        if (person.newGaiOrder) {
            try { 
                ...
                extendUtils.showToast(`该订单状态不允许改签退票操作【改签订单状态${person.newGaiOrder.changeOrderStatus}】`);
                res = true;
            }
        }
        if (person.newTuiOrder) {
            try {
                ...
                extendUtils.showToast(`该订单状态不允许改签退票操作【退票订单状态${person.newTuiOrder.orderStatus}】`);
                res = true;
            }
        }
