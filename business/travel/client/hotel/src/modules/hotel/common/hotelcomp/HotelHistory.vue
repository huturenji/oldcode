    ...
    <div class="page-history">
        ...
        <section class="trip-history" v-if="tripHistory.length>0">
            ...
            <HotelList class="hotel-list"  v-for="(item,index) in tripHistory" :key="index"  @click.native='openDetail(item)'
                  :hotel="item" :locationPoint='locationPoint' :isCityPosition='isCityPosition' :scorerIsFavorableRate="true"/>
        </section>
    </div>
</template>
...
export default {
    ...
    methods: {
        ...
        clearSearchHistory(){
            hotelHandler.removeStorage(hotelHandler.primaryKey + key);
            this.getSearchHistory();
        },
        async getSearchHistory(){
            await hotelHandler.authInterceptor();
            this.searchHistory = JSON.parse(hotelHandler.getStorage(hotelHandler.primaryKey + key) || '[]');
        ...
        getTripHistroy(){
            ...
            hotelHandler.getOrderList(param).then(res => {
                if (!!res && !!res.result){
