<template>
    <div class="carOrderDetail">
                ...
                <div v-else>
                    ...
                    <div class="carTypeWrap" v-if="carOrderNameMap.CREATED==orderStatus">
                        ...
                        <div class="buttonWrap">
                            <div class="button normal-btncursorp" @click="cancelCall">取消呼叫</div>
                        </div>
                    </div>
                    <!-- 行程中的订单详情 -->
                    ...
                    <div class="priceDataWrap" v-if="carOrderNameMap.UNPAID==orderStatus">
                        ...
                        <div class="priceWrap">
                            ...
                            <div class="buttonWrap">
                                <div class="button normal-btncursorp" @click="openPay">支付</div>
                            </div>
                        </div>
                    </div>
                    <!-- 取消行程的订单详情 -->
                    ...
                    <div class="completedWrap" v-if="carOrderNameMap.PAID==orderStatus || carOrderNameMap.COMPLETED==orderStatus">
                        ...
                        <div class="optionWrap">
                            ...
                            <div class="option cursorp share" @click="shareTrip">行程分享</div>
                            <div class="option cursorp sevice" @click="callPhone('server')">联系客服</div>
                            <div class="option cursorp invoice" @click="codeing">开发票</div>
                        </div>
                    </div>
                </div>
                ...
                <div class="cancelDetail">
                    <div class="cancelContect">
                        ...
                        <div class="buttonsWrap">
                            <div class="button normal-btncursorp empty" @click="showCancelDetail=false">暂不取消</div>
                            <div class="button normal-btncursorp" @click="showCancelReason=true;showCancelDetail=false">确认取消</div>
                        </div>
                    </div>
                </div>
            </popup>
            ...
            <popup v-model="showCancelReason" height="100%">
                <div class="cancelReason">
                    ...
                    <div class="buttonWrap pcDialog">
                        <div class="button normal-btncursorp" @click="cancelTrip">确认提交</div>
                    </div>
                </div>
            </popup>
        </div>    
        ...
        <swp-pay v-if="swpPayLoad" v-model="showPayTypes"
            ...
            :orderNo="orderInfo.orderNo" :goodsDesc="getGoodsDesc"
        ...
        <!-- 发票明细 -->
        <!-- <div v-transfer-dom>
            <popup v-model="showInvoiceDetail" height="100%" width="100%" position="right" class="Pop">
                <InvoiceDetailIndex v-if="showInvoiceDetail" @closePop='showInvoiceDetail=false' :orderNo="orderInfo.orderNo" type="hotel"
                    :InvoiceContent='InvoiceContent' :InvoiceInfo='InvoiceInfo' :InvoiceRemarks='InvoiceRemarks' />
            </popup>
        </div> -->
    </div>
</template>
...
export default {
    ...
    data () {
        let _this = this;
        let managerData = requestHandler.stateManager.setData([
        ...
        return Object.assign(managerData, {
            role: this.$route.query && this.$route.query.role,//用户角色  company表示是企业管理跳转过来的
            loading: true,//页面加载中
    ...
    methods: {
        ...
        //     let that = this;
        //     return requestHandler.stateManager.closeTopPop(() => {
        //         if (!callback) {
        //             let loadData = {
        //                 orderNo: that.orderInfo.orderNo,
        //                 orderStatus: that.orderInfo.orderStatus,
        ...
        //             // push是推送过来的，必须返回2次，否则安卓有问题
        //             if(that.pageFrom=='advance' || that.pageFrom=='push' || that.pageFrom=='cash'){
        //                 requestHandler.goBackPage('', 2, loadData);
        //             }else if(that.pageFrom=='orderListPc'){
        //                 that.$router.go(-1)
        //             }else{
        //                 requestHandler.goBackPage('', 1, loadData);
        //             }
        //         } else {
        //             callback();
        //         }
        ...
        getOrderDetail(){
            ...
            OrderClass.getOrderDetail(_this.orderNo).then(function (res) {
                _this.loading = false;
                _this.orderInfo = res.data.basicOrder;
                _this.orderStatus = res.data.basicOrder.orderStatus;
                _this.driverInfo = res.data.driverInfo;
        ...
        getDrivePosition(type){
            ...
            requestHandler.queryDriverLocationByOrderId(obj).then(function (res) {
                if (res.code == 0){
                    //添加司机位置
                    _this.addMarkers(res.data.longitude,res.data.latitude,'','car',false);
                    if ('waitTrip'==type){
                        //添加司机位置-出发点路径
                        _this.getDriving(res.data.longitude,res.data.latitude,_this.orderInfo.fromLongitude, _this.orderInfo.fromLatitude,type,true);
                    } else if ('triping'==type){
                        //添加司机位置-目的点路径
                        _this.getDriving(res.data.longitude,res.data.latitude,_this.orderInfo.toLongitude,_this.orderInfo.toLatitude,type,true);
                    }
                }
            }).catch((e) => {
                console.log(e);
            ...
            if (!haveMarker){
                if (!isContent){
                    ...
                    var startIcon = new AMap.Icon({
                        size: new AMap.Size(29, 29),// 图标尺寸
                        image: _this.markerMap[name].src,// 图标的取图地址
                        imageSize: new AMap.Size(29, 29)// 图标所用图片大小
                    });
                        // 创建标记点对象
        ...
        cancelCall(){
            let _this = this;
            requestHandler.showConfirm('正在努力为您呼叫司机，确认要取消吗？', function(){
        ...
        cancelTrip(){
            ...
            if (!!!_this.choosedReasonId){
                requestHandler.showToast('请选择取消原因');
                return;
            } else if (_this.choosedReasonId == 7 && _this.choosedReason == ''){
                requestHandler.showToast('请输入取消原因');
                return;
            }
            ...
            requestHandler.cancelOrderCar(obj).then(function (res) {
                ...
                if (res.code == 0){
                    if (res.data.Success == true){
                        requestHandler.showToast('取消成功');
                        if ('call' == type){
                            let loadData = JSON.stringify({refresh:true});
                            extendUtils.closePage(loadData,1,'');
                        } else if ('trip' == type){ //取消行程
                            _this.initData();
                        }
                    } else {
                        requestHandler.showToast('取消失败');
                    }
                } else {
                    requestHandler.showToast('取消失败');
                }
        ...
        payTimeout(){
            ...
            if (requestHandler.isPC()){
                let msg = '抱歉，您的订单由于超时未支付已取消，请重新预订';
                requestHandler.showConfirm(msg, function(){
                    that.getOrderDetail();
        ...
        callPhone(typeOrNo) {
            if ('server'==typeOrNo){
                sinosdk.sino.callTel(requestHandler.BIS_CUSTOMER_SERVICE_PHONE);
            } else {
        ...
        /**
             ...
             */
        codeing(){
            requestHandler.showToast('程序小哥努力开发中，敬请期待...')
        },
        /**
             * 格式化显示时长
             */
        ...
        openChat (item) {
            ...
            var appIconPath = location.origin + location.pathname.split('enterprise/index.html')[0] + 'assets/img/car/icon_car.png'
            var pcIconPath = location.origin + location.pathname.split('enterprise/index.html')[0] + 'assets/img/car/icon_car.png'
            ...
            var flowUrl = location.origin + location.pathname + "#/order/detail/car/app?ProdId="+_this.prodId+"&pageType=businessTrip&pageFrom=share&OrderNo=" + _this.orderNo+ "&appletUAId=" + _this.appletUAId;
            var flowInfor = {
            ...
            sinosdk.sino.getUserInfo({"UAId":item.uAId}).then(function(uaData){
                if (1 == uaData.isActive && undefined != uaData.uName && "" != uaData.uName){ //用户存在
                    ...
                    // showToast(item.uAName+'已被删除，无法分享', 'middle');
                }
            })
        },
        /**
