<template>
    <div class="page-flight-card">
        <section class="order-card" :class="{disable: airline.disableStyle}">
            <FlightTimesComp :item="airline" :iconType="roundTrip" class="flight-times" />
                ...
                <div class="psg-info" v-for="(psg,index) in airline.passengers" :key="index"
                    ...
                    <div class="customer-list">
                        <div class="customer-info">
                            ...
                            <div>{{strIdType(psg.cardType)}}:{{desensitization(psg.cardNo)}}</div>
                            <!-- airline:改签票显式卡票 -->
                    ...
                    <div class="cust-btn-group clear">
                        <div class="psg-status fl" :style="{color: getPsgStatusColor(psg.status)}"
                            ...
                            <template v-if="getUserInsType(psg) && psgStatuState(psg.status) == StateStyle.SUCCESS">
                                <span
                                    class="quitInsTips">{{getUserInsTips(psg,getPsgStatuType(psg.status)=='1')}}</span>
                            </template>
                        </div>
                    ...
                    <div class="status-message ticket-failed-info" v-if="psgStatuState(psg.status)== StateStyle.UNKNOW">
                        <Icon size='.28' type="icon_common_prompt" class="icon icon-btn" />
                        请联系客服确认出票结果或以短信通知为准
                    </div>
                    <div class="status-message ticket-failed-info" v-if="orderDetail.orderStatus=='FAILED_OUT_TICKET'">
                        <Icon size='.28' type="icon_common_prompt" class="icon icon-btn" />
                    ...
                    <div class="status-message gai-failed-info"
                        ...
                        <template v-if='psgStatuObj(psg.status).reason=="timeout"'>
                            您改签的航班{{psg.endorseShortInfo && '（'+psg.endorseShortInfo+'）'}}由于超时未补款（￥{{psg.gaiQianAddAmount}}）改签失败，如有需要可重新提交
                        </template>
                ...
                <div class="add-money" v-else-if="getIsinsuranceOrderPay(airline)">
                    ...
                    <div class="btn-group">
                        <SnButton class="btn cursorp" inline ghost :disabled="!selfOrder" type="warning"
                            @click="selfOrder && toPayIns(airline)">
                            去支付
                        </SnButton>
                    </div>
                </div>
    ...
    methods: {
                ...
                that.$set(that.flightTimeInterval, key, setInterval(function () {
                    ...
                    if (!!remainingTime) {
                        if (remainingTime / step <= 1) {
                            ...
                            that.$parent && (that.$parent.limitTime = that.flightRemaining[key]);
                            that.$forceUpdate();
                        }
        ...
        clearCountdown(key) {
            ...
            if (!!that.flightTimeInterval && !key) {
                Object.keys(that.flightTimeInterval).forEach((keyName) => {
                    that.clearCountdown(keyName);
                })
                return;
            }
            ...
            that.$parent && (that.$parent.limitTime = null);
            that.$forceUpdate();
        },
        /**
                    * 补款
