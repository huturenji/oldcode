...
export const data = function (extend) {
    return function () {
        ...
        let data = { // eslint-disable-line
            ...
            baseConfig: {}, //服务器的基础设置（附件的大小、数量等设置）
...
export const methods = {
    ...
    goReimburseDetail() {
        ...
        requestHandler.openPageLib('express/index.html#/invoice/flight?orderNo=' + this.orderDetail.orderNo);
    },
    /**
     * 再次预定
     */
    ...
    //         //     requestHandler.openPageLib('flight.html#/');
    //         //     return;
    //         //   }
    //         let loadData = {
    //             orderNo: that.orderDetail.orderNo,
     ...
     */
    paySuccCallback() {
        if (this.isInsAddMoney){
            this.getOrderDetail();
            extendUtils.showToast('保险支付成功');
            return
        }
    ...
    onConfirm(changeObj) {
        ...
        if (that.popState == 0) { //改签
            ...
            requestHandler.openPageLib('flight/index.html#/list?type=endorse');
    ...
    changeTicketState(person, airline, isRoundTrip, type) {
        ...
        extendUtils.throttle(()=>{
            ...
            if ((nowTime - startTime) / 1000 / 60 / 60 / 24 >= 365) {
                extendUtils.showConfirm('机票已过退改有效期，如需办理相关业务，请前往航司值机柜台现场办理或拨打客服热线<span class="text-phone icon-btn">'+this.orderDetail.providerPhone+'</span>', function () {
                    that.callPhone();
    ...
    getPsuInsurance(e) {
        let _this = this;
        let data = e; // eslint-disable-line
        //有保险订单才处理
        if (!!data.insuranceOrders && 0 < data.insuranceOrders.length) {
            let airLinesLeng = data.airLines.length;
            for (let i = 0; i < airLinesLeng; i++) {
                let PsgLength = data.airLines[i].passengers.length;
                for (let j = 0; j < PsgLength; j++) {
                    data.airLines[i].passengers[j].insuranceOrders = _this.getinsuranceOrdersByPsgID(data.airLines[i].passengers[j], data.insuranceOrders);
                }
            }
        }
        return data;
     ...
     * 异步调起支付组件
     */
    openPay(checkTravelAbnormal=true){
        ...
        var core = ()=>{
            if (!that.swpPayLoad){
                that.popLoading = true;
                setTimeout(()=>{
                    that.openPay(checkTravelAbnormal);
                },100);
                return;
            }
            that.popLoading = false;
        ...
        //先校验差标，再打开支付列表
        if (!!checkTravelAbnormal){
            this.$refs.travelAbnormal.checkTravelAbnormal({orderNo: this.chaOrder.orderNo || this.orderDetail.orderNo}).then(res=>{
        ...
        // checkTravelAbnormal && this.$refs.travelAbnormal.checkTravelAbnormal({orderNo: this.chaOrder.orderNo || this.orderDetail.orderNo}).then(res=>{
        //     this.checkSign = res;//保存校验成功的签名
        //     core();
        // })
    },
