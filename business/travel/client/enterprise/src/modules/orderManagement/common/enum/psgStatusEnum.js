...
/**
     *  改签单类型枚举
     */
export const gaiOrderTypeMap = {
    1: {text:'改期',showQuitIns:true},//改期
    2: {text:'升舱',showQuitIns:false},//升舱
    3: {text:'其他',showQuitIns:false}//证件号变更等等
}
/**
     * 机票：根据乘机人状态码返回状态对象
     * 注意：只返回退改签的状态
...
export function getFlightPsgStatusColor(status, index){
    try {
        ...
        return color[index] || '@text-color';//默认颜色是@text-color
    } catch (e){
        return "";
    }
}
...
/** ========================================火车乘客状态Start========================================== */
/**
 * 乘机人Status的枚举。（对应火车票TuiOrder里面的OrderStatus字段）
 * 每个状态有对应的文字描述和颜色，文字描述和颜色可以不唯一，因此用Array装填。可使用下标来选择需要返回的文字或颜色
 * 【注意】：原则上，name和color数组中，每增加一个场景（即一组值），所有状态下的数组都要加一个值。因为数组的每个下标都对应同一个场景。
 *  本对象是可以根据需求和场景进行扩展的，具体扩展方式是在状态对象中增加新的属性
 *
 ...
 * @returns {string} 状态对象
 */
export function getTrainPsgStatusObj(status){
    try {
        return trainPsgStatusEmum[status];
    } catch (e){
        return {};
    }
}
 ...
 * @returns {string} 状态名字
 */
export function getTrainPsgStatusName(status, index){
    try {
        if (index==null || index==undefined){
            index = 0;
        }
        let name = getTrainPsgStatusObj(status).name;
        if (name) {
            index = name.length>= index ? index : 0;//如果index越界，则默认取第一个
            return name[index];
        }
 ...
 * @returns {string} 状态颜色
 */
export function getTrainPsgStatusColor(status, index){
    try {
        if (index==null || index==undefined){
            index = 0;
        }
        let color = getTrainPsgStatusObj(status).color;
        if (color) {
            index = color.length>= index ? index : 0;//如果index越界，则默认取第一个
            return color[index] || '@text-color';//默认颜色是@text-color
        }
