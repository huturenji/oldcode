...
export default class CarOrder extends OrderClass{
    ...
    toProductReimburse(targetUrl){
        ...
        requestHandler.openPageLib(targetUrl+'?tripType=car&orderNo=' + this.orderDetail.orderNo + '&payType=' + this.orderDetail.useType)
    }
    ...
    toProductHome(url='car.html#/'){
        requestHandler.openPageLib(url + "?TEntry=2&ArriveTime=" + (this.car.arriveDate && new Date(this.car.arriveDate).getTime()) + "&DepartTime=" + (this.car.departDate && new Date(this.car.departDate).getTime()) + "&EndCity=" + this.car.arriveCityName + "&pageFrom=order");
    }
