...
def _do_deploy(args_list):
    ...
    if status != 0:
      ...
      if args.svn_url and args.svn_name and args.svn_password:
        #获取svn上一个版本的包地址
        svn_list_cmd = "svn list %s --username %s --password %s --no-auth-cache |sort|grep -e '.tar$'|tail -1" %(args.svn_url,args.svn_name,args.svn_password)
