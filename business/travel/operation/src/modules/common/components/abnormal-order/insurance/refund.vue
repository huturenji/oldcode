      ...
      <div class="scroolContent">
        ...
        <div class="recordForm">
          ...
          <div class="conditionDiv" :class="{'conditionDivUnable':recordType != 1}">
            ...
            <div
              class="conditionProName"
              :class="{'conditionOperationUn':recordType != 1}"
            >{{providerOrderName}}</div>
            <label
...
export default {
  ...
  data() {
    return {
      ...
      providerOrderName: null, //供应商名字，动态获取
      outData: {
        providerType: -1
      },
      ticketNum: [], //电子票号
  ...
  methods: {
    ...
    getProviderUrl(val) {
      ...
      for (let i = 0; i < that.providerTypeArr.length; i++) {
        if (val == that.providerTypeArr[i].providerType) {
          ...
          that.providerOrderName = that.providerTypeArr[i].providerName;
          break;
        }
      }
    },
    ...
    processOrder(processInfo) {
      ...
      tmHandler.cancelInsurance(request).then(
        function(res) {
          if (0 == res.resultCode) {
