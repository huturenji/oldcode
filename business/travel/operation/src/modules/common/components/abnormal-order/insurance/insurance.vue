<template>
  <!--保险手动订票模块-->
  <div class="manualBookingInsurance">
    <div class="normalUI" v-if="exceptionProcessedStatus >= 0 && exceptionProcessedStatus <= 3">
      ...
      <div class="bookBtnWrap">
        ...
        <div class="failBox" v-if="showFailBox">{{failMsg}}</div>
      </div>
      ...
      <div v-transfer-dom v-show="showAddRecordBox">
        <!-- 如果是2表示投保失败，兼容老数据，默认也是投保失败 -->
        <manualBookInsurePop
          v-if="orderItem.bpProductExceptionType==2 || !orderItem.bpProductExceptionType"
          :toGetMsg="isToGetMsg"
          :orderItem="orderItem"
        ...
        />
        <!-- 如果是8表示退保失败， -->
        <manualbookrefundinsurepop
          v-else-if="orderItem.bpProductExceptionType==8"
          :toGetMsg="isToGetMsg"
          :orderItem="orderItem"
          @closeAddRecordPop="closeAddRecordPop"
          @submitAddRecord="submitAddRecord"
...
const manualBookInsurePop = () => import("./reinsure.vue");
const manualbookrefundinsurepop = () => import("./refund.vue");
