<template>
    <div class="orderListPageTrain">
        <div class="headTitle">火车票</div>
        <orderListPage
            orderType="Train"
            :tabList="tabList"
            :pageData="pageData"
...
export default {
    ...
    data() {
        ...
        return {
            ...
            formDataBase: {
                ...
                postSaleOrder: undefined, //售后单入参，true 售后单 false非售后单
                orderType: 3, //写死，标识 机票
                pageIndex: 1, //分页-页码
    ...
    methods: {
        ...
        onTabSelected(tabIndex) {
            ...
            switch (tabIndex) {
                case 1:
                    ...
                    that.formDataBase.postSaleOrder = undefined
                    that.formDataBase.queryContentType = undefined
                    break;
                case 2:
                    result = travelfun.getStatusByCategoryUniq("t22");
                    that.formDataBase.postSaleOrder = undefined
                    that.formDataBase.queryContentType = 1 
                    break;
                case 3:
                    result = undefined;
                    that.formDataBase.postSaleOrder = true
                    that.formDataBase.queryContentType = undefined 
                    break;
                case 4:
            ...
            tmHandler.getOrders(resquestBody).then(
                function(res) {
                    if (0 == res.resultCode) {
                        ...
                        if(resquestBody.postSaleOrder){
                            orderArr &&
                                orderArr.sort(function(order1, order2) {
                                    
                                    let time1 = order1 && order1.postSaleLastApplyTime;
                                    let time2 = order2 && order2.postSaleLastApplyTime;
                                    if (!time1 || !time2) {
                                        return 0;
                                    }
                                    // 如果相同的，使用orderTime来比较
                                    if(time1 === time2){
                                        time1 = order1 && order1.orderTime;
                                        time2 = order2 && order2.orderTime;
                                    }
                                    if (time1 < time2) {
                                        return 1;
                                    } else if (time1 > time2) {
        ...
        //     let that = this;
        //     getProvinceCityCounty4Net().then(
        //         function(res) {
        //             if (0 == res.resultCode) {
