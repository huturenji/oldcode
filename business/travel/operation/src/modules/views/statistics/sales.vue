    ...
    <section>
      ...
      <div class="detailBox" v-if="!isLoading&&!noFlag">
          ...
          <div class="detailList" v-for="(item,index) in orderList" :key="index">
              <ul>
                  <li><img :src="require('assets//icon_'+getOrderDataFromList(item.orderType).eng+'.png')" alt="">{{getOrderDataFromList(item.orderType).text}}</li>
                  <li>{{item.orderNo}}</li>
...
<script>
  ...
  export default {
    ...
    props:{
        channelId:{default:''},useTypeId:{default:''},payTypes: { default: [] },companyId:{default:''},dateStart:{default:''},dateEnd:{default:''},searchStatus:{default:''}
    },
    data () {
     return {
         isLoading:true,
        ...
        pageMethod: {
            ...
            orderTypeList:{
                1:{text:'机票',eng:'flight',capital:'Flight'},
                2:{text:'酒店',eng:'hotel',capital:'Hotel'},
                3:{text:'火车票',eng:'train',capital:'Train'},
                4:{text:'快递',eng:'express',capital:'Express'},
                5:{text:'保险',eng:'insurance',capital:'Insurance'},
                6:{text:'打车',eng:'taxi',capital:'Taxi'},
            }
      }
    },
    created: function () {
         console.log("alive.sales.created")
    },
    methods: {
        OuterOrderFormat(order){ // 中文与单号分行显示
            let orderArr=[]
            // if(!!order){
            //     orderArr=order.split(' ')
            //     orderArr.splice(1,0,'<br>')
            // }
        ...
        getPaymentNameFn(payment){ // 获取支付方式
        ...
        goDetail(orderType,orderNo){
          ...
          var orderTypeName=that.getOrderDataFromList(orderType).capital
          this.$router.push({
        ...
        getSaleList(){
            ...
            tmHandler.getOrdersByAmount(request)
             .then((data)=>{
                 if(data.result && data.result.orders && data.result.orders.length!=0){
                    ...
                    that.$parent.totalPayAmount=data.result.totalPayAmount
                    console.log(data.result.totalPayAmount)
                    that.isLoading=false
                 }else{
                       that.noFlag=true
                      that.isLoading=false
        ...
        getOrderDataFromList(orType){
            const that = this;
            return that.orderTypeList[orType] || that.orderTypeList[0]
        }
    },
    mounted () {
         console.log("alive.sales.mounted")
        let that=this
            // if(utils.getSession('travelManageDateStart')!=undefined){
            //     that.$parent.dateStart=new Date(parseInt(utils.getSession('travelManageDateStart')))
            //     tmHandler.removeSession('travelManageDateStart')
            // }
            // if(utils.getSession('travelManageDateEnd')!=undefined){
            //     that.$parent.dateEnd=new Date(parseInt(utils.getSession('travelManageDateEnd')))
            ...
            //     that.pageMethod.currPage=parseInt(utils.getSession('travelManageCurrPage'))
...
section{
     .empty-message {
        ...
        width: 81px;
        height: 92px;
        background: url(~assets//icon_empty.png) no-repeat 0 0 transparent;
        margin-bottom: 16px;
        }
    }
    .loading-container{
