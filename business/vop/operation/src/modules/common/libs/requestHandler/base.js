...
class BaseHandler {
     ...
     * @param {*} method 
     */
    sendApiRequest(url, param, method, headerObj = {},timeout, notAuth, noUseBsl, loadFile = false) {
        let that = this
        //如果长时间不操作，则登出
        ...
        if (loadFile){
            ...
            paramCopy = param && JSON.parse(JSON.stringify(param));
        }

        let realUrl = url.indexOf("http") == 0 ? url : this.defaultUrl + url
        return this.apiClient.apiCallHandler(realUrl, paramCopy, { "method": method, 'timeout':timeout, headers: headerObj,noUseBsl: noUseBsl }, notAuth);
    }
}
