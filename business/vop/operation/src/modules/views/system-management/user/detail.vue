    ...
    <div class="userdetails">
        <div class="btnbox">
            <el-button 
                ...
                plain 
                @click="deluser=!!1"
                v-if="hasTheAuth('delUser')"
            >删除</el-button>
            <el-button 
                type="primary" 
                @click="gotoedit(formData.userId)"
                v-if="hasTheAuth('editUser')"
            >编辑</el-button>
        </div>
        <div>
            <el-row>
                ...
                <el-col :span="24">
                    ...
                    <div class="userTypeList">
                        <div class="userlist">
                            <div
                                v-for="(item,index) in formData.roles"
                                :key="index"
                                class="payTypeLi"
                            >
                                <el-tooltip 
                        ...
                        <div
                            v-show="formData.roles && formData.roles.length==0"
                            class="notypeBox"
                        >
                            <div>{{ "该人员暂无所属角色" }}</div>
                        </div>
...
export default {
    ...
    methods: {
        ...
        queryUserInfo() {
            let that = this;
            that.$iLoading.show();
            systemhandler.getUser({ userId: this.userIds }).then(res => {
                that.$iLoading.hide();
                if (res.resultCode == 0) {
        ...
        defineDel() {
            ...
            systemhandler.deleteUser({ userId: this.userIds }).then(res => {
