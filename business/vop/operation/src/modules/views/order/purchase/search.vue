<template>
    <div class="all-order">
        <template v-if="showPurchaseAuth">
            <search-header
                @on-export="onExport"
                @on-search="search"
                ref="PURCHASE"
            ...
            >
                ...
                >
                    <template slot-scope="scope">
                        <span>{{ orderAmount(scope.row.purchaseAmount) }}</span>
                    </template>
                </el-table-column>
                <el-table-column
                    width="140"
        ...
        <noAuth :isShowModal="!showPurchaseAuth" />
    </div>
</template>
<script>
...
export default {
    ...
    created() {
        this.showPurchaseAuth && (this.tabActive = "PURCHASE");
    },
    mounted() {
        let _this = this;
