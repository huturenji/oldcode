...
class MyTripPage(BasePage):
    ...
    # 行程卡片中入住人的名字
    people_name_loc = "(//div[text()='{}'])[last()]/../descendant::span[@class='name'][{}]"
    ...
    def click_hotel_trip(self, name):
        ...
        self.switch_to_new_window_wait(
            ...
            self.driver.find_elements(*locator)[-1],
            "我的行程页面"
        )

    @allure.step("我的行程页面：上滑页面找到需要的酒店")
    def swipe_find_element(self, name):
        locator = (By.XPATH, self.hotel_name_loc.format(name))
        self.swipe_up_find_element(locator, total=10)
