...
from test_cases.hotel_test.process import quick_payment
...
@allure.suite("酒店测试套件")
@allure.feature("酒店模块")
class TestHotelQuickPayment:

    @pytest.mark.skip("公款闪付目前只能在sit环境的‘梦工场’企业下运行")
    @pytest.mark.hotel_quick_payment
    @allure.severity("blocker")
    @allure.testcase("点我查看禅道用例……")
    @allure.story("测试场景：不同支付方式下单")
    @allure.title("测试用例标题：预订酒店时，使用‘公款闪付’支付")
    @pytest.mark.parametrize("test_info", hotel_online_pay_search_keywords)
    def test_hotel_quick_payment(self, test_info, driver):
        # 初始化酒店首页
        hotel_home_page = HotelHomePage(driver)
        # 初始化酒店列表页
        hotel_list_page = HotelListPage(driver)
